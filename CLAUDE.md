...
* **Non aspettare per una build in un altro clone**, e non dichiararla dannosa: `Binaries/` è per clone e l'Engine è una *installed build*. Aspetta invece per: stesso clone, Editor che tiene il DLL, **qualunque misura di tempo**, e qualunque target **Engine**. La misura sta in `AGENTS.md` §9.
  * ⚠️ **«Misura di tempo», non «misura di performance», e la differenza è costata una tornata.** Questa riga diceva `performance`, che si legge come *benchmark* — e il 2026-09-20 ho concluso che la mia build non disturbasse una sessione che stava misurando un **tasso di intermittenza**. Non è un benchmark e vive nei tempi identicamente: venti run ripetute il cui esito dipende da quanto il resolver è stato veloce. La domanda da farsi è quella di `AGENTS.md` §9 — **cosa stai misurando**: un esito non lo cambia una build altrui, un tempo sì.
