...
- ⛔ **Il repository non ha più gate Python.** Restano **Node 22** in `tools/radar/` — rubrica dei
  ...
  `doc-tables.ts` esce `1`, e non da oggi**: dieci righe di `docs/archive/` non hanno la larghezza
  delle sorelle, più due che non stanno in nessuna tabella. Questa riga dichiarava «verde anche con
  `--with-archive`» ed era **già falsa** prima che le orfane esistessero — misurata il 2026-08-25.
  L'archivio è storico e resta fuori dal giro normale. Ciò che **non** copre è nel suo docstring —
  ancore, URL, percorsi in prosa, e gli altri Markdown della radice, che sono materiale importato — e
  va letto prima di fidarsene.
