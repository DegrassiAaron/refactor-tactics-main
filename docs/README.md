...
| **Lore e worldbuilding** — Harmonic Coupling, The Refactor, ARC, le quattro fazioni | [`product/lore-e-worldbuilding.md`](product/lore-e-worldbuilding.md) | 🟡 `PROPOSED`, **non normativo e non ratificato** ([D-246](decisions/RT_PDR_00_Decision_Log.md)). Non compare nella gerarchia delle fonti qui sopra ed è voluto: nessun termine di questa pagina è nel canone, e i quattro `FactionId` hanno **zero** occorrenze in `Source/`. È la sorgente interna della [`Lore`](https://github.com/DegrassiAaron/refactor-tactics-main/wiki/Lore) player-facing, e tiene il livello con spoiler che la Wiki dichiara di non pubblicare. La ratifica passa da *«Identità originale (nomi, lore)»* in [`OPEN_DECISIONS.md`](OPEN_DECISIONS.md) |
