...
> | **Conoscenza parziale** | È incompleta l'informazione sulle **unità e sugli eventi** — LOS geometrica + rilevamento + rumore + ultimo contatto ⇒ **Team Knowledge** a tre livelli. Il rumore è un **secondo canale**, propagato con interi sul grafo. 🔴 **Questa riga diceva anche *«Non è "fog of war": la mappa statica è nota"*, e [D-225](../decisions/RT_PDR_00_Decision_Log.md) l'ha resa falsa il 2026-08-28**: in v0.1 la geometria che nessuno della squadra osserva **non viene disegnata**. La distinzione sopravvive ma cambia asse — non più *«geometria nota, unità ignote»*, bensì **geometria nascosta in v0.1**, mentre restano P1 la fog of war come sistema competitivo e ciò che riguarda unità ed eventi | `gameplay/brief-conoscenza-parziale.md` · E13 · [D-225](../decisions/RT_PDR_00_Decision_Log.md) |
...
> | **Formato di partita** | ✅ **Deciso il 2026-08-30**: **3v3 è il formato Standard definitivo**; **2v2** resta **Skirmish / vertical slice** ed è ciò che la v0.1 consegna; **4v4+** resta **Operations / stress e scala** (E17), non formato competitivo. ⚠️ La riga diceva *«non è deciso»* e chiedeva di non trattare il 3v3 come formato di prodotto scelto: dopo `D-256` è l'opposto, ed è una **decisione d'autore**, non una misura. ⛔ **Non sposta lo scope della v0.1** | [D-256](../decisions/RT_PDR_00_Decision_Log.md) *(supera la parte «formato non deciso» di [D-011](../decisions/RT_PDR_00_Decision_Log.md))* |
...
> | **Ownership contenuti** | Tre livelli che **non** diventano una sola fonte dati: **Ability/Action Definition → singolo owner** (eroe, equipaggiamento, oggetto, regola generica); **interazioni → sistemi** (`Wet`, conduttività, `Push`, cover, LOS, rumore); **sinergie/fazioni/scenari → riferimenti ed esempi**, mai una seconda fonte di numeri o di logica. Nessun bonus implicito di coppia/fazione, nessuna dipendenza hard-coded fra `HeroId` | [D-029](../decisions/RT_PDR_00_Decision_Log.md) · [ADR-0006](../decisions/adr-0006-ownership-abilita-sinergie.md) · [`../gameplay/spec-ownership-abilita-interazioni-sinergie.md`](../gameplay/spec-ownership-abilita-interazioni-sinergie.md) |
>
...
4. **Determinismo**: niente `DeltaTime` non controllato nella logica dei turni; niente dipendenza dall'ordine di container non ordinati; ogni RNG usa seed/stream espliciti; ogni formato serializzato è versionato.
   > **Le sei procedure che lo verificano, e lo stato di ciascuna** — misurato il 2026-08-15 su `a7c4a799` per [`#578`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/578), che chiede che una procedura scartata porti la ragione **qui** e non solo nella issue: *«una procedura scartata in silenzio si ripropone fra sei mesi»*. L'elenco delle sei è di **PDR-05 §10**.
   >
   > | # | Procedura | Stato | |
   > |---|---|---|---|
   > | 1 | **Golden** | ✅ | 5 test, fra cui `Simulation.GoldenCorpusMatches` e `GoldenCorpusDetectsDivergence` |
   > | 2 | **Permutation** | ✅ | ≥13 test `*.PermutationInvariant`, più `Simulation.ChecksumStableAcrossPermutations` |
   > | 3 | **Repeat** | ✅ **×100** · ⛔ **×1000 fuori scope** | vedi sotto |
   > | 4 | **Seed** | ✅ **chiuso da un guardia**, non da un test di equivalenza | vedi sotto |
   > | 5 | **Frame-rate** | ⛔ **non applicabile** | vedi sotto |
   > | 6 | **Packaged** | ❌ **aperto** | resta l'unica delle sei senza copertura né ragione |
   >
   > **3 · Repeat ×1000 — fuori scope, e la ragione non è solo il costo.** La procedura **esiste già a ×100**: `RTSimulationDeterminismTests.cpp` dichiara `constexpr int32 Repetitions = 100` dentro `Replay.Verifier.ResimulationIsDeterministic`, con confronto di `StateHash` e `Outcome`. Non si trova cercando `Repeat` fra i nomi dei test, ed è la ragione per cui #578 la dava per mancante. Il decuplo non si prende perché: (a) `.github/workflows/` **non esiste in questo repository, per scelta** — non c'è CI dove pagarlo né nightly dove metterlo; (b) il bersaglio non cambia. Ciò che le ripetizioni catturano — un `TMap` iterato in ordine diverso, un puntatore usato come chiave, un indice che dipende dall'ordine di spawn — è già colto a 100, perché `RunIsolated` costruisce un `UWorld` **nuovo** a ogni giro e gli indirizzi cambiano. ⚠️ **Da rileggere se nasce una CI**: allora il costo cambia di natura, e con esso questa decisione.
   >
   > **4 · Seed — il gap si chiude al contrario.** «Stesso seed → stesso output» su un progetto **senza alcun RNG** confronta una funzione deterministica con sé stessa: verde per costruzione, verde anche a resolver rotto. Misurato: zero occorrenze di `FMath::Rand`, `RandRange`, `FRandomStream` e simili in tutto `Source/RefactorTactics/`, test compresi; e `RTTestScenario.h` lo dichiara già («Seed dichiarato ma non consumato»). L'invariante che morde è la negazione, ed è coperta da **`Simulation.SeedIsDeclaredAndUnconsumed`**: due seed **diversi** devono dare lo stesso risultato. Quando quel test diventa rosso, un RNG è entrato — e allora **questa riga dell'invariante #4 diventa esigibile**: ogni estrazione deriva il proprio seed come `Hash(TurnSeed, ActionId, RollKind)` (PDR-05 §5), il guardia va sostituito da due test, e la sostituzione è una decisione da registrare qui.
   >
...
6. **Privacy dell'intento**: le intenzioni di pianificazione non raggiungono i client avversari — stato server + replica filtrata per squadra + autorizzazione server-side (invariante di `Intenti condivisi`). Nell'MVP offline: nessuna mossa avversaria mostrata/replicata durante la pianificazione.
   ...
   >
   > **Presidiato strutturalmente dal 2026-09-04** — [#589](https://github.com/DegrassiAaron/refactor-tactics-main/issues/589), PDR-04 §9 passo 6. Fino a quel giorno l'invariante aveva **quattro** test, e tutti e quattro guardavano lo stesso lato: che `URTIntentPrivacyLibrary::FilterForTeam` consegni la cosa giusta — privacy **logica**. Nessuno guardava la **superficie di replica**, e la differenza non è accademica: un filtro perfettamente corretto non impedisce a `FRTPlannedIntent` di finire dentro una `UPROPERTY(Replicated)` in un refactor futuro, e a quel punto il byte parte senza che nessun test logico se ne accorga — il *leak indiretto* di PDR-04. Ora un tipo si dichiara `USTRUCT(meta = (RTServerOnly))` — la classe *Server-only* di PDR-04 §2 — e `RefactorTactics.Privacy.ServerOnlyTypesAreNotReplicated` verifica sulla reflection che nessuna proprietà replicata e nessun parametro di RPC lo raggiunga, nemmeno annidato dentro un `TArray`. ⚠️ **Non sostituisce il canary di `M10.3`**, che resta l'unica prova che nessun byte sia *effettivamente* partito, e che va eseguito **packaged**: [`procedura-canary-anti-leak.md`](../technical/systems/procedura-canary-anti-leak.md).
...
- **P1**: 4v4, 6+ eroi, draft, fog of war *(⚠️ il **nascondimento della geometria** non osservata è passato in **v0.1** con [D-225](../decisions/RT_PDR_00_Decision_Log.md); resta qui la fog of war come sistema competitivo)*, replay, spectator, matchmaking, ranking, **Intenti condivisi** completo.
...
> `docs/archive/src/design/overwatch-e-fast-reaction.md` propone l'Overwatch come primo caso di un modello
