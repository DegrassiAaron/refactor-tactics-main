...
4. **Determinismo**: niente `DeltaTime` non controllato nella logica dei turni; niente dipendenza dall'ordine di container non ordinati; ogni RNG usa seed/stream espliciti; ogni formato serializzato è versionato.
   ...
   >
   > **6 · Packaged — resta aperto, ma non richiede codice nuovo.** Confronto di `StateHash` fra build Development e Shipping. È l'unica delle tre voci rosse di #578 che **non si chiude con una ragione**. Misurato però che gli strumenti ci sono già: `ScenarioHarness/` **non ha esclusioni di build** — nessun `#if !UE_BUILD_SHIPPING`, nessun `WITH_EDITOR` — e `RTTestReportWriter` scrive `Saved/RTTests/<Id>/<Run>/result.json` con il campo `stateHash` (`%08x`) accanto a `seed` e `result`. La procedura è quindi: eseguire `rt.Test.Run <ScenarioId>` nelle due build e confrontare quel campo nei due `result.json`. ⚠️ **Il punto di rischio da verificare eseguendo** è l'accessibilità della console in Shipping, non la presenza del codice: i comandi sono registrati con `FAutoConsoleCommand…`, e se in Shipping la console non è raggiungibile serve un'altra porta d'ingresso (un flag di commandline, o un'esecuzione all'avvio). ⛔ **Non scritta qui**: la procedura operativa appartiene a [`../technical/test-e-diagnosi.md`](../technical/test-e-diagnosi.md), che non è nel `writable` di questa track — D-139, riallocazione e non «solo questa riga».
