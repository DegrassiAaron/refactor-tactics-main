...
4. **Determinismo**: niente `DeltaTime` non controllato nella logica dei turni; niente dipendenza dall'ordine di container non ordinati; ogni RNG usa seed/stream espliciti; ogni formato serializzato è versionato.
   ...
   >
   > **6 · Packaged — resta aperto, ed eseguirlo ha spostato il motivo.** Confronto di `StateHash` fra build Development e Shipping. ✅ **In Development la procedura gira per intero**, misurato su un pacchetto vero: un solo cook e due binari (`-clientconfig=Development+Shipping`), `AUTO-RUN Movement.Collision` → `PASS (3/3 assertion)` → `stateHash 572184bb` in `Saved/RTTests/<Id>/<Run>/result.json`. 🔴 **In Shipping no, e le due cause sono misurate — nessuna delle due è «manca il codice».** (1) `Scenarios/` **non entra nel pacchetto**: sono `.json` fuori da `Content/`, il cook non li vede e lo staging non li copia — l'indice dice «0 scenari», e `DefaultGame.ini` non ha mai dichiarato `+DirectoriesToAlwaysStageAsUFS`. (2) **In Shipping `-dpcvars` è compilato fuori**: in `DeviceProfileManager.cpp` tutto il parsing sta dentro `#if !UE_BUILD_SHIPPING`, quindi `rt.Test.Scenario` non viene mai impostata e il `GameMode` allestisce la partita normale. ⚠️ **La previsione era giusta a metà, e va detto**: la riga precedente scriveva «il rischio è l'accessibilità della console in Shipping» — vero, ed era la causa (2); la causa (1) non l'aveva vista nessuno, e si è manifestata per prima. ⛔ Nessuno dei path che chiudono le due cause è nel `writable` di questa track — `Config/` è `integration_only`, `RTGameMode.cpp` e `BP_GameMode.uasset` non sono assegnati: misurati e registrati su **#926**, non scritti. La procedura operativa, con i comandi, sta in [`../technical/test-e-diagnosi.md`](../technical/test-e-diagnosi.md).
