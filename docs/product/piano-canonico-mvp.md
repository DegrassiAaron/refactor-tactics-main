...
4. **Determinismo**: niente `DeltaTime` non controllato nella logica dei turni; niente dipendenza dall'ordine di container non ordinati; ogni RNG usa seed/stream espliciti; ogni formato serializzato è versionato.
   ...
   >
   > **6 · Packaged — resta aperto, ma metà causa è caduta.** Confronto di `StateHash` fra build Development e Shipping. ✅ **In Development la procedura gira per intero**, misurato su un pacchetto vero: un solo cook e due binari (`-clientconfig=Development+Shipping`), `AUTO-RUN Movement.Collision` → `PASS (3/3 assertion)` → `stateHash 572184bb` in `Saved/RTTests/<Id>/<Run>/result.json`. ✅ **Prima causa chiusa il 2026-08-16**: `Scenarios/` non entrava nel pacchetto — sono `.json` fuori da `Content/`, e l'indice diceva «0 scenari» — e ora ci entra con `+DirectoriesToAlwaysStageAsUFS=(Path="../Scenarios")`. ⚠️ Il `Path` è relativo a `Content/`, non alla radice: scritto senza `../` stagea zero file **lasciando la build `SUCCESSFUL`**, e l'unico segnale è un warning nel log della cottura. Verificato da un pacchetto pulito, senza copie a mano, e con lo **stesso** hash ottenuto prima copiando la cartella: il meccanismo non ha cambiato l'esito. 🔴 **Seconda causa aperta, ed è ciò che tiene ferma questa voce**: in Shipping `-dpcvars` è **compilato fuori** — in `DeviceProfileManager.cpp` tutto il parsing sta dentro `#if !UE_BUILD_SHIPPING` — quindi `rt.Test.Scenario` non arriva e il `GameMode` allestisce la partita normale. Serve una porta d'ingresso che `UE_BUILD_SHIPPING` non tocchi: **#926**. ⚠️ **La previsione scritta qui il 2026-08-15 era giusta a metà, e resta scritto**: diceva «il rischio è l'accessibilità della console in Shipping» — vero, ed è la seconda causa; la prima non l'aveva vista nessuno e si è manifestata per prima. La procedura operativa, con i comandi, sta in [`../technical/test-e-diagnosi.md`](../technical/test-e-diagnosi.md).
