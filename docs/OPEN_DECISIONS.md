...
| `INT-8` | Un'azione che opera su una **struttura** conta come un attacco su chi le sta accanto? | ⏳ **Risposta data il 2026-08-27, codice non ancora scritto** — resta qui perché ora aspetta chi lo scrive, non chi decide. **(1) Il colpo è UN concetto, non quattro.** `FRTHexAttackHit` governa insieme danno, trigger `HitByDirectAttack`, `EnergyOnHit` e `Marked`; un'azione o produce un colpo o non lo produce, e non esistono vie di mezzo per consumatore. Implementabile perché `Plan.Hits` nasce in **un punto solo** (`RTHexCombatLibrary.cpp:364`) e ha molti consumatori: il cancello si mette una volta e lo ereditano tutti. **(2) Il predicato si DICHIARA**: campo `bCountsAsAttack` su `FRTActionDef`, **`false` di default**. Fail-closed nel verso giusto — un attacco che si dimentica il campo non fa niente e lo prende il primo test, mentre una non-aggressione che si dimentica **non può** diventare un attacco. È il verso opposto a `bAllowsReaction`/`bFriendlyFire`/`bCanBeInterrupted`, e la rottura di simmetria è deliberata: quelli descrivono permessi, questo un'identità. **(3) L'energia non è più una domanda**: si carica per un colpo, e un colpo è un'aggressione — quindi è ricompensa per l'aggressione, per costruzione. ⚠️ **Conseguenza di bilanciamento già presa**: una cura a bersaglio singolo non caricherà energia. Non rompe però la cura — misurato: gli effetti passano da `URTActionEffectLibrary::ProduceEvents(Instance)` e **non** dai colpi, quindi `Heal` a `false` cura lo stesso. ⚠️ `MarkTarget` va a **`true`**: senza colpo non c'è `ApplyMarkedBy`, che è anche l'unico posto da cui si sa **quale squadra** ha marchiato. 🟢 **E il campo TOGLIE un'eccezione invece di aggiungerne una**: `Action.ModifyArc` è già intercettata **per nome** a `RTTurnManager_Blast.cpp:424` (`IsCoreAction(Def, ActionModifyArc)`) con la motivazione *«non è un intento d'attacco … un'azione che cambia la topologia si metterebbe a fare danno»* — cioè questa stessa domanda, risolta a mano per una azione sola, con un branch nel core che CLAUDE.md vieta. Diventa `bCountsAsAttack = false`. **Migrazione**: 17 azioni (11 core in fase `Attack`, 6 d'eroe). ⚠️ **Cosa manca per chiuderla**: il codice, il `D-nnn` che lo accompagna (un `D` senza owner né test sarebbe una dichiarazione non verificabile), e il ribaltamento di `Scenarios/Spec/Reaction/InteractCountsAsADirectHit.json`, che oggi pinna il comportamento vecchio |
