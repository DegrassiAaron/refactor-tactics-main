...
| 🔴 `ICON-TAX-7` — **L'UNICA CHE RESTA** | **`Result`** (2 chiavi: `Success`, `Failure`) | 🔴 **La domanda era posta sull'asse sbagliato.** *Lo zero era stato ribaltato sul suffisso `Result`; le chiavi però dicono «Mission Success/Failure», e l'esito di partita **esiste ed è canonico sotto il nome `Outcome`**.* Restano veri: gli `ERT*Result` sono per dominio, e `UI.Icon.Objective` ha **zero** glifi nel manifest generato. ⚠️ Ed è l'unica riga marcata **`CORE`**. 🔑 **Tenuta aperta per DECISIONE il 2026-09-22 ([D-434](decisions/RT_PDR_00_Decision_Log.md)), non per indecisione: nessuna delle due uscite è pronta.** (c) manderebbe fuori le uniche due chiavi marcate `CORE`; (a) → `Objective` punta all'unica famiglia con **zero** glifi nel manifest generato (`grep -c UI.Icon.Objective Content/Icons/manifest.json` → 0), e il commento di quell'enum («neutro, in cattura, conteso, posseduto, bloccato») non copre un verdetto di partita. **Si riapre quando si avvera UNA delle due**: (i) esiste una superficie che mostra l'esito di partita con icone — `WBP_RT_ResultScreen` oggi non ne referenzia nessuna; (ii) `Objective` riceve la propria famiglia di glifi. ⛔ E va riposta sull'asse giusto: l'esito di partita è canonico sotto **`Outcome`**, e il **pareggio** che la v0.1 mostra più spesso non esiste nel manifest sotto nessun nome |
