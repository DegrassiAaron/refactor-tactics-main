...
| ~~`STA-5`~~ | ~~`Unbalanced` e `Prone` si **anticipano** fuori dall'epic G.2, o restano dietro al motore del ghiaccio?~~ | ✅ **Chiusa il 2026-09-04 da [D-319](decisions/RT_PDR_00_Decision_Log.md)**: **si anticipano**, e diventano lavoro proprio. Lo scivolamento applica `Unbalanced` durata `2`; una spinta subita mentre si è sbilanciati applica `Prone` durata `2`, che **1 MP** rimuove in anticipo — il pagamento compra il turno, non l'uscita. La rappresentazione adottata è **durata + `StatusTurns.Remove`**: nessun contratto nuovo, e risolve il caso di chi ha `Root` o budget esaurito, che la durata solleva comunque. 🔑 **La misura che ha deciso la voce non era nel brief**: il bot è **cieco** — `0` occorrenze di `HasStatus`/`TAG_Status` e `0` di `Push`/`Pull` in `Source/RefactorTactics/Bot/` — quindi in v0.1 la catena sarebbe stata **a senso unico**, uno strumento del giocatore invece che una minaccia. La Fase 1 porta perciò un termine **mirato** in `ScorePlan`, che **non** dipende da `STA-4`: quella è prerequisito del canale generico, non di un singolo termine. ⚠️ **L'uscita *(c)* — anticipare il solo `Unbalanced` — è stata scartata** perché il suo effetto caratteristico, `+1` cella alla spinta, esiste per portare a `Prone`: da solo è un debuff senza culmine. ⛔ **`G.3 non parte senza il fuzzing` resta vero**: CP 12.6 lega il fuzzing allo **slide a catena**, che la decisione non tocca. Il perimetro residuo di G.2 è Traction e Stability per eroe. **Istruttoria conservata** |
