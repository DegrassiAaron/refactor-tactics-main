...
> 🔁 **Aggiornato il 2026-09-01 — la tabella ha otto righe, e le ultime due non sono di questo mandato.**
> `COV-11` e `COV-12` vengono dal kit d'autore *Cover · Melee · AoE · Facing v0.1*
> ([referto](roadmap/plans/cover-melee-aoe-facing-consolidamento-spec-panel-2026-09-01.md)), consumato il
> giorno dopo `D-305`. Stanno qui perché è la stessa materia: `COV-11` ha lo stesso owner
> ([`#1833`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1833)), `COV-12` ha il proprio
> ([`#2009`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2009)). ⛔ **Nessuna delle due è
> fra le ventuno clausole misurate sopra**, e i conteggi «queste sei» di questa sezione restano veri per il
> mandato che nominano.
...
| `COV-11` | Il **livello** di copertura continua a determinare vista, passo e proiettili, o le domande si separano in flag indipendenti sul segmento? | È il `Core Principle` del kit d'autore v0.1 (§1), e chiede l'**opposto** di una scelta già argomentata. Oggi `URTHexCoverLibrary::BlocksTraversal` è **un solo** predicato — *«l'UNICA funzione che vista, grafo e combat interrogano»* — e [`spec-copertura-alta-cp92.md`](gameplay/spec-copertura-alta-cp92.md) scrive il perché: *«una copertura che togliesse la vista ma non il passo sarebbe un difetto invisibile finché qualcuno non ci cammina attraverso»*. Il kit non cita quell'argomento e non gli risponde. 🔑 **La domanda è una sola benché tocchi tre sezioni**: i preset a quattro valori (§2) e il modello moltiplicativo `Low 75% / High 50%` (§11) **presuppongono** entrambi che `High` lasci passare qualcosa, cioè presuppongono §1 — stanno o cadono insieme. ⚠️ **Costo misurato**: `BlocksTraversal` ha **15** file consumatori in `Source/`; `BlocksLOS`, `BlocksProjectiles`, `BlocksMeleeContact`, `CoverArc`, `Vaultable` ne hanno **zero**. ⛔ **Non riapre `D-271`** (vocabolario `None`/`Low`/`High`) né le percentuali, che restano `BAL-*`: chiede se il livello resta *l'unico* asse, non quanti livelli esistano. ⚠️ Se la risposta è «si separano», ogni flag nuovo deve arrivare con la sua riga in `URTHexMapAsset::ValidateMap`, o l'ortogonalità diventa un generatore di mappe incoerenti. **Istruttoria**: [`roadmap/plans/cover-melee-aoe-facing-consolidamento-spec-panel-2026-09-01.md`](roadmap/plans/cover-melee-aoe-facing-consolidamento-spec-panel-2026-09-01.md) §3. **Owner**: [`#1833`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1833) 🔁 **Aggiornata il 2026-09-01 — l'owner non è `#1833`, ed è un errore di perimetro.** Il thread di [`#1833`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1833) ha chiarito (2026-08-31) che le `COV-*` aprono la copertura **selezionabile in posa** ([#1829](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1829)), mentre il perimetro **ambientale** è *«confermato implementato e separato dal Facing per firma […] non tocca la geometria del bordo già in vigore»*. Questa voce sta sulla **geometria del bordo ambientale** — `BlocksLOS`/`BlocksProjectiles`/`BlocksTraversal` su un segmento — quindi **non appartiene alla serie**. ⛔ **L'ID non si rinumera**: è già citato dal referto su `main`, e rinominarlo lascerebbe riferimenti stale senza che nessun gate lo veda. **Owner della semantica**: [`spec-copertura-alta-cp92.md`](gameplay/spec-copertura-alta-cp92.md) · **nessuna issue consumatrice**: non c'è lavoro che la aspetti, ed è la ragione per cui può restare aperta senza costo. |
| `COV-12` | La riga di [`D-302`](decisions/RT_PDR_00_Decision_Log.md) *«area = centro d'impatto→bersaglio»* è **inerte**, oppure supera [`spec-copertura-cp91.md`](gameplay/spec-copertura-cp91.md) §4 *«le aree non sono mai ridotte»*? | 🔴 **Due autorità realmente applicabili, e [`D-282`](decisions/RT_PDR_00_Decision_Log.md) punto (5) vieta di scioglierle con una gerarchia**: il conflitto *«si registra e si escala all'owner competente»*. Per il punto (2) della stessa voce la precedenza è **tipizzata** — il Decision Log possiede le decisioni esplicite, le **specifiche possiedono la semantica delle regole** — e «l'area riceve mitigazione?» è semantica, quindi l'owner è `CP 9.1`. ⚠️ **`D-302` non nomina mai `CP 9.1`**: zero occorrenze di `CP 9.1`/`9.1`/`cp91` nel corpo della voce, e la frase nasce chiudendo `COV-5`, cioè dentro il modello di copertura **selezionabile** che non è implementato. 🔑 **Il ramo `Area` non è legacy**: `CP 9.1` §3(1) lo dichiara normativo, §4 lo registra fra le decisioni **con l'alternativa scartata** — *«trattare l'area come un proiettile con un'origine avrebbe aggiunto una geometria in più per non cambiare nessun esito»* —, [`RT_ActionCatalog_v0.1.md`](balance/RT_ActionCatalog_v0.1.md) lo ripete per `Circular AoE`, e `Cover.LowCover.AoESameSide` lo pinna con un mutation test già in tabella ([#1529](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1529)). **Tre uscite**: *(a)* `D-302` inerte per l'area → nota di delimitazione, zero `Source/`, ma quella riga resta **senza consumatori**; *(b)* `D-302` supera `CP 9.1` → cambio di **regola di gioco**, con `CP 9.1`, il catalogo, il test e il corpus golden da rifare; *(c)* due oggetti distinti — area ambientale non ridotta, area selezionabile sì — che oggi **non hanno nomi diversi**. ⚠️ **L'uscita (b) non si fa togliendo una riga**: `EffectiveCoverReduction` passa `Attacker.Cell` a `IsInFrontalArc`, quindi senza il ramo il test di `CP 16.2` userebbe la cella dell'attaccante proprio dove `D-302` chiede il centro d'impatto — serve un cambio di firma. **Istruttoria**: [`roadmap/plans/cover-melee-aoe-facing-consolidamento-spec-panel-2026-09-01.md`](roadmap/plans/cover-melee-aoe-facing-consolidamento-spec-panel-2026-09-01.md) §4. **Owner**: [`#2009`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2009) 🔁 **Aggiornata il 2026-09-01 — la domanda non si pone più nei termini in cui era scritta, e l'istruttoria è nel commento di [`#2009`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2009#issuecomment-5490762502).** ✅ **Nessuna delle tre uscite**: `D-302` punto (3) chiude `COV-5` **e dichiara di chiudere tre quarti di `FAC-13`**, cioè la policy `FromSource`/`FromTrajectory`/`FromImpactCenter` — quindi risponde a *«da dove arriva il colpo per i test DIREZIONALI»*, non a *«l'area è mitigata»*. Le due sedi non si sovrappongono: `CP 9.1` §4 possiede la **mitigazione** (nessuna, per le aree, e resta vera), `D-302` possiede la **direzione d'impatto** (centro d'impatto, ed è **accettata e non implementata**). 🔑 **Il consumatore vero non è la copertura ma il `Guard`**: `IsInFrontalArc` ha due chiamanti di produzione, e quello in `EffectiveCoverReduction` è inerte per le aree (`Reduction` è già `0`), mentre il pool frontale di [`D-292`](decisions/RT_PDR_00_Decision_Log.md) in `RTTurnManager.cpp:4677` passa la cella dell'**attaccante** anche sotto un'area — quindi una granata fatta cadere davanti al bersaglio da un lanciatore alle spalle produce `RearHitBypassedGuard`. ⚠️ **Nessun test copre il caso**: nessun file combina `TAG_Status_Guarded` con `ERTAbilityShape::Area`. ⛔ **Non è più una domanda d'autore**: è implementazione di una decisione accettata, e il dato serve (`FRTHexAttackIntent::TargetCell`, con `Hit.IntentIndex` che mappa 1:1). **Owner**: [`#2009`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2009) |
| `PER-5` | L'esposizione **geometrica** osservatore-relativa entra come grado discreto, e con quali nomi? | Oggi `URTHexVisionLibrary` risponde *vede / non vede* **cella-a-cella**, e non esiste un grado intermedio derivato dalla geometria intra-cella. ✅ **La geometria del mandato non va ri-derivata, ed è stato verificato**: le baseline §13/§14 sono coerenti con l'anello canonico `E → NE → NW → W → SW → SE` di `ERTHexDirection` — per `Edge_NE` gli adiacenti sono `E` e `NW`, per `Corner_NE_E` gli immediatamente esterni sono `SE` e `NW`, ed è esattamente ciò che le due tabelle dicono. 🔴 **Ciò che non si può adottare è il terzo nome.** `Exposed` è occupato due volte: `TAG_Status_Exposed` (`RTGameplayTags.h:9`) è *«+5 al PRIMO danno diretto»*, il prezzo che paga `Action.Sprint`, letto dal resolver di combattimento (`RTCombatResolver.h:78`); `FRTExposedEdge` (`RTVisibilityBorder.h:21`) è il **bordo** della regione visibile. Un terzo significato sullo stesso token è il difetto che [`D-291`](decisions/RT_PDR_00_Decision_Log.md) ha respinto per `ResolutionLayer`. ⚠️ **`Hidden` e `Partial` sono liberi**, e il vocabolario di contatto esistente — `Nascosto`/`ContattoIncerto`/`Rilevato` ([`brief-conoscenza-parziale.md`](gameplay/brief-conoscenza-parziale.md) §3) — vive su un **altro asse**: è percezione, cioè l'esito a valle. ⚠️ **Dipende da [`#1830`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1830)**, l'implementazione di [`D-269`](decisions/RT_PDR_00_Decision_Log.md)/[`D-270`](decisions/RT_PDR_00_Decision_Log.md): senza LOS che legga la geometria intra-cella l'esposizione non ha da dove leggere. **Innesco**: il primo consumatore di quella LOS |
...
> 🔁 **Aggiornato il 2026-09-01 — sono quattro, e la quarta non è una lacuna.** `FAC-16` è aperta dal kit
> d'autore *Cover · Melee · AoE · Facing v0.1*
> ([referto](roadmap/plans/cover-melee-aoe-facing-consolidamento-spec-panel-2026-09-01.md) §5.1) e sta in
> questa sezione **per collocazione, non per natura**: le altre tre sono buchi del canone, questa è un
> **default già deciso e implementato** che il kit propone di invertire senza citarlo. ⚠️ È anche la sola
> delle quattro che, se decisa nel senso del kit, **cambierebbe codice con test verdi** — le altre tre non
> fermano lavoro costruibile, questa lo riscriverebbe.
...
| `FAC-16` | Il **forced movement** ruota il bersaglio verso la sorgente, o lo lascia dov'era rivolto? | 🔴 **Non è una lacuna: è un ribaltamento proposto senza saperlo.** Il kit d'autore v0.1 (§10, §26) dichiara *«Push/Pull/Knockback → Facing unchanged»* come **default**, con `FaceSource` come opt-in d'abilità. Il codice fa l'opposto ed è deciso: `URTFacingLibrary::FacingAfterDisplacement` ruota verso `SourceCell` quando `Cause == ERTDisplacementCause::Forced`, e lascia il facing invariato per ogni altra causa — owner [`ADR-0008`](decisions/adr-0008-rotazione-e-policy-di-facing.md) §3, pinnato da `RTFacingTests`. ⚖️ **Metà del kit è già vera**: `Teleport → Facing unchanged` (§9) **è** il ramo `Cause != Forced`. Diverge solo la causa `Forced`. ⚠️ **Il kit non nomina né `ADR-0008` né `ERTDisplacementCause`**, quindi applicarlo alla lettera invertirebbe un default con test verdi per inerzia — ed è esattamente ciò che [`D-305`](decisions/RT_PDR_00_Decision_Log.md) §5(b) ha scartato per nome sullo stesso perimetro. ⛔ **Non riapre `FAC-14`** (la rotazione forzata come effetto a catalogo): quella chiede se esiste un effetto che ruota, questa chiede quale sia il **default** quando nessun effetto lo dichiara. **Istruttoria**: [`roadmap/plans/cover-melee-aoe-facing-consolidamento-spec-panel-2026-09-01.md`](roadmap/plans/cover-melee-aoe-facing-consolidamento-spec-panel-2026-09-01.md) §5.1. **Owner**: [`#339`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/339) 🔁 **Aggiornata il 2026-09-01 — non è una lacuna, e il default è già scelto**; istruttoria nel commento di [`#339`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/339#issuecomment-5490795607). [`ADR-0008`](decisions/adr-0008-rotazione-e-policy-di-facing.md) §3 fissa `ERTDisplacementFacingPolicy` con **due default per causa** — `FaceSource` per `Forced`, `Keep` per `Environmental` — quindi la struttura dichiarativa che il kit presuppone **esiste già come decisione**, e ciò che il kit chiede è **invertire il default** e **collassare le due cause in una**. ⛔ **Nessuna decisione successiva**: sweep del Decision Log da `D-200` su `displacement|spostat|knockback|forzat` non trova nulla; `ADR-0008` resta l'owner. 🔴 **E non c'è legacy**: [#176](https://github.com/DegrassiAaron/refactor-tactics-main/issues/176) (CP 16.1, **chiusa**) porta il contratto a **tre** casi come voce di DoD spuntata, e tre test lo pinnano (`Facing.ForcedMovementFacesSource`, `…EnvironmentalDisplacementKeepsFacing`, `…VoluntaryMoveWinsOverForced`). ⏳ **Implementazione parziale**: `ERTDisplacementCause` esiste, `ERTDisplacementFacingPolicy` dà **zero** occorrenze — `ADR-0008` è *«Accettato, non implementato»* ([#1605](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1605)) — ma implementarla **non cambierebbe nessun esito**, per costruzione. ✅ **La domanda utile è un'altra**: *esiste un caso di gioco in cui girarsi verso la sorgente produce un esito sbagliato?* `ADR-0008` §3 registra che *«nessuno è stato prodotto»*, e il kit non ne produce uno. Se esistesse, si dichiara `Keep` **su quell'effetto** senza toccare il default né i tre test. |
