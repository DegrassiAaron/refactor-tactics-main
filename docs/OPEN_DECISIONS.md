...
|---|---|---|
| `BEAT-1` | **Quale meccanismo tiene il ritmo osservato indipendente dal tempo di risposta altrui, e con quale tolleranza numerica — sapendo che la slow-motion di finestra, già prescritta, è il canale che quel meccanismo deve neutralizzare?** | 🔴 **§7-bis elenca tre alternative e non ne sceglie nessuna**: *«Buffering, pacing, *fixed resolution beat* o meccanismo equivalente»*. Un requisito con tre risposte accettabili è un requisito che ogni implementazione futura può dichiarare soddisfatto scegliendo la più comoda — e la DoD di #759 chiede infatti *«il meccanismo è dichiarato e implementato […] come scelto, non come elenco di alternative»*. `grep` in tutto il repository trova *«fixed resolution beat»* in **due** punti, e sono le due copie dello stesso elenco: §7-bis e la riga di `D-021`. Zero occorrenze in `Source/`. 🔴 **E la tolleranza non è un numero da nessuna parte**: la DoD la chiede esplicitamente (*«un numero scritto, non "impercettibile"»*), e le sole tolleranze scritte in `docs/decisions/` e `docs/technical/` sono colorimetriche e geometriche. Senza quel numero la voce 4 della DoD — *«le due timeline devono essere indistinguibili entro la tolleranza dichiarata»* — non è scrivibile come asserzione: manca il termine di paragone. 🔑 **Ciò che rende questa voce URGENTE invece che teorica, e che #759 non sa**: il meccanismo che produce la pausa vietata **esiste già, ed è prescritto da altre decisioni accettate**. `ARTTurnManager::UpdateReactionSlowMotion` (`Turn/RTTurnManager.cpp`, chiamata a ogni `Tick`) scrive `ViewerPlaybackSpeed = bWindowOpen ? ReactionWindowPlaybackSpeed : 1.f` con `ReactionWindowPlaybackSpeed = 0.35f` — cioè la resolution rallenta **esattamente quando** una finestra privata è aperta e **per esattamente il tempo** che il difensore impiega a rispondere. È la definizione letterale di ciò che §7-bis vieta: *«nessuna pausa variabile correlata alla scelta privata […] il ritmo osservato non deve dipendere dal tempo di risposta di un altro giocatore»*. ⛔ **E non è un difetto sfuggito: è dovuto.** [`D-355`](decisions/RT_PDR_00_Decision_Log.md) stabilisce che la finestra umana si apra **durante** il playback, e [#166] dichiara la slow-motion *«sola presentazione»*; [`D-350`](decisions/RT_PDR_00_Decision_Log.md) toglie al giocatore il controllo manuale della velocità proprio dove una finestra può aprirsi. Le tre decisioni sono coerenti fra loro e incompatibili con §7-bis **nel momento in cui esiste un secondo osservatore** — e nessun documento le mette nella stessa frase. ✅ **Oggi non è un leak, e va detto con la misura**: `ViewerPlaybackSpeed` non è replicato perché **niente** lo è (`grep -rn "DOREPLIFETIME" Source/ \| grep -v /Tests/` → nessuna riga), quindi non esiste un avversario che osservi quel ritmo. È precisamente il caso che §7-bis aveva previsto — *«se in v0.1 il requisito non è pienamente verificabile, si documenta l'architettura e si apre la issue»* — con la differenza che l'architettura da documentare **ha ora un nome e un valore**. **Uscite**: *(a)* **`ViewerPlaybackSpeed` diventa per-osservatore**, e l'avversario non riceve il rallentamento — ⚠️ ma allora i due schermi mostrano la stessa resolution a ritmi diversi, e va deciso cosa significhi per un replay che li deve riprodurre entrambi; *(b)* **fixed resolution beat**: il ritmo è una costante di formato e la finestra vive dentro il beat, mai oltre — ⛔ vincola la durata massima della finestra al beat, cioè tocca `CP 14.5`/`CP 14.6` e il campione `p50`/`p90` di `D-351`; *(c)* **buffering**: l'avversario osserva con un ritardo fisso ≥ timeout della finestra — ⛔ il costo è quel ritardo su **ogni** resolution, anche quando nessuna finestra si apre, ed è il prezzo che la tolleranza numerica deve pesare. ⚠️ **Da non confondere con la porzione che resta di M10**: latenza reale, riconnessione e «client lento» come entità di rete non hanno oggi un soggetto e restano `post-v0.1`. Questa voce riguarda la **forma** del meccanismo, che è decidibile ora. ⛔ **E non autorizza a spostare #759 in v0.1**: [#2462](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2462) lo vieta in quattro punti del proprio corpo (*«owner separato, non spostare»*). **Innesco**: la prima issue che implementi una superficie di rete, oppure la prima modifica a `UpdateReactionSlowMotion` |
