...
> ⚠️ *La prima stesura di questo paragrafo diceva «era l'eccezione … ed è chiusa», ri-ancorando il blocco
> alla **decisione** invece che all'**atterraggio**. Chi si ferma alla conclusione in grassetto — la frase
> che la sezione è costruita per consegnare — conclude che si può produrre, e produce asset ancora fuori
> misura. Trovato in code review.*
...
| **Come si chiama la categoria di log del Decision Time Bank: `Decision` o `ReactionDecision`?** | ⏳ **aperta il 2026-08-17**, e apre come `CONFLICT` perché **nessuna delle due fonti prevale**: [`technical/spec-turnlog.md`](technical/spec-turnlog.md) §4.2 — deciso il 2026-08-09 con [`#361`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/361) — prescrive che `ERTLogCategory` guadagni **`Decision`** in coda, e `RTTestScenario.h` lo ripete nel commento di `LogEventAmount`; il codice atterrato con **CP 14.5** ha in coda **`ReactionDecision`** (`RTTurnLog.h`), già serializzata in TurnLog **v8** e letta da cinque call site. Una spec normativa contro un formato in `main`: non si sceglie per plausibilità. ⚠️ **Aggiungerne una seconda in coda darebbe due categorie sulla stessa finestra**, che è il difetto che il commento di `ERTReactionDecisionOutcome` argomenta di aver evitato tenendo un enum solo. Le due uscite sono *(a)* il bank scrive dentro `ReactionDecision` con esiti propri e §4.2 va **emendata**, oppure *(b)* `Decision` nasce distinta perché il bank serve anche finestre che **non** sono reazioni — è la ragione per cui il FeatureId è `RT-FEAT-CORE-*` e non `-REACTION-*` — e allora va scritto perché due categorie non siano due verità. **Va chiusa prima della prima riga di runtime di CP 14.8**, non dopo: dopo significa scegliere il nome che giustifica il codice già scritto. Riga **75** di [`DOC_CONFLICT_MATRIX.md`](DOC_CONFLICT_MATRIX.md) · voce di DoD di [`#319`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/319) · owner del nome: `spec-turnlog.md` |
