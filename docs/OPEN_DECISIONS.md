...
> `OPEN` · **Stato**: vivo · **Ultimo aggiornamento**: 2026-09-09 (`BLIND-4`, la misura che la riga dichiarava impossibile)
...
| `INT-9` | Il comando **esplicito** `SetDoorState(..., Destroyed)` resta di **gruppo** o diventa **per voce**? | Aperta il 2026-09-10 da [`D-375`](decisions/RT_PDR_00_Decision_Log.md), che rende la distruzione da **danno** per segmento e non tocca il comando. Oggi `URTHexDoorLibrary::CommutateEdge` non ha eccezioni per `Destroyed`, quindi esisterebbero **due vie** allo stesso stato terminale con esiti diversi — e chi implementa sceglierebbe per primo. Non si deduce: entrambe sono coerenti con la spec, che dice solo *«se un bordo del gruppo viene distrutto, gli altri restano»*. |
