...
| `FAC-15` | La relazione a **sei direzioni relative** dev'essere **equipartita** o **speculare**? | Non può essere entrambe, ed è **misurato**: equivarianza per rotazione, simmetria speculare e partizione in sei classi sono **incompatibili** su un anello esagonale di raggio pari — a `r = 2` la riflessione rispetto all'asse di facing fissa **solo** `(2, 0)` e `(-2, 0)`, quindi una classe `Front` speculare da due celle costringe «dritto davanti» e «dritto dietro» nella stessa classe. [D-126](decisions/RT_PDR_00_Decision_Log.md) fissa i sei **nomi**, non l'invariante **geometrico**, e [#726](https://github.com/DegrassiAaron/refactor-tactics-main/issues/726) sceglie in silenzio: la sua regola a settore semiaperto — `cella − difensore = a·D(i) + b·D(i+1)` con `a > 0, b >= 0` — è equipartita (**36** celle per lato a raggio `1..8`, esattamente `r` per anello: verificato per `r = 1..8`) ma **non speculare**, con **168 celle su 216** che non ricevono la direzione speculare della propria immagine speculare, contro **zero** di `IsInFrontalArc`. `Front` è cioè il raggio dritto davanti **più l'intero spicchio a destra**: uno `Shield = {Front}` non proteggerebbe a sinistra, e un `Backstab = {Rear}` colpirebbe da dietro-sinistra ma non da dietro-destra. ⚠️ La via d'uscita ovvia — *«si dichiarino solo insiemi simmetrici»* — **non esiste**: **0** sottoinsiemi propri su **62** producono una regione speculare. Le due strade sono **A)** tenere l'equipartizione e **dichiarare** lo skew, accettando che nessun insieme di lati sia bilanciato; **B)** rinunciare alle sei classi — dodici classi (sei raggi più sei spicchi aperti), oppure sei più un esito «sul confine». 🔴 **Blocca #726**, il cui DoD contiene oggi un test che la regola scelta **fallisce**: chiede che `(-1, 2)` e `(1, -2)` restino `FrontLeft`/`FrontRight`, e a settore `(-1, 2)` è **`RearLeft`** — né è una divergenza dal cono. Quei due valori, e il «**50**» di celle divergenti che `D-126` cita, sono le misure della regola a **linea** che lo spec panel del 2026-08-13 ha scartato lasciando in piedi i numeri: a settore le divergenze su raggio `1..10` sono **45**, e a distanza `2` ce n'è **una sola**, `(1, -2)`. Il numero sopravvive in `D-126`, [ADR-0005](decisions/adr-0005-orientamento.md), [ADR-0008](decisions/adr-0008-rotazione-e-policy-di-facing.md), `roadmap/feature-registry.yaml` (più la vista `.json`) e [`roadmap/plans/facing-visualdocs-triage-2026-08-13.md`](roadmap/plans/facing-visualdocs-triage-2026-08-13.md) — **sweep unico, in integrazione**, e `docs/archive/` non si tocca. ⚠️ La **conclusione** che quei documenti traggono dal numero — il cono è strettamente contenuto nell'insieme dei tre lati, quindi sostituirlo sarebbe un buff difensivo — **regge**: cade solo la cardinalità |
