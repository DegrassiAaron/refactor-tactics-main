...
[D-231](decisions/RT_PDR_00_Decision_Log.md)). Restano queste sette righe.
...
| `ICON-TAX-3` | **`Weapon`** (6 chiavi) — corrispondenza **esatta** fra manifest e codice (`Weapon.Environmental · Impact · Overcharge · Precision · Split · Suppressive`) | È l'unico dei sette dove le due liste coincidono senza residui, il che rende la mappatura banale **e** la scelta (a)/(b) arbitraria: nessuna descrizione delle dodici copre «profilo d'arma», e nessuna macchina deriva oggi quelle chiavi |
| `ICON-TAX-4` | **`Stat`** (11 chiavi) — `Charge · Cooldown · Cost · Detection · Duration · Health · Noise · Range · Resource · Shield · Vision` | Misurato: **zero id**. Sono letture **numeriche** — `Cooldown`/`Range` su `FRTActionDef`, `Health`/`Shield` su `ARTUnit` — e `Vision`/`Noise` non esistono nemmeno come campo lì. ⚠️ Ma [D-231](decisions/RT_PDR_00_Decision_Log.md) colloca costo/cooldown/cariche fra i **satelliti della card**, e `10-catalogo-sette-categorie.md` arbitra `Cooldown` sotto **`Warning`**: tre sedi, tre collocazioni, nessuna normativa sull'enum |
...
🔴 **E una divergenza che chi decide `ICON-TAX-2` deve chiudere nello stesso passaggio**: le due liste
`Gadget` **non coincidono in nessuno dei due versi** — il codice ha `Gadget.Mine`, che il manifest non
nomina; il manifest ha `Gadget.Anchor`, che il codice non ha. ⚠️ E `Gadget.Anchor` è anche una **trappola di
nome**: sarebbe una cosa diversa da `Reaction.Anchor` con lo stesso nome.
