...
> voci **aperte** è **812** caratteri al 2026-09-20 — cambia insieme al documento, quindi si rimisura invece di
> citarla a memoria, e il comando misura **la popolazione che la regola governa**, non tutte le righe:
>
> ```bash
> awk '/^\| `[A-Z][A-Z0-9]*(-[A-Z0-9]+)*-[0-9]+`/{print length($0)}' docs/OPEN_DECISIONS.md | sort -n | awk '{a[NR]=$1} END{print "n="NR, "mediana="((NR%2)?a[(NR+1)/2]:int((a[NR/2]+a[NR/2+1])/2)), "max="a[NR]}'
> ```
>
> ⚠️ **Il filtro non è cosmetico**: `/^\| /` da solo prendeva 301 righe — 110 intestazioni e 129 voci
> barrate allo stesso 2026-09-20 — e la mediana che ne usciva era di un altro insieme.
>
> 🔑 **Sopra i `4000` caratteri una voce APERTA scorpora il corpo** in `decisions/open/<id>.md` e qui lascia
> un puntatore breve: la domanda, **lo stato di oggi**, l'innesco, ciò che chi la prende rischia di non
...
> questo scorporo **nessuna voce aperta arriva alla soglia**: la più lunga misura **3341** caratteri, e la
> soglia è ~4,9× la mediana. Quindi oggi la regola **non morde**: è una guardia per la prossima voce che
> cresce, e le aperte da 3000+ restano in cella. ⛔ **Abbassarla è una scelta di scope, e questa issue non la
...
> dicesse «tutte» nascerebbe violata da cinque voci chiuse — ed è esattamente il difetto che questo
...
> numero sia simile.
>
> 🔚 **Quando la voce si chiude** il puntatore resta barrato qui con l'esito, e il file in `decisions/open/`
> passa da `OPEN` a `HISTORICAL` col rimando alla `D-xxx` che l'ha chiusa. Un file che resta `OPEN` dopo la
> chiusura è un secondo posto dove la domanda sembra ancora aperta.
