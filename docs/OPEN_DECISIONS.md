...
🔑 **Queste sette non sono le sole domande della sorgente: sono quelle che restano senza owner.** La sezione
...
|---|---|---|
| `SKB-1` | **La v0.1 riapre lo `Stun`, oppure le regole della sorgente si riscrivono sugli stati di controllo che esistono?** | 🔴 **Sei sezioni della sorgente poggiano su uno stato che il gioco non ha.** Misurato: `grep -rniE '\bstun\|stordi' Source/RefactorTactics/` → **1** occorrenza, ed è un sottotesto dentro un'altra parola. Non è una lacuna: è [`D-169`](decisions/RT_PDR_00_Decision_Log.md) — *«`Stun` e `Disarm` **escono dalla v0.1** perché non esistono»* — che per questo riscrisse la prima riga di DoD del CP 14.6. ∴ *«uno stun fa decadere Guardia»*, *«se già attivo in Planning il comando è disabilitato»*, *«uno stun su chi sorveglia non annulla il colpo simultaneo»* non sono false: sono **senza soggetto**. **Uscite**: *(a)* **riscriverle su ciò che esiste** — `Unbalanced`, `Prone`, `Root`, `Slow` — costo zero di scope, ⛔ ma i quattro stati non fanno le stesse cose e la mappatura non è 1:1: `Root` blocca il movimento e non l'azione, `Unbalanced` nega la corsa, nessuno dei due «spegne» una difesa; *(b)* **riaprire `Stun` nella v0.1** — dà alla sorgente il soggetto che presuppone, ⛔ ma riapre `D-169`, che l'aveva chiusa misurando, e aggiunge uno stato di controllo a una release in chiusura. ⚠️ **Non si deduce quale**, perché la sorgente non mostra di sapere che lo stato non esiste. **Innesco**: la prima implementazione fra `D-408`, `D-409`, `D-410`, `D-411` — tutte e quattro la incontrano |
...
| `SKB-4` | **Su che cosa agiscono le varianti arma, se non più sull'attacco base?** | Coda di [`D-415`](decisions/RT_PDR_00_Decision_Log.md) punto (2). Le varianti (`D-086`…`D-100`) hanno **un solo** consumatore di gameplay: `URTCatalogLibrary::EquipWeaponVariant(Abilities[0], Piece)` (`Source/RefactorTactics/Unit/RTUnit.cpp:1661`), e `Abilities[0]` **è** l'attacco base per la convenzione di [`ADR-0007`](decisions/adr-0007-attacco-base-per-eroe.md) §6. ∴ recidere il legame non sposta una dipendenza: **svuota** le varianti, che restano nel catalogo senza effetto misurabile. **Uscite**: *(a)* dare loro un altro punto d'applicazione — le skill del kit, i costi, il rumore; *(b)* ritirarle. ⛔ **Nessuna delle due si deduce**, e la sorgente non nomina mai l'equipaggiamento. **Innesco**: l'implementazione di `D-415`, che **non deve partire prima** |
| `SKB-5` | **Come si riprezza `Action.Mortar` se il tiro alla cieca diventa gratuito?** | Coda di [`D-415`](decisions/RT_PDR_00_Decision_Log.md) punto (4). [`D-380`](decisions/RT_PDR_00_Decision_Log.md) ha **pagato** la licenza di tiro indiretto: `Action.Mortar` a **12** danni invece di 18 e ricarica **3** invece di 2. Concederla a quattro attacchi base gratuiti e senza cooldown significa che quel prezzo non comprava una capacità, comprava un'**esclusiva**. ⚠️ **Non è una deduzione, è aritmetica**: chi implementa `D-415` senza toccare `D-380` lascia in catalogo un'abilità che paga per ciò che tutti hanno gratis. **Uscite**: *(a)* restituire i 6 danni e il turno di ricarica; *(b)* trovarle una seconda capacità che giustifichi il prezzo; *(c)* ritirarla. **Innesco**: l'implementazione di `D-415` |
