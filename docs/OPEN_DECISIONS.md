...
fra il 2026-08-07 e il 2026-08-10. Ciò che resta sono queste sette domande, di cui **sei aperte**: `AE-1` è stata chiusa il 2026-08-12 da
[D-114](decisions/RT_PDR_00_Decision_Log.md) e resta qui barrata, perché era la voce da cui dipendevano le altre.
...
| `AE-2` | Il **profilo di movimento** può cambiare la **legalità** e l'**efficacia** di un'azione (`NORMAL/IMPAIRED/ENHANCED/BLOCKED`)? | È l'unico contributo genuinamente nuovo del kit, e risolve un difetto **già registrato**: senza costo di slot, il prezzo dello `Sprint` regge tutto su `Status.Exposed` ([D-028](decisions/RT_PDR_00_Decision_Log.md) lo dichiara come allarme). Costa un asse di bilanciamento per abilità × quattro profili e un problema di leggibilità in Planning. Feature `RT-FEAT-ACTION-MOVEMENT-COMPAT`. ➕ **Dal 2026-08-12 la domanda ha un modello concreto davanti**, non più un'idea: [`gameplay/spec-compatibilita-azioni-movimento.md`](gameplay/spec-compatibilita-azioni-movimento.md) (`PROPOSED`, issue [#606](https://github.com/DegrassiAaron/refactor-tactics-main/issues/606)) propone una **soglia** — `MinStability` sull'azione contro `Stability` sul profilo — invece della matrice azione × profilo del kit: **31 numeri + 4 contro 124 celle**, misurati sul catalogo. 🔴 E smonta la giustificazione con cui il kit la proponeva: *«non puoi mirare mentre corri»* **non descrive niente** su questo modello di turno — con il `Move` normale spari **prima** di partire (Blast precede Move), con lo `Sprint` spari **dopo** essere arrivato (D-068). La regola, se si adotta, si giustifica come **impegno del turno**, non come fisica del gesto — e se quella giustificazione non convince, la risposta è **no**. Criteri di decisione in §7 della spec |
