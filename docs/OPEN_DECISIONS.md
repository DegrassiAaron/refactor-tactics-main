...
fra il 2026-08-07 e il 2026-08-10. Ciò che resta sono queste **sette** domande, e nessuna si chiude leggendo i
documenti: due modelli diversi funzionano entrambi, e la scelta è di gioco.
...
|---|---|---|
| `AE-1` | L'economia del turno resta a **slot** (1 movimento · 1 principale · 1 reazione) o diventa una **capacità numerica** con un costo per azione? | **Generalizza `ECO-1`**, che chiedeva la stessa cosa per due azioni sole. Non è un'aggiunta ma una **sostituzione**: `ERTActionSlot` gira, [D-028](decisions/RT_PDR_00_Decision_Log.md) l'ha decisa e il catalogo registra già l'equivalenza «Slot ≡ Action Points, cap 2» (2026-08-07). Il modello a slot rende `Guard + attacco` illegale **per struttura**; un budget lo renderebbe una questione di totale, e con esso cambierebbero validatore, HUD, bot e ogni riga del catalogo. La domanda vera non è quale sia più espressivo — è se il gioco voglia che due azioni leggere valgano una pesante |
| `AE-2` | Il **profilo di movimento** può cambiare la **legalità** e l'**efficacia** di un'azione (`NORMAL/IMPAIRED/ENHANCED/BLOCKED`)? | È l'unico contributo genuinamente nuovo del kit, e risolve un difetto **già registrato**: senza costo di slot, il prezzo dello `Sprint` regge tutto su `Status.Exposed` ([D-028](decisions/RT_PDR_00_Decision_Log.md) lo dichiara come allarme). Costa un asse di bilanciamento per abilità × quattro profili e un problema di leggibilità in Planning. Feature `RT-FEAT-ACTION-MOVEMENT-COMPAT`, oggi `IDEA` |
...
| `AE-7` | Le **eccezioni per eroe** alla compatibilità e al vincolo dell'`Overwatch` sono contenuto di kit o regole? | Il pattern è già fissato da [D-014](decisions/RT_PDR_00_Decision_Log.md)/[D-028](decisions/RT_PDR_00_Decision_Log.md) — un'eccezione si dichiara **nel kit dell'eroe**, mai nella regola generale — ma nessun kit ne dichiara una, quindi il meccanismo che le renderebbe esprimibili non esiste. Da decidere **dopo** `AE-1` e `AE-2`: prima non si sa a cosa si fa eccezione |
