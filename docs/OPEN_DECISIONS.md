...
| ID | Domanda | Perché non si deduce |
|---|---|---|
| `MOV-3` | I profili di movimento restano a **cadenza unica** — un arco di grafo per micro-step per tutti — o alcuni diventano eleggibili a un **FAST micro-step** alternato? | Il mandato §7 lo marca esplicitamente `NON LOCKED — PLAYTEST OPTION`, e non si deduce perché **la domanda è di sensazione, non di codice**: oggi `Sprint` si distingue per budget (`8 MP` contro `5`), rumore (`5`), `Status.Exposed` e rinuncia alla reazione, e nessuna misura dice se basti. ⚠️ **Ha una dipendenza che il mandato non nomina**: lo `Sprint` ha una migrazione aperta di macro-fase — oggi risolve in `20` (`FastMovement`, con Dash) mentre il catalogo lo dichiara profilo `Move` post-Blast ([`RT_ActionCatalog_v0.1.md`](balance/RT_ActionCatalog_v0.1.md) §2.1, owner [`#199`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/199)). Decidere la cadenza **prima** di quella migrazione la deciderebbe per inerzia. ⛔ **Anche in un FAST micro-step resta `1` arco per unità**: questo il mandato lo blocca, e non è in discussione. **Innesco**: un playtest che misuri lo Sprint come «abbastanza lungo ma non abbastanza rapido» |
