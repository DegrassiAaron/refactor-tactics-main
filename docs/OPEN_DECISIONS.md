...
> ➕ **`STA-5` aggiunta il 2026-09-04**, dalla seduta di design che ha prodotto
> [`brief-stati-unbalanced-prone.md`](gameplay/brief-stati-unbalanced-prone.md)
> ([#2229](https://github.com/DegrassiAaron/refactor-tactics-main/pull/2229)). Il numero in prosa qui sopra
> è stato aggiornato **nello stesso commento** che aggiunge la voce: questa sezione porta già la cicatrice di
> una volta in cui non lo si è fatto, dodici righe più in basso, ed è l'unico posto del documento in cui
> quell'errore è documentato invece che ripetuto.
...
| `STA-5` | `Unbalanced` e `Prone` si **anticipano** fuori dall'epic G.2, o restano dietro al motore del ghiaccio? | Aggiunta il **2026-09-04**, e sta qui e non fra le `MOV-*` perché è la stessa domanda di `STA-3` su due stati diversi — *entrano ora, o restano nel framework?* — mentre `MOV-1`…`MOV-4` sono decisioni del **movement resolver**. ⚠️ **Tre sedi vive la collocano fuori, non due**: [`brief-ghiaccio.md`](gameplay/brief-ghiaccio.md) §5 (epic G.2), [`roadmap-v0.1.md`](roadmap/roadmap-v0.1.md) §*«Fuori dalla v0.1»*, che elenca `Unbalanced/Prone` **dentro** «il motore del ghiaccio», e il documento sorgente. Contro di esse [`brief-stati-unbalanced-prone.md`](gameplay/brief-stati-unbalanced-prone.md) propone di anticiparli **senza** motore, separandoli per **causa** dello spostamento invece che per soglia di Momentum. 🔑 **La proposta è verificabile e regge** — la distinzione ha già un tipo, `ERTDisplacementCause`, e non c'è nessuna scala da tarare. ⛔ **Ma non fa cadere la dipendenza dal fuzzing**: [roadmap-v0.1 §5 CP 12.6](roadmap/roadmap-v0.1.md) scarta il fuzzing deterministico *«solo perché il motore del ghiaccio (slide a catena) resta fuori dalla v0.1 — se rientrasse, andrebbe riaperto»*, e i due stati non sono lo slide a catena. `G.3 non parte senza il fuzzing` (`brief-ghiaccio.md` §5) **resta vero in tutte le uscite**. ⚠️ E il tipo non è ancora un canale: `ERTDisplacementCause::Environmental` compare **solo nei test**, l'unico produttore vivo scrive `Forced` costante. 🔑 **La conseguenza di scope che nessuna sede aveva tratto** — la *misura* è del brief, che la dichiara in testa e in §8.1: `Unbalanced` è uno status a durata come gli altri, mentre `Prone` finisce *quando paghi 1 MP*, e `ARTUnit::ApplyStatus` conosce solo `N` turni o il legame alla cella. I due stati **non costano lo stesso**, quindi la domanda di scope si può risolvere **a metà**. **Uscite**: *(a)* anticiparli entrambi — prima va scelta una delle tre vie di rappresentazione che il brief elenca, di cui la più economica (durata + `StatusTurns.Remove`) non chiede contratto nuovo · *(b)* non anticipare nulla — il brief resta design-ready e le tre sedi non si toccano · *(c)* anticipare il solo `Unbalanced`, che rispetto ad `ApplyStatus` non chiede nulla di nuovo. ⚠️ **`Unbalanced` non è però gratis**: il brief lascia aperti l'amplificazione del `Pull` (che crea un asse, [D-038](decisions/RT_PDR_00_Decision_Log.md): *«la trazione non è resistita»*), il prezzo di «`Guard`/`Brace` inerti», il punto in cui si nega lo `Sprint` e il livello del tag. ⛔ **In nessuna uscita si implementa prima di un valore proprio in `ERTMoveOutcome` per lo scivolamento**. **Innesco**: la pianificazione di G.2, o una decisione d'autore che voglia i due stati prima. **Dipendenza**: `STA-4` — la tassonomia delle capability è l'elenco di *cosa un effetto può togliere*, ed è esattamente ciò che `Prone` toglie (reazione, Overwatch, predictive, interposizione) |
