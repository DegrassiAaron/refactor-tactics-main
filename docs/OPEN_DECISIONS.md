...
ogni eroe portava **una** reazione e la domanda non esisteva; oggi Riktor e Gadget ne portano **due** (kit +
modulo di loadout), e il bot deve sceglierne una.

| ID | Domanda | Esito, e l'istruttoria che ci è arrivata sotto |
|---|---|---|
| `BOT-REACT-1` | **Con due reazioni disponibili, quale dovrebbe armare il bot?** | ⏳ **Aperta.** [D-220] ha stabilito una regola **deterministica e dichiarata** — vince il modulo di **loadout** — ma la sua ragione è **sistemica, non tattica**: *un loadout che il bot non usa non esiste per metà delle unità in campo*. ⚠️ **Non dice che il modulo sia la scelta migliore.** Per Riktor significa preferire `Reaction.Cleanse` (cooldown 0, annulla `Root`/`Slow` in arrivo) a `Interposition` (cooldown 3, cambia il bersaglio di un colpo su un alleato): quale valga di più dipende dalla situazione, ed è precisamente ciò che un'euristica di bot dovrebbe valutare. È lavoro di **E15**, e finché non c'è la regola resta questa. 🔴 **Il difetto che l'ha resa urgente**: prima di [D-220] il bot armava la **prima** reazione trovata e faceva `break`, cioè sceglieva per **ordine di indice** — e il modulo, che `EquipLoadout` accoda, non veniva armato **mai** |
