...
| ~~**Come si chiama la categoria di log del Decision Time Bank: `Decision` o `ReactionDecision`?**~~ | ✅ **Chiusa il 2026-08-17 da [D-166](decisions/RT_PDR_00_Decision_Log.md)**, dopo meno di un giorno di apertura. Esito: **`Decision` nasce distinta**, con `ERTDecisionOutcome` (`BankConsumed`, `BankAfter`, `BankExhausted`) e `Amount` in **millisecondi**; `ReactionDecision` resta con `Amount` in **danni**. Prevale [`technical/spec-turnlog.md`](technical/spec-turnlog.md) §4.2, che lo prescriveva dal 2026-08-09 (`#361`). **L'istruttoria, perche' e' la parte che resta utile**: la domanda sembrava sui nomi ed era su `Amount`. Quel campo e' il payload numerico **della categoria** — danni per `Combat` e `ReactionDecision`, celle per `Move`, direzione per `Facing` — e il commento di `ERTReactionDecisionOutcome` argomenta gia' contro *«due assi per un campo solo»*. Metterci i millisecondi del bank avrebbe reintrodotto quel difetto dall'altra parte. **Due categorie non sono due verita'**: una dice *cosa e' stato scelto*, l'altra *quanto e' costato*. Il formato serializzato non cambia versione. ⚠️ Resta da **scrivere** l'enum e la voce in coda a `ERTLogCategory`: e' lavoro di CP 14.8, e la decisione non lo anticipa |
