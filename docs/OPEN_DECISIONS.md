...
> **Aggiornamento 2026-08-16 — ne restano quattro.** `FAC-15` si apre da uno spec panel su
> [#726](https://github.com/DegrassiAaron/refactor-tactics-main/issues/726), e nasce da ciò che la chiusura di
...
| `FAC-15` | La relazione a **sei direzioni relative** dev'essere **equipartita** o **speculare**? | Non può essere entrambe, ed è **misurato**: equivarianza per rotazione, simmetria speculare e partizione in sei classi sono **incompatibili** su un anello esagonale di raggio pari — a `r = 2` la riflessione rispetto all'asse di facing fissa **solo** `(2, 0)` e `(-2, 0)`, quindi una classe `Front` speculare da due celle costringe «dritto davanti» e «dritto dietro» nella stessa classe. [D-126](decisions/RT_PDR_00_Decision_Log.md) fissa i sei **nomi**, non l'invariante **geometrico**, e [#726](https://github.com/DegrassiAaron/refactor-tactics-main/issues/726) sceglie in silenzio: la sua regola a settore semiaperto — `cella − difensore = a·D(i) + b·D(i+1)` con `a > 0, b >= 0` — è equipartita (**36** celle per lato a raggio `1..8`, esattamente `r` per anello: verificato per `r = 1..8`) ma **non speculare**: **168 celle su 216** non ricevono la direzione speculare della propria immagine speculare, mentre `IsInFrontalArc` ne ha **zero**. ⚠️ Le due cifre **non stanno sulla stessa scala** e non vanno citate come «168 contro 0»: il cono è un predicato **booleano**, la cui unica classe è speculare per costruzione, mentre una partizione equivariante in sei classi **non può** valere zero su un anello di raggio pari — è ciò che questa riga dimostra due frasi sopra. La lettura onesta è *speculare* contro *necessariamente non speculare*. `Front` risulta cioè il raggio dritto davanti **più uno solo dei due spicchi adiacenti**. 🔴 **Quale dei due, questa voce non lo afferma, ed è parte della domanda**: la mappatura fra i sei nomi e gli indici di `RT_HEX_DX/DY` non è fissata da nessuna parte — `ERTRelativeDirection` **non esiste in `Source/`** — e le due fonti disponibili si contraddicono. Gli indici girano `E, NE, NW, W, SW, SE`, e `AxialToWorld` manda `NE` a `-Y`; con la convenzione UE (`+X` avanti, `+Y` a destra) l'indice `f+1` cade a **sinistra** di chi guarda, mentre l'elenco di `D-126` chiama `FrontRight` proprio la posizione `f+1`. Il difetto resta in ogni caso — uno `Shield = {Front}` protegge un fianco e non l'altro, un `Backstab = {Rear}` colpisce da un solo lato dietro — ma **il verso va deciso, non dedotto dai nomi**. ⚠️ La via d'uscita ovvia — *«si dichiarino solo insiemi simmetrici»* — **non esiste**: **0** sottoinsiemi propri su **62** producono una regione speculare. Le due strade sono **A)** tenere l'equipartizione e **dichiarare** lo skew, accettando che nessun insieme di lati sia bilanciato; **B)** rinunciare alle sei classi — dodici classi (sei raggi più sei spicchi aperti), oppure sei più un esito «sul confine». 🔴 **Blocca #726**, il cui DoD contiene oggi un test che la regola scelta **fallisce**: chiede che `(-1, 2)` e `(1, -2)` restino `FrontLeft`/`FrontRight`, e a settore `(-1, 2)` è **`RearLeft`** — né è una divergenza dal cono. Quei due valori, e il «**50**» di celle divergenti che `D-126` cita, sono le misure della regola a **linea** che lo spec panel del 2026-08-13 ha scartato lasciando in piedi i numeri: a settore le divergenze su raggio `1..10` sono **45**, e a distanza `2` ce n'è **una sola**, `(1, -2)`. Il numero sopravvive in **sette** sedi vive, misurate e non ricordate: `D-126` nel [Decision Log](decisions/RT_PDR_00_Decision_Log.md), [ADR-0005](decisions/adr-0005-orientamento.md), [ADR-0008](decisions/adr-0008-rotazione-e-policy-di-facing.md), [`DOC_CONFLICT_MATRIX.md`](DOC_CONFLICT_MATRIX.md) (riga 67, «Primitiva geometrica del facing»), [`roadmap-v0.1.md`](roadmap/roadmap-v0.1.md) (epic **E16**), `roadmap/feature-registry.yaml` con la sua vista `.json`, e [`roadmap/plans/facing-visualdocs-triage-2026-08-13.md`](roadmap/plans/facing-visualdocs-triage-2026-08-13.md) — **sweep unico, in integrazione**, e `docs/archive/` non si tocca perché è storico. Questo file è già allineato: la riga `FAC-11` è stata corretta il 2026-08-16. 🔴 **La prima stesura di questa voce elencava cinque sedi**, e le due mancanti — `DOC_CONFLICT_MATRIX` e `roadmap-v0.1` — sono entrambe `integration_only`, cioè esattamente i file che lo sweep tocca: chi l'avesse eseguito da quell'elenco avrebbe lasciato sei documenti a `45` e due a `50`, la stessa incoerenza che lo sweep esiste per evitare. ⚠️ La **conclusione** che quei documenti traggono dal numero — il cono è strettamente contenuto nell'insieme dei tre lati, quindi sostituirlo sarebbe un buff difensivo — **regge**: cade solo la cardinalità |
