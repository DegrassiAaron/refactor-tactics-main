...
## ✅ Chiuse il 2026-08-31 da `D-302` — copertura selezionabile: cinque risposte d'autore, e meta' di quel che dicono era gia' vero
...
> 🔎 **Cosa queste cinque voci NON decidono.** `COV-1` non è fra loro: le tre categorie di footprint sono
> decise, la **clearance** no, e la riconciliazione con [`D-071`](decisions/RT_PDR_00_Decision_Log.md) resta aperta qui sotto. `COV-7` e
> `COV-8` non hanno owner più recente e restano aperte. `MSE-4` è un'altra domanda, con owner
...
🔁 **Rimasugli di otto: tre.** `COV-2`…`COV-6` sono chiuse da [`D-302`](decisions/RT_PDR_00_Decision_Log.md) — la sezione qui
sopra ne conserva l'istruttoria — e `COV-1` è **cambiata**: le tre categorie di footprint sono decise, e
aperta resta soltanto la loro riconciliazione con [`D-071`](decisions/RT_PDR_00_Decision_Log.md).
...
|---|---|---|---|
| `COV-1` | **Come si riconcilia il modello `Small`/`Medium`/`Large` gia' approvato con [`D-071`](decisions/RT_PDR_00_Decision_Log.md)?** ⛔ **Non riapre le tre categorie**: sono decise dall'autore e ratificate da [`D-302`](decisions/RT_PDR_00_Decision_Log.md) — con clearance esplicita e data-driven, e il *Safe Placement Volume* come guida visiva d'authoring, **non** come clearance di gameplay. Aperta e' la **clearance**, non la tassonomia | `D-071` ha scelto **un solo raggio** — `StandardUnitClearance` = apotema — e ha dichiarato scartato il secondo: *«con un solo raggio non esiste il varco “ci passo ma non ci sto” … servirebbe il secondo numero, scartato qui»*. Tre categorie **sono** quel secondo numero, quindi la riconciliazione o supera `D-071` punto (1) o mappa le tre categorie su un raggio solo: è una scelta umana, e dedurla qui sarebbe deciderla per inerzia. 🔎 **Istruttoria conservata**: Nessun produttore esiste: nessuna unità dichiara un footprint, e il repository non ha mai misurato una posa. `FRTFootprintProfile::MinContiguousWedges` vale `1` — l'**identità**, che non decide niente. È un numero di bilanciamento, e `FRTOccupancyThresholds::BlockedFrom = 6` è il precedente da non ripetere: nessuno lo scelse per una regola, e per mesi ha significato «cella non calpestabile» | [#1827](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1827), quando la posa arriva nel percorso di partita |
