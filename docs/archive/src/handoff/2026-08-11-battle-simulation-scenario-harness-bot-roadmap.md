...
> [`roadmap/plans/battle-simulation-harness-bot-spec-panel-2026-08-11.md`](../../../roadmap/plans/battle-simulation-harness-bot-spec-panel-2026-08-11.md).
> **Questo file non è autorità.** Classificazione delle sue voci: **14 `ALREADY_PRESENT`**, **11 `UPDATE`**,
> **9 `NEW`**, **5 `CONFLICT`**, **3 `DEFERRED`**, **2 `SUPERSEDED`**. Sopravvivono **cinque** contributi,
> elencati al §6 del referto.
>
> ✅ **Sul piano fattuale è il più accurato della serie**, e va detto: **16 dei 17 path** che dichiara
> esistono davvero, `#326` e `#328` sono vive e le due Feature ID (`RT-FEAT-BOT-BASE`,
> `RT-FEAT-BOT-TACTICAL`) sono quelle giuste. Il problema non è cosa dice: è **quando**.
>
> ⚠️ **Le milestone `v0.2` e `v0.3` portano scritto nella propria descrizione che nessuna loro epic si apre
> prima che i 15 gate della v0.1 siano verdi.** Oggi ne è verde **1** (`G12`, packaging), uno è giallo con
> riserva (`G13`), **13 sono aperti**. Le §22 e §23 — 35 issue candidate — sono corrette nel merito e
> **inapplicabili**. Il documento non nomina mai questo vincolo.
>
> ⚠️ **La `v0.4` è già presa.** Il §24 propone «GAME RELEASE v0.4 — Unified Battle Simulation & Balance Lab»
> premettendo «se il roadmap reale non possiede già un nome/scopo v0.4 differente». Lo possiede: milestone
> **`v0.4 · Operations`**, 19 issue aperte, epic `#330`–`#333`. E le milestone `v0.5`/`v0.6`, a cui le
> §25–§26 assegnano 27 issue, **non esistono**: la numerazione reale si ferma a `v0.4`.
>
> ⚠️ **La §2.2 (fair knowledge) è collocata fra le «decisioni correnti da preservare» ed è un requisito non
> implementato.** `ARTTurnManager::PlanBots()` pianifica su `MakeCurrentSnapshot()` — stato **canonico e
> completo**, tutti i nemici vivi in `Ctx.Enemies` — e `TeamKnowledge` non esiste come tipo runtime (la
> stringa compare in un solo file, `RTPerceptionTests.cpp`). L'invariante «same TeamKnowledge → same Intent»
> non è falsificabile oggi. Il suo posto è **`E13`/#151**, dove il DoD della v0.1 lo ha già scritto.
>
> ⚠️ **L'epic v0.1 del §21 duplicherebbe struttura esistente**: `RT-FEAT-BOT-BASE` è `RELEASE_READY` su
> epic `E2`/CP 2.6, `RT-FEAT-TEST-SCENARIO-HARNESS` è `INTEGRATED` su `E15`. Delle 13 issue candidate ne
> restano **3** non coperte, e tre issue non sono un'epic.
>
> ⚠️ **Due owner citati non sono aggiornabili**: `docs/wiki/feature-status.md` (§33) è stato **rimosso** da
> `D-076` il 2026-08-10 — la wiki vive in un repository separato — e gli otto `PDR-03`…`PDR-10` del §32 sono
> **PDF archiviati** in `docs/archive/pdr-v0.1/`, con l'ownership passata agli `ADR-0001`…`ADR-0009`.
>
> ⚠️ **La CI su cui poggiano i quattro tier del §19 non esiste**: `.github/workflows/` non è presente nel
> repository, e nessuna delle 96 issue candidate la costruisce.
>
> ℹ️ **Un predecessore già consumato copre gran parte del perimetro**:
> [`2026-08-08-master-scenari-qa-e-bot.md`](2026-08-08-master-scenari-qa-e-bot.md) (846 righe, tre giorni
> prima) porta già Execution Mode, Scenario Registry, assertions, golden corpus, determinismo, bot roadmap
> v0.1/Tactical/Expert e test scenarios. Il valore **incrementale** di questo documento è il §6 del referto.
>
> **Il testo sotto non è stato riscritto.**
