...
>
> 🟡 **Dodicesima volta, e la seconda con lo scarto dichiarato prima di prodursi.** Questo ramo
> (`docs/consolidamento-4-processi`) porta la radice da **4** a **5** e il totale a **71** =
> 17 + 47 + 2 + **5**, misurato sulla propria base col comando completo. Ma
> [#836](https://github.com/DegrassiAaron/refactor-tactics-main/pull/836) archivia **nella stessa
> radice**: quando entrambe atterrano il totale sarà **72**, e nessuno dei due rami può scriverlo
> perché nessuno dei due vede l'altro. Previsto leggendo `gh pr list --state open` *prima* di
> scrivere il numero, come l'undicesima. Il rimedio resta **rimisurare sull'albero mergiato**.
>
...
> *(rimisurato il 2026-08-14 col comando completo — `find docs/archive/src -name '*.md' ! -name README.md | wc -l` → **71** = 17 `design/` + 47 `handoff/` + 2 `audit/` + **5 in radice**, dopo l'archiviazione del consolidamento *4 processi paralleli*; il valore precedente era **70** = … + **4 in radice**, misurato il 2026-08-13 sera sull'albero mergiato dopo *Decision Time Bank*, i due sorgenti *mouse interaction*, il consolidamento *roadmap→v1.0* e l'handoff *Wiki player-first*, ciascuno con la sua riga d'indice qui sotto. ⚠️ **Il valore va rimisurato dopo il merge**, non incrementato: è la lezione che questa riga ha già imparato nove volte, e un `+1` scritto prima dell'unione è giusto solo se nessun altro ramo archivia oggi.
