...
> col comando.
>
> 🔴 **Quindicesima rimisura, 2026-08-17 — e il totale era indietro di TRE, non di uno.** Il ramo
> `docs/tactical-designer-consolidamento` ha eseguito il comando canonico sulla propria base **prima** di
> archiviare: **79**, non i **76** che la riga qui sotto dichiara. Con la propria archiviazione: **80** =
> 17 `design/` + 48 `handoff/` + 2 `audit/` + **13 in radice**. Non è un `+1` sommato al valore scritto — è
> il comando, e i due numeri non coincidevano già prima che questo ramo toccasse la cartella.
>
> ⚠️ **E lo scarto ha un nome, misurato invece che ipotizzato.** L'indice della radice risponde **9** contro
> **13** file su disco, e i quattro scoperti sono:
> `CLAUDE_Apply_Elemental_Proficiency_Consolidation_2026-08-16.md`,
> `CLAUDE_Reconcile_v0.1_Skill_Ability_Issues_2026-08-16.md`,
> `RefactorTactics_v0.1_Characters_Elemental_Consolidation.md` e — fino a questo commit — il sorgente
> *Tactical Designer*. **Solo l'ultimo è stato aggiunto qui.** Gli altri tre appartengono alle track
> `docs_kit` e `proficiency` di [`parallel-batch.yaml`](../../roadmap/parallel-batch.yaml), e la loro riga
> d'indice deve dire *da chi sono stati recepiti* — un'informazione che chi non ha fatto quel consolidamento
> non possiede. Scriverla a indovinare sostituirebbe una lacuna nota con una falsa precisione, che è
> esattamente ciò che la nota in fondo a questa pagina rimprovera al conteggio `40`. È la stessa forma di
> [#579](https://github.com/DegrassiAaron/refactor-tactics-main/issues/579), fuori da `handoff/`.
>
> ⏭️ **Previsione, scritta prima del merge**: la PR
> [#1099](https://github.com/DegrassiAaron/refactor-tactics-main/pull/1099) archivia
> `CLAUDE_RefactorTactics_Graybox_Kit_Cover_CellVolume_Consolidation_2026-08-17.md` **nella stessa radice**,
> e nessuno dei due rami vede l'altro. Quando atterrano entrambi il valore sarà **81** = 17 + 48 + 2 + **14**,
> e questa riga andrà in conflitto. Non si somma: si riesegue il comando **sull'albero unito**.
>
> Gli **80** documenti Markdown in questa cartella sono i **sorgenti** da cui è nata parte della documentazione
