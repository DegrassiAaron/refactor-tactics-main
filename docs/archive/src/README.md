...
>
> Gli **82** documenti Markdown in questa cartella sono i **sorgenti** da cui è nata parte della documentazione
> normativa.
> *(⏱️ **Quindicesima rimisura, 2026-08-17**: `81` → **82** con l'archiviazione del kit *MultiHero / Decision
> Time Bank / Preferred Response*. Misurato col comando canonico e **scomposto**, che è il controllo che il
> totale da solo non offre: `17` `design/` + `48` `handoff/` + `2` `audit/` + **`15` in radice** = **82**.
> Nessun addendo incrementato a mano. ⚠️ **E il rapporto fra totale e indice non torna, misurato a parte**:
> l'indice della radice risponde **12** — `` sed -n '/^## Radice/,/^## /p' … | grep -c '^| [`' `` — contro
> **15** file sul disco. I **tre** scoperti sono `CLAUDE_Apply_Elemental_Proficiency_Consolidation_2026-08-16.md`,
> `CLAUDE_Reconcile_v0.1_Skill_Ability_Issues_2026-08-16.md` e
> `RefactorTactics_v0.1_Characters_Elemental_Consolidation.md`, e **non** vengono indicizzati qui: appartengono
> alle track `docs_kit` e `reconcile_skill` in `parallel-batch.yaml`, e indovinare *cosa quel sorgente ha
> prodotto* sostituirebbe una lacuna visibile con una plausibile — è la ragione già scritta cinquanta righe
> più su, applicata invece che ricordata. È la stessa famiglia di [#579](https://github.com/DegrassiAaron/refactor-tactics-main/issues/579).
> ⚠️ Il valore va **riletto sull'albero mergiato**: `+1` scritto prima dell'unione è giusto solo se nessun
> altro ramo archivia oggi, e oggi ce ne sono tre aperti.)* ➕ **E un file che non è Markdown**: `handoff/2026-08-12-grid-data-consolidation-audit.xlsx`.
...
> 2026-08-17**: la riga in testa non dice più `40` da parecchi giri, e quel comando rispondeva **81** quando
> questa correzione è stata scritta — **82** dopo l'archiviazione del kit *MultiHero* nella stessa giornata,
> che è il motivo per cui una cifra dentro una nota di correzione va datata come tutte le altre —
