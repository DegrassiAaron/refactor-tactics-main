...
> `find docs/archive/src -name '*.md' ! -name README.md | wc -l`.
>
> - **Albero di questo lavoro** (`4396d2b2`, **detached**, 20 commit dietro `origin/main` `ef0414bc`),
>   che aggiunge il solo `handoff/2026-08-31-github-epic-issue-reconciliation.md`: **`119`** =
>   `17` `design/` + **`71`** `handoff/` + `2` `audit/` + `4` `graytoolkit/` + `25` in radice. La somma torna.
> - **Albero `origin/main`** (`ef0414bc`), senza quel file: **`118`**. `118 + 1 = 119`, **e stavolta senza
>   riserve**.
>
> ✅ **La divergenza di 20 commit non tocca questa cartella**: `git diff --name-only HEAD origin/main --
> docs/archive/src` restituisce **zero** file. È la ragione per cui qui non serve il doppio conteggio che i
> due blocchi precedenti hanno dovuto dichiarare — non perché l'albero sia allineato, ma perché la parte di
> albero che questo totale misura lo è. ⚠️ La verifica va **rifatta**, non ereditata: è esattamente la
> differenza che nel giro del 2026-08-30 valeva un file cancellato e nove link rotti.
>
> ⚠️ **Il sorgente di oggi non è verificabile con `diff`**, ed è il primo di questa cartella a non esserlo:
> è arrivato **inline** come argomento di `/sc:spec-panel`, non come file, quindi la copia archiviata è una
> **trascrizione** e non un originale spostato. Dichiarato in testa al file.
>
> 📏 **Rimisura del 2026-08-30 (secondo giro del giorno) — kit *Tactical Grid Overlay*, e il totale va
> dichiarato DUE volte perché l'albero di lavoro è indietro.** Criterio invariato: **solo `.md`**,
> `README.md` escluso per basename, comando canonico
> `find docs/archive/src -name '*.md' ! -name README.md | wc -l`.
>
> - **Albero `origin/main`** (`aec66789`): **`112`**.
> - **Albero di questo lavoro** (`main` @ `d9feb9b0`, **7 commit dietro**), che aggiunge il solo
>   `handoff/2026-08-30-tactical-grid-overlay-issue-driven.md`: **`114`** = `17` `design/` + **`66`**
>   `handoff/` + `2` `audit/` + `4` `graytoolkit/` + `25` in radice, e la somma torna. Contando **tutti i
>   file**, stessa esclusione per basename: **`123`**.
>
> 🔴 **`112 + 1 = 113`, non `114`, e la differenza non è un errore di conteggio: è un file che `origin/main`
> ha cancellato e questo albero ha ancora.** È `RefactorTactics_FeatureRegistry_Roadmap_Wiki_Claude_2026-08-08.md`
> in radice, rimosso dal commit `c0cc0693` che questo checkout non ha ancora. Sul ramo principale la radice
> è `24`, non `25`. Il numero che varrà dopo il merge è **`113`**; il `114` è vero del disco su cui la
> misura è stata presa, ed è per questo che il blocco precedente pretende di dichiarare **l'albero** e non
> solo il totale. È la stessa riserva del 2026-08-29 — *«il worktree protegge dalle scritture concorrenti,
...
> nella stessa forma. Criterio invariato dal blocco precedente — *nessun link Markdown al file da questa
> pagina*, misurato con `']([^)]*\.md)'` sul README contro il `find`. ⛔ **Non lo indicizza questo giro**: la
> riga la scrive chi ha revisionato quel kit.

> 📏 **Rimisura del 2026-08-30 — consolidamento dei Markdown di radice, e il criterio dello scarto si è
...
> al file da questa pagina*, misurato con `grep -o ']([^)]*\.md)'` sul README contro il `find` qui sopra.
>
> 🔴 **E il secondo criterio del 2026-08-29 — «senza alcuna menzione, nemmeno in prosa» — oggi risponde
> `0`, ma non perché il difetto sia stato chiuso: perché quel blocco li ha nominati tutti e sei per
> diagnosticarli.** Un criterio che cerca una stringa in una pagina smette di funzionare nell'istante in cui
> la pagina scrive quella stringa per lamentarsene. Resta utile il primo, che chiede una **riga**, non una
> menzione.
>
> ⚠️ **Il `7` del blocco precedente e il `10` di oggi non misurano la stessa cosa.** La differenza sono
> esattamente i tre file di `docs_kit` — `CLAUDE_Apply_Elemental_Proficiency_Consolidation_2026-08-16.md`,
> `CLAUDE_Reconcile_v0.1_Skill_Ability_Issues_2026-08-16.md` e
> `RefactorTactics_v0.1_Characters_Elemental_Consolidation.md` — che questa stessa pagina dichiara in fondo
> a §«Radice» come *«senza riga d'indice»* e che il conteggio non contava. `7 + 3 = 10`: nessun file nuovo è
> entrato senza riga da allora.
>
...
>
> 🔁 **Rieseguita il 2026-08-30, ed è falsificata nel verso che lei stessa prevede.** L'albero di `20d59973`
> risponde **`104`** al comando canonico, con **`58`** `handoff/` — non `102`/`56` né `103`/`57`. Fra il 28 e
> il 30 agosto ha archiviato **anche qualcun altro**, ed è esattamente il caso in cui questa pagina prescrive
> di **rieseguire invece che aggiustare**: i numeri qui sopra restano scritti, non corretti. Il **disco** ne
> porta **`105`** e **`59`**, e l'unità di differenza è questo giro —
> [`2026-08-30-crud-epic-issue-mcp.md`](handoff/2026-08-30-crud-epic-issue-mcp.md), archiviato `untracked`
> mentre questa riga viene scritta, insieme alla propria riga d'indice.
...
| [`2026-09-09-prompt-scenario-lab-non-eseguito.md`](handoff/2026-09-09-prompt-scenario-lab-non-eseguito.md) | Prompt per dieci milestone *Scenario Lab v0.1…v1.0*, epic ombrello e issue figlie | ⛔ **respinto, non eseguito** — spec panel del 2026-09-09: baseline v0.1 non ferma, controllo duplicati che non trova [#1105](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1105), e un **terzo nome** per la stessa superficie contro [`spec-tactical-designer.md`](../../technical/tooling/spec-tactical-designer.md) §2. La disposizione è nel banner del file |
...
| [`2026-08-29-visual-slice-v01-master.md`](handoff/2026-08-29-visual-slice-v01-master.md) | Visual Slice v0.1: sequenza a quattordici nodi da `L_DevSandbox` graybox a packaged gate — precheck GBX, scena U25, grammatica board, LOS overlay, conoscenza parziale, velo, HUD UMG, catalogo icone — più due issue proposte e un template di handoff | [referto](../../roadmap/plans/visual-slice-v01-spec-panel-2026-08-29.md). 🔴 **La sequenza è dichiarata vincolante e i primi tre nodi lavorano su [#1094](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1094), CLOSED dal 2026-08-19** — uno di essi dice *«riapri #1094»*. 🔴 Il nodo 5 riapre [#956](https://github.com/DegrassiAaron/refactor-tactics-main/issues/956) (CLOSED 2026-08-23) e chiede una grammatica che è in `Source/`: `URTHexLibrary::SurfaceRingCount` + `GetCellGlyphMesh`, con il gate `Hex.SurfaceColorsAreDistinguishable` che già confronta gli anelli. 🔴 Il nodo 1 manda a verificare `GBX-2` e `GBX-4`, chiuse il 2026-08-18 da `D-171`/`D-173`, e prescrive *«non creare la porta»* mentre `SM_Graybox_Door_Locked` è già versionata. 🟠 `NEW-01` propone un kit graybox di cui **sei mesh** esistono già, generate da `RTBuildGrayboxMeshesCommandlet` sotto `D-173`. 🔴 **E delle cinque voci che chiede ne resta UNA**, incrociando §8 del contratto: «Wall Full» è l'elemento #5 la cui presentazione **esiste come volume derivato** (`RTHexMapActor.cpp:1272`, sui `InteriorWalls`), «Wall Broken» è **`DEFER`** (dipende da `RT-FEAT-MAP-STRUCTURAL`, v0.2+), «Pillar» e «Door Frame» **non sono nel catalogo dei diciannove**. 💡 E nessuna mesh del kit usa un **inset** — `CellPlan` è il footprint esterno, il resto è `EdgeBound` — quindi `GBX-1` non blocca l'authoring di nulla: il primo volume `CellBound` non esiste ancora. 🟠 Il nodo 10 è [#1535](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1535) riscritta più corta, senza la tabella delle tre uscite né il precedente dello `0` cablato a `RTHUD.cpp:391`; i sei WBP che il nodo 12 cita *«storicamente»* esistono tutti. ✅ **Un solo contributo netto**: `NEW-02`, il LOS debug overlay — quindici `rt.*` in `Source/` e nessuno per la LOS — più il nodo 14 come unico gate visivo end-to-end. ⚠️ **Terza fotografia della stessa famiglia in due giorni**, e `#1094`/`#956` erano già il rilievo `C2` del referto GrayKit del giorno prima. ⛔ **Nessun nodo eseguito**, nessun `D-nnn` assegnato. ➕ **Due azioni, autorizzate dopo il referto**: `NEW-02` aperta come [#1712](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1712) e `NEW-01` **ridotta al solo materiale** in [#1714](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1714) — le sei mesh escono dal commandlet con `FStaticMaterial()` vuoto mentre un test garantisce che siano ombreggiabili (`D-229`) — non copiando il corpo del kit, ma riscritto sulle misure: la reason **non** è esposta (`HasLineOfSight` è un `bool` nudo) ed è derivabile senza toccare l'algoritmo; **cinque `rt.Debug.Draw*` su sei non disegnano** e `RTDebugConsole.cpp:163` lo dichiara; il comando va nel dominio che ispeziona e non in `Debug/` ➕ **Seconda consegna il 2026-08-30**, lo stesso kit rimpaginato in un file solo (`RefactorTactics_Claude_v0.1_VisualSlice_ALL_IN_ONE.md`, 1284 righe): **743 righe uniche su 768 sono verbatim** di questo sorgente e il contenuto tecnico aggiunto è **zero** — le 25 righe nuove sono un cappello d'uso e un prompt di lancio. **Non archiviata a parte**: due copie dello stesso kit sono due source of truth (§13 del referto), e il delta sta verbatim in [§18](../../roadmap/plans/visual-slice-v01-spec-panel-2026-08-29.md). Scritta alle `2026-08-29T22:16:40Z`, cinque minuti prima che la PR del referto entrasse in `main` |
| [`2026-08-30-claudecloud-debughud-graybox.md`](handoff/2026-08-30-claudecloud-debughud-graybox.md) | Debug/Graybox HUD v0.1: **un solo slice** dello Screen HUD — round, fase, timer, Ready opzionale — da costruire come `WBP_RT_TurnHeader` stilizzato a graybox invece che come widget di debug parallelo, con gate `G01`–`G18` / `R01`–`R06`, checklist d'Editor e formato del report finale | [referto](../../roadmap/plans/debug-hud-graybox-spec-panel-2026-08-30.md). ✅ **Per una volta la fotografia è in pari**: dichiara `fff33020` ed è esattamente `origin/main`; le tre issue che nomina — [#613](https://github.com/DegrassiAaron/refactor-tactics-main/issues/613), [#77](https://github.com/DegrassiAaron/refactor-tactics-main/issues/77), [#705](https://github.com/DegrassiAaron/refactor-tactics-main/issues/705) — sono tutte `OPEN`, e manca il difetto ricorrente di questa famiglia. ✅ **La sua §2 è la parte che vale**: vieta il doppione `WBP_RT_DebugPlanningHeader` e impone che il graybox sia *il widget canonico stilizzato male*, così che il passaggio d'arte sostituisca lo stile e non l'impianto. 🔴 **Ma ordina di costruire ciò che esiste da cinque commit**: `WBP_RT_TurnHeader.uasset` c'è, con `RoundText`/`PhaseText`/`TimerText` in un `HorizontalBox`, legati a `GetRoundCounterText` e a `ERTMatchPhase`; `WBP_RT_TacticalHUD` lo compone già insieme a roster, pannello unità e action dock; `DefaultGame.ini:65` cabla `MatchHudWidgetClass`; `PresentMatchHud()` è nel Navigator; `rt.HUD.CanvasPanels` e `bShowDebug` esistono. **Dieci delle sue quattordici caselle P0 erano già verdi all'arrivo**, e la sua checklist d'Editor usa nomi (`Text_Round`…) diversi dai reali, quindi eseguita alla lettera **duplica o sovrascrive** l'asset. 🔴 **Il timer**: chiede `--:--` per «nessun contesto» mentre il repository ha già scelto `—`, e `MM:SS` mentre l'implementazione mostra secondi troncati per decisione committata — due divergenze introdotte riscrivendo da zero un difetto che `16ba67a4` aveva già diagnosticato *e progettato* («serve un `Select` su `< 0`»). 🟠 **Ready costa quattro sezioni e sei gate per un ramo già determinato**: `FRTMatchHeaderView` non ha il campo e `SetReady`/`RequestReady`/`CommitPlan` hanno **zero** occorrenze in `Source/` — omesso, e la materia è di `#77`, non di `#613`. 🟠 Il suo §20 dichiara «fuori scope» tre widget che il root **già ospita**: letto insieme a *«non allargare il passaggio»* autorizza una rimozione. 🟠 **La causa a monte non è il kit ma il registro**: il piano di `#613` porta **61 caselle e zero spuntate** mentre il lavoro è al Task 7-bis, e il kit manda a leggerlo — la riconciliazione di quelle caselle vale più del suo intero P0. ➕ **Ed è stata fatta lo stesso giorno**, nella sezione «Riconciliazione del registro» in testa al piano: **36 caselle su 61** spuntate su `285d2322`, e due reperti che nessuno dei due documenti aveva — lo **Step 7.4 non era implementato** (`WBP_RT_ActionDock` non chiamava `GetArmedActionIndex()` e `bArmed` era una costante `false`: nessuno slot poteva accendersi mai, e lo stato neutro di [D-128](../../decisions/RT_PDR_00_Decision_Log.md) era corretto per la ragione sbagliata — ✅ **chiuso lo stesso giorno**, insieme alla guardia di ricostruzione che confrontava una variabile mai ripopolata e faceva ricostruire il dock **a ogni tick**) e `WBP_RT_UnitCard`, che il piano non prevedeva, ha reso migliore ciò che chiedeva — le barre stanno in una card riusabile invece che duplicate in due Blueprint. ⛔ **Nessuna suite, nessuna build, nessun Editor aperto** — i due `.uasset` sono misurati per estrazione di stringhe — **e nessuna scrittura su GitHub**: nessuna issue aperta, chiusa o commentata |
| [`2026-08-30-crud-epic-issue-mcp.md`](handoff/2026-08-30-crud-epic-issue-mcp.md) | Mandato CRUD su Epic e Issue GitHub per la roadmap v0.1: ventotto sezioni, ancore GitHub, roadmap operativa `R0`–`R7`, batch A–E, protocollo Unreal MCP e gate di release | [referto](../../roadmap/plans/crud-epic-issue-mcp-spec-panel-2026-08-30.md) · ✅ **Il kit più accurato arrivato su questo terreno**: **29/29** ancore GitHub esistono coi titoli che attribuisce loro, i **10** percorsi del preflight ci sono, la baseline `G1–G14` è corretta (`G15` è ritirato da `D-181`) e «*il parent `#14` può essere indietro*» è **vero** — quattro epic `[EPIC v0.1]` aperte non vi compaiono: [#324](https://github.com/DegrassiAaron/refactor-tactics-main/issues/324), [#934](https://github.com/DegrassiAaron/refactor-tactics-main/issues/934), [#952](https://github.com/DegrassiAaron/refactor-tactics-main/issues/952), [#1408](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1408). 🔴 **Il §13 prescrive scritture che il ponte MCP non ha**: `RTDevToolset` espone **5** `UFUNCTION(meta = (AICallable))`, tutte `static` e tutte di lettura, quindi §13, l'ultima riga del §22 e la catena di chiusura del §23 non sono eseguibili. 🔴 **Terzo giro sullo stesso terreno**: non nomina i due gemelli consumati il 2026-08-28 né le 31 issue `#1557`–`#1587` che il secondo ha aperto, e il suo `R2` ignora `E48` ([#1408](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1408)), che ne è l'owner più recente. ⚠️ **Senza data**, unico fra i kit archiviati. ✅ **Eseguito solo `R0`**, su conferma d'autore: `#14` passa da **21** a **25** epic figlie e perde la **quinta copia** del totale `21 epic, 100 checkpoint` — quella che lo sweep di [#962](https://github.com/DegrassiAaron/refactor-tactics-main/issues/962) non poteva vedere, perché cercava con `grep -rn "21 epic" docs/` e il corpo di una issue non è in `docs/`. Dichiarata anche la divergenza lato roadmap: `E48` è fuori dalla tabella §3 e non ha una serie `CP 48.x`, quindi il totale **non è stato toccato**. ➕ Eseguite anche le due sweep di `R0` su 286 issue aperte, e **riparato**: sette padri allineati (**21** figli che li dichiaravano e che il loro corpo non nominava; `#151` da sola ne guadagna undici) e tre righe `LINK` che nominavano l'epic per etichetta senza il numero, ora seguibili. **Zero duplicati** — cinque collisioni di `CP` verificate una per una, tutte split o follow-up; massimo Jaccard sui titoli **0.429** su 40 755 coppie. 🔴 **Due criteri meccanici su tre erano sbagliati nello stesso modo**: misuravano il *formato* della dichiarazione invece del fatto — la checklist ignorava 30 epic su 52, l'intestazione ignorava dieci issue che dichiarano l'epic in riga di tabella, e `CP 11.8` non era una collisione ma una gerarchia già dichiarata tre volte. ⛔ Zero issue create, chiuse o riaperte — lo STEP 3 del §25 autorizza sé stesso e il guardrail di `CLAUDE.md` §7 dice il contrario |

| [`2026-08-30-tactical-grid-overlay-issue-driven.md`](handoff/2026-08-30-tactical-grid-overlay-issue-driven.md) | Tactical Grid Overlay in modalità issue-driven: griglia hex semi-trasparente ON di default con toggle, hover della cella, selezione coerente, debug `FRTCellId`, multilivello, più audit GitHub, decisione Epic, 2–4 issue da creare o riusare, aggiornamento roadmap, PIE e packaged — 912 righe, 22 sezioni, senza data e senza `HEAD` | [referto](../../roadmap/plans/tactical-grid-overlay-spec-panel-2026-08-30.md). ✅ **La §0 è la sua parte migliore e si è ritorta contro di lui**: `SEARCH BEFORE CREATE`, `REUSE BEFORE DUPLICATE`, i path dichiarati «solo come hint», e *«se un gate non viene eseguito, scrivere `NOT RUN`. Mai dichiararlo verde»* — è la disciplina con cui il referto lo ha smontato. Reggono anche il divieto di `Disable Depth Test`, il «niente rebuild O(N) sul mouse move» e la reopen policy, che #1614 aveva già applicato due giorni prima. 🔴 **§14 chiede di «mantenere verdi» tre test che non esistono** — `HoverNeverCommits`, `HUDConsumesPointerBeforeWorld`, `HiddenEnemyCannotBecomeHoverTarget`: zero occorrenze, i `PlayerInput.*` reali sono **16**, ed è esattamente il difetto per cui [#1616](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1616) è stata aperta **e chiusa** il 2026-08-29. Eseguito alla lettera produce un verde su una prova assente. 🔴 **§9 `GridLiftCm = 2.0` disegna DENTRO il tile**: `RTCellTopZ` vale **7,5 uu** dal 2026-08-28 (era `2,5`), quindi il fill finirebbe `5,5` uu sotto una faccia opaca — il difetto che [`RTMapVisuals.h`](../../../Source/RefactorTactics/Map/RTMapVisuals.h) dichiara essere già costato **due** incidenti; e i sei default in **cm assoluti** contraddicono `D-168`/`D-163`, che hanno spostato le altezze a frazioni di `H` proprio perché uno spessore assoluto si era desincronizzato in silenzio quando `HexSize` è passato da `100` a `150`. 🔴 **Le issue A e B esistono già**: [#1614](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1614) (`OPEN`, `P1`, `v0.1`) copre hover visibile, separazione hover/selezione, no-Actor-per-cella e debug `HoveredCell`, con un oracolo **migliore** di quello del kit; [#705](https://github.com/DegrassiAaron/refactor-tactics-main/issues/705) possiede il contratto, [#622](https://github.com/DegrassiAaron/refactor-tactics-main/issues/622) la griglia d'editor. 🔴 **La §3 «Decisione Epic» ha come risposta il proprio Caso A**: [E11 #25](https://github.com/DegrassiAaron/refactor-tactics-main/issues/25) e [E21 #286](https://github.com/DegrassiAaron/refactor-tactics-main/issues/286), milestone *v0.1 · Leggibilità*, già popolata. 🔴 **Il modello «griglia sopra la mappa» non è quello del repository**: la board *è* le celle, e sopra il colore di superficie stanno il glifo ad anelli (`D-183`) e il velo che moltiplica l'RGB per `0,35` (`D-225`/`D-227`) — due canali che un fill translucido su tutta la board attenua, e che il kit non nomina. 🟠 `G` è occupato da `Action.Guard` e il toggle esiste come `rt.Debug.DrawCells` (uno dei **26** `rt.*`); «packaged Development» come DoD per-issue è **insoddisfabile** (suite Editor-only per costruzione: `1373` `EditorContext` contro `11` `ClientContext`); `HoveredCell == SelectedCell` introduce un'entità che il contratto non ha (`D-128`: il click in stato neutro **ispeziona**); il §13 multilivello è consegnato da #567/#588. ➕ **Un solo contributo netto, e non dove il kit lo mette**: `FRTCellId` non si legge a schermo — `grep HoveredCell` in `Debug/` e `UI/` dà **zero** — ed è già una casella di #1614. ➕ **E misurando per scriverla è emerso un reperto che falsifica una premessa di #1614**: l'overlay hover non è «grande esattamente quanto la cella», è `DrawCellOutline(…, 0.88f)` a `RTHexMapActor.cpp:994` — **12% più piccolo** — quindi la baseline `1,15×` è un `+31%` e non un `+15%`, la riga citata (`:718`) è scaduta, e l'hover **attraversa già le unità** senza che la issue lo registri. ⛔ **Nessuna issue creata, chiusa o riaperta; nessun `D-nnn` assegnato** (ultimo: `D-248`); nessuna riga di codice, asset o roadmap toccata. 🔵 **Una sola scrittura, come atto separato dopo il referto e su autorizzazione esplicita dell'autore**: le due correzioni raccomandate applicate al **corpo di [#1614](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1614)** — premessa rifatta sulla misura, nome della funzione di geometria corretto (`HexCorners`, non `HexVertices`: **quarto reperto**, emerso applicando), e due caselle di DoD in più (da `10` a `12`), entrambe di **conservazione** — quota sopra `RTCellTopZ` che riusa `RTLiftPreview` invece di scrivere un numero, e `SDPG_Foreground` che l'hover ha già. Nulla rimosso, stato/label/milestone invariati, corpo remoto riverificato `IDENTICO` a quello inviato |
| [`2026-08-31-td-skill-actionlab-roadmap-prompt.md`](handoff/2026-08-31-td-skill-actionlab-roadmap-prompt.md) | Consolidamento `TD 0.1 → TD 1.0` del Tactical Designer sul lato skill: loop *Character → Skill → Action Lab → Scenario Harness → TurnLog*, Golden Skill end-to-end, dieci stadi di maturità e dieci issue candidate `TD01-A`…`TD01-J` — 923 righe, 15 sezioni | [referto](../../roadmap/plans/td-skill-actionlab-roadmap-spec-panel-2026-08-31.md). ✅ **La sua disciplina è ciò che lo confuta**: il §5 impone *«REUSE / UPDATE prima di CREATE»* e il §11 pretende una matrice obbligatoria — applicandoli, le dieci candidate danno **CREATE = 0** (`REUSE 6 · LINK/UPDATE 3 · DEFER 1`). 🔴 **La scala `TD 0.1 … TD 1.0` esiste già**, decisa da [D-154](../../decisions/RT_PDR_00_Decision_Log.md) e scritta in **due** posti — §6 di [`spec-tactical-designer.md`](../../technical/tooling/spec-tactical-designer.md) e corpo di [#1105](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1105) — con checkpoint **M9.4**: nove stadi su dieci coincidono nel significato, ma la sua `TD 0.1` ne **riassegna il numero** a un'altra cosa, e quel numero è citato come *stadio con residui* da `#622`/`#695`/`#1186` e dalla riga `M9.1` di `roadmap-checkpoint.md`. Ciò che il kit chiama `TD 0.1` ha già un nome ed è un **taglio trasversale**, non uno stadio: la *TD Trial / Scenario Sandbox* del §6.1, decomposta in `T0`–`T8`. 🔴 **La Golden Skill è nominata con un token ritirato**: `Flux.ArcPulse` è stato rinominato in `Hero.Gadget.ArcPulse` da [D-130](../../decisions/RT_PDR_00_Decision_Log.md) e ha perso il redirect con [D-134](../../decisions/RT_PDR_00_Decision_Log.md) — `Flux.` dà **zero** occorrenze in `Source/` — mentre lo stesso §3 vieta di rinominare Stable ID. <!-- rename-exempt: la riga dichiara la rinomina del token: sostituirla la renderebbe muta --> 🔴 **Tre candidate su dieci descrivono capability consegnate**: `TD01-B`, `TD01-C` e `TD01-I` sono [#1117](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1117) più lo slice `T0`, protetto da `RefactorTactics.Scenario.RunFromTheEditorMatchesTheHeadlessRun` e da **dieci** test in `RTScenarioRunResetTests.cpp`; lo `StateHash` che chiede di introdurre è `RTMatchStateHash`. 🔴 **`TD01-F` chiede di creare uno scenario che esiste** — `Scenarios/Combat/BasicAttack.json`, seed fisso e aspettative dure — e `Hero.Gadget.ArcPulse` è già l'anchor del corpus con **34 occorrenze in 21 scenari**. 🔴 **La presentation sbaglia eroe**: «Gadget — Plasma Blast» non esiste nel pack (`FX/Abilities` = `ElectroGate`, `Primary`, `RollingBot`, `StickyBomb`, `Ultimate`, `VisionBot`); quel nome è di `ParagonTwinblast`, e l'audit che `TD01-J` ordina è già [#1663](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1663) col suo esito misurato — *«756 asset Paragon cotti, zero clip»*. ⚠️ **Il §9 indirizza `docs/technical/spec-tactical-designer.md`**, che non esiste (è sotto `tooling/`), e gli chiede di portare **stato**, che il banner di quell'owner dichiara essere un difetto; ⚠️ e usa `candidate`, il nome che `D-154` aveva già respinto in favore di `Variant`. ➕ **Il contributo netto non è nella sua lista di titoli**: la sua domanda — *quale singola azione ha un percorso completo dal catalogo alla presentation* — ha fatto emergere che `Combat.BasicAttack` asserisce `103` HP mentre **tre documenti owner e la nota interna dello scenario** dicono ancora `98`, uno scarto di `5` che è il `BaseShield` di `D-224`. ⛔ **Nessuna issue creata, aggiornata o chiusa; nessun `D-nnn` assegnato; nessun documento owner toccato.** 🔵 **Il sorgente è archiviato byte-identico**, col token ritirato dentro: sostituirlo cancellerebbe la prova su cui poggia il rilievo qui sopra |
