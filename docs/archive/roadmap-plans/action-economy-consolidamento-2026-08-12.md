...
| §19–§20 HUD e Ghost Timeline | **nuovo sul contenuto, esistente sulla forma** | `RT-FEAT-UI-PLANNING`, `RT-FEAT-UI-ACTION-GHOSTS`, [`progettazione-hud.md`](../../technical/systems/progettazione-hud.md). Il feedback dinamico ha senso **solo se** §7 viene adottata: senza compatibilità non c'è nulla da mostrare |
...
| §29 editor map | ✅ **chiuso lo stesso giorno: non serve** | La condizione era «quando `AE-1` è decisa», e [D-114](../../decisions/RT_PDR_00_Decision_Log.md) l'ha decisa poche ore dopo: **no**. Metà di ciò che §29 chiedeva di autorare — `ActionCost`, capacità residua, costo per azione — nomina cose che non esisteranno; l'altra metà (`MovementProfile`, `PivotCostPerStep`, `FacingPolicy`) **esiste già come dato** e non ha bisogno di una seduta per essere scritta. L'authoring diventerà lavoro vero se `AE-2` passa, perché la compatibilità abilità↔movimento è una matrice per abilità × profilo, cioè la prima cosa del catalogo che si sbaglia a mano. 🔄 **`AE-2` è passata lo stesso giorno** ([D-116](../../decisions/RT_PDR_00_Decision_Log.md)) — e la condizione si avvera **a metà**: il modello adottato non è la matrice ma una **soglia**, cioè **31 interi** in un catalogo markdown invece di 124 celle. Un numero per azione si scrive a mano senza strumenti, come tutti gli altri del catalogo. La seduta editor resta quindi **non necessaria**, ma per una ragione diversa da prima: non «non si sa cosa autorare», bensì «c'è poco da autorare». Si riaprirebbe se qualcuno reintroducesse una matrice. `editor-sessions.yaml` resta invariato, e `editormap.shortlist.md` con lui |
...
- **Non ha aperto sedute editor** (§29): senza `AE-1` non si sapeva cosa autorare. ✅ **Aggiornato due volte il 2026-08-12**, e la seconda smentisce la prima a metà. `AE-1` decisa **no** ha reso gran parte di §29 senza oggetto; `AE-2` decisa **sì** avrebbe dovuto riaprirla, ma il modello scelto è una **soglia** e non una matrice — 31 interi invece di 124 celle — quindi non c'è abbastanza da autorare perché una seduta serva. ⚠️ Registrato per intero perché la condizione era scritta e si è avverata: un «si aprirà con X» che nessuno rilegge quando X accade è il modo in cui una prescrizione atterra a metà.
