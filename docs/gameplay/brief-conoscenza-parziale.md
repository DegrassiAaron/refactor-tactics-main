...
| **D3-ter** | Il terreno esplorato accumula anche ciò che si è visto **attraversando**, non solo dalle pose dei due refresh — decisione di [`#2873`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2873) ramo **(a)**, implementata da [`#2885`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2885) *(2026-09-10)*. `CurrentVersion` **resta 2**: nessun campo nasce o muore | 🔴 **Il comportamento precedente non era una decisione, era un accidente di ordinamento.** I refresh sono due — `RefreshTeamKnowledgeForPlanning` e `RefreshTeamKnowledgeForBlast` — e l'ordine delle fasi è `Planning → Prep → Dash → Blast → Move → Cleanup`: la fase `Move` scorreva **intera** fra l'uno e l'altro, quindi un corridoio non visibile né dalla partenza né dall'arrivo restava «mai visto» **per sempre**. ⚠️ Non era solo grafica: `URTTurnLogLibrary` legge `VisibleCells ∪ ExploredCells` fail-closed per decidere quale cella il combat log può **nominare** come causa di un tiro fermato, e taceva su un muro che il giocatore aveva costeggiato. ⛔ **`Contacts` resta fuori**, ed è lo stesso argomento di `D-227` applicato una seconda volta: il terreno non si muove, un'unità sì — un contatto raccolto a metà transito sarebbe la vista sotto mentite spoglie |
| **D4** | La visibilità si ricalcola ai **confini di fase**, non a ogni micro-step | Il Dash precede il Blast: riposizionarsi *può* ancora aprire o chiudere una linea prima degli attacchi — il 90% del valore tattico, senza toccare «raccogli poi applica» (invariante #3). ⏱️ **Precisata da D3-ter** *(2026-09-10)*: resta vero per `VisibleCells` — ciò che la squadra vede **ora**, e su cui targeting, bot e privacy decidono — e **non** per `ExploredCells`, che da `#2885` accumula lungo le celle attraversate. I due campi rispondono a due domande diverse, e questa riga parla solo della prima |
...
| ~~**D11**~~ | ~~«Energia» è una **risorsa firma per personaggio**: nome e trigger di ricarica cambiano, la regola no (cap 4, ricarica 1)~~ ⛔ **SUPERATA** da [`D-265`](../decisions/RT_PDR_00_Decision_Log.md) (2026-08-30): non esiste una risorsa firma universale, l'economia comune è **slot, cooldown e drawback**. [`D-324`](../decisions/RT_PDR_00_Decision_Log.md) ha tolto `Energy` dal gameplay ([#610](https://github.com/DegrassiAaron/refactor-tactics-main/issues/610)). ✅ Un kit **può** dichiararne una propria, ma allora è sua e non «per personaggio» come regola generale | ~~Dà identità senza moltiplicare le meccaniche — coerente col pilastro «scelta orizzontale»~~ ⚠️ *Istruttoria conservata:* la ragione era buona, e il difetto stava nel renderla **universale** invece che opzionale |
...
soglia d'udito per eroe · contatto incerto con area a **due ampiezze** — larga `4`, stretta `2`, scelte dal **margine sopra soglia** ([D-113](../decisions/RT_PDR_00_Decision_Log.md)) · l'attacco rivela almeno la direzione · TurnLog filtrato.
