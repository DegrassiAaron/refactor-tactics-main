...
| **Round per partita** (2v2) | 10–14 (cap 14–16) | 🔴 **21** misurato su partita reale (2026-08-19, `-game -RTAutobattle` sulla configurazione **spedita**, registrato in #149) — **fuori banda del 50%**. Il **10** che questa riga portava è del 2026-08-06 ed è anteriore alla correzione del deadlock di #1088: era `HexMatch.PlaysToCompletion`, che allestisce l'arena nel test. ⚠️ Resta *«un bot contro un bot»*, e **D-102** lo rende inammissibile come evidenza di bilanciamento: [**D-184**](../decisions/RT_PDR_00_Decision_Log.md) decide di non ritarare nulla su questo dato, e accetta il pareggio allo scadere come esito legittimo . 🧭 **Competenza del bot accanto al numero** ([D-102], `#543`, [`bot-competence.yaml`](bot-competence.yaml)). ⚠️ **Quali capability determinino la durata e' una SCELTA, non una derivazione**, e queste sono le quattro scelte: `Movement` e `BasicAttack` perche' una partita finisce avvicinandosi e colpendo, `MapControl` perche' l'obiettivo e' la condizione di vittoria alternativa, `AdvancedCounter` perche' e' il modo in cui un giocatore la accorcia. `Defense` e' lasciata fuori: allunga la partita invece di deciderla, e includerla renderebbe la lista «tutto». Stanno cosi' — `Movement` `PASS` per Muiren, Branth e Ivrin, `PARTIAL` per Aevik; `BasicAttack` `PASS` per il solo Ivrin; `MapControl` `PASS` per **nessuno** (`PARTIAL` per Muiren e Branth, `UNTESTED` per Aevik e Ivrin); `AdvancedCounter` `FAIL` per tutti e quattro. ∴ il **21** e' prodotto da un bot non certificato a colpire con tre eroi su quattro, che sa prendere un obiettivo solo con Branth e in un solo scenario, non sa negare il terreno e non anticipa mai: la conclusione ammessa resta *«il bot non sa giocarla»*, non *«la partita e' lunga»*. Gli stati si rileggono con `python tools/bot-competence/check.py --check`. |
