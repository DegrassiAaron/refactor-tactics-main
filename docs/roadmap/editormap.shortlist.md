...
Attivare la modalita' non presidiata (CP 47.1), avviare su **mappa esagonale multilivello** — non
...
⚠️ **Le voci nuove — `PIE-V01-BOARD`, `PIE-V01-PACKAGED` — non sono state aperte.**
`docs/technical/test-manuali-pie.md` e' nel `writable` della track `content_editor` (#451), e D-139
dice che un file non assegnato e' uno STOP. Si aprono quando quel file torna `integration_only`, o
con una riallocazione dichiarata. E' lo stesso vincolo che E46 ha dichiarato per le sue sei
`PIE-V01-FRONTEND-*`.
