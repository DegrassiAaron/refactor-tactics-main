...
| **38.3** 🥇 | La compatibilità abilità↔movimento come dato — **primo lavoro dell'epic** | Un'abilità dichiara come si comporta sotto ciascun profilo (`NORMAL`/`IMPAIRED`/`ENHANCED`/`BLOCKED` o equivalente) e il validatore la legge. Criterio d'accettazione, nella forma di quello di E4: **aggiungere «Wraith spara in corsa» non deve toccare `ARTTurnManager`**. Se lo tocca, il modello non serve. ✅ **`AE-2` decisa sì** il 2026-08-12 ([D-116](../decisions/RT_PDR_00_Decision_Log.md)): categorie da negare allo `Sprint` = **precisione, preparazione, azioni pesanti**. 🔴 **Il checkpoint cresceva**: D-116 porta con sé la **migrazione di fase dello `Sprint`** (da `FastMovement` a `NormalMovement`, superando `D-068`) e `Status.Exposed` a **2 turni** — e le tre voci **non sono separabili**, perché la migrazione da sola produce l'upgrade puro vietato da `D-015`. ✅ **Quelle due sono atterrate il 2026-09-12** con [#641](https://github.com/DegrassiAaron/refactor-tactics-main/issues/641): il gate ha cambiato nome in `Actions.SprintIsAMoveProfileResolvedAfterBlast` invece di cadere, e i costi allora previsti — golden replay da rebaseline, 4 file di test su `Exposed` — sono stati pagati lì. ⛔ **Resta questa**, la compatibilità abilità↔movimento, che è quindi tutto ciò che il checkpoint deve ancora fare. ⚠️ Il **bot** invece non è un costo: non nomina `Action.Sprint` — e non sceglierlo mai è un dato a sé — che dal 2026-08-12 ha un **modello davanti** invece di un'idea: [`../gameplay/spec-compatibilita-azioni-movimento.md`](../gameplay/spec-compatibilita-azioni-movimento.md) (`PROPOSED`). Una **soglia** invece della matrice del kit — 31 numeri + 4 contro 124 celle — e **tre stati invece di quattro**, perché `ENHANCED` dipende dai fatti del percorso (`AE-3`). ⚠️ Due costi che il kit non mostrava: i **profili di movimento non esistevano come tipo** (`grep -rn MovementProfile Source/` → zero il 2026-08-12), quindi il primo lavoro non era assegnare soglie ma dargliene uno — ✅ **fatto da `#653`** il 2026-09-12, e questo checkpoint ora trova il profilo su cui scrivere `Stability`; e senza **38.2** il dato non è osservabile |
...
>
> ✅ **Chiuso il 2026-09-12 — il tipo esiste.** `#653` ha consegnato `FRTMovementProfile` (`Id`,
> `StepBudget`, `MoveBudget`, `Stability`) col suo catalogo: `Still`, `Move`, `Sprint` 8, `Withdraw` 2
> ([D-070](../decisions/RT_PDR_00_Decision_Log.md)) e `Sneak` **senza numeri**, quindi non pianificabile
> (`AE-5`). Il profilo si **ricava dal piano** — l'azione di movimento nomina il proprio
> (`FRTActionDef::MovementProfileId`) — invece di essere un campo che potrebbe contraddirla.
> ⚠️ **Nessun numero si è mosso**: `Move` e `Still` ereditano il budget dall'unità, e le famiglie
> `HexMatch` · `HexMove` · `Movement` · `Scenario` restano verdi. Mutando l'eredità a `0` ne cadono 44 —
