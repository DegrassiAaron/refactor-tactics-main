...
| **39.6** | Rumore, percezione e privacy | `DepartureNoise` e `ArrivalNoise`; **mai** rumore lungo il percorso. In Planning la destinazione è **team-only** — riusa `CanonicalIntentStore`/`FilterForTeam` e il dominio di [#159](https://github.com/DegrassiAaron/refactor-tactics-main/issues/159). Mai in un Actor globalmente replicato |
