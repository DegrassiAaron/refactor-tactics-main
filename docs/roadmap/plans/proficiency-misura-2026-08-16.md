...
Fonte: `Source/RefactorTactics/Ability/RTHeroCatalogLibrary.cpp`, le quattro factory `MakeGadget`, `MakePhase`, `MakeRiktor`, `MakeWraith`.
...
| `Hero.Riktor.KineticPanel` | crea copertura (struttura, non superficie) | ❌ |
| `Hero.Riktor.Reconfigure` · `Hero.Riktor.Ram` · `Hero.Riktor.Interposition` | nessun effetto elementale | ❌ |

### Wraith (`Hero.Wraith`) — `Affinity.Movement`, debolezza `Affinity.Structures`

| Abilità | Effetti dichiarati | Elementale? |
|---|---|:--:|
| attacco base | `Damage 21` | ❌ |
| `Hero.Wraith.InterceptShot` · `PassingBlade` · `Feint` · `Deflection` | solo `Damage` | ❌ |
