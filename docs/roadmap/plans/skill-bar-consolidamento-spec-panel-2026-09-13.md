...
⛔ **Costo**: li pinnavano i test qui sotto — ⏱️ **elenco del 2026-09-13; sei di questi nomi non esistono più dal 2026-09-20**, quando la PR di [#3129](https://github.com/DegrassiAaron/refactor-tactics-main/issues/3129) li ha riscritti come [D-408] prescrive: `GuardPoolIsPermutationInvariant` → `GuardReductionIsPermutationInvariant` (e la sua metà `Deflect` → `DeflectPoolIsPermutationInvariant`) · `GuardPoolRemainderIsNotWasted` → `GuardReductionHasNoCeiling` · `GuardPoolIsNotConsumedFromBehind` → `GuardReductionSkipsHitsFromBehind` · `DeflectPoolAbsorbsBeforeGuardPool` → `DeflectAbsorbsBeforeGuardReduces` · `Actions.Guard.FirstHitOnly` → `Actions.Guard.ReducesEveryFrontalHit` · `Actions.Guard.ReducesFirstDirectDamageInMatch` → `Actions.Guard.ReducesDirectDamageInMatch`. ⚠️ *Un filtro d'automation su un nome di questo elenco eseguirebbe **zero** test senza dirlo: è lo stesso rimando morto che questo programma ha già pagato una volta. Trovato da una code review.* Erano in `RTCombatResolverTests.cpp` · `RTCoreActionTests.cpp` · `RTDamageBreakdownTests.cpp` · `RTFacingDefenseTests.cpp` · `RTHexCombatIntegrationTests.cpp`. `Combat.GuardPoolIsPermutationInvariant` ·
