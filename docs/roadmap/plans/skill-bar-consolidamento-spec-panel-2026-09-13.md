...
### 11.3 Da aprire — le migrazioni che rendono eseguibili le decisioni

Una per decisione, perché una per decisione è la granularità a cui si può tornare indietro: `D-408`
(Guardia per colpo, coi suoi dodici test) · `D-409` (`Evasion`) · `D-410` (`Reflect`) · `D-411` (occasione
per nemico) · `D-413` (sequenza di `Interact`) · `D-414` (consumo al piano definitivo) · `D-415` (attacco
base, **dopo** `SKB-4` e `SKB-5`).

⛔ **`SKB-1` viene prima di tutte e sette**: quattro di esse incontrano lo stordimento, e finché non si sa se
la v0.1 lo riapre non hanno un soggetto su cui scrivere il proprio DoD.
