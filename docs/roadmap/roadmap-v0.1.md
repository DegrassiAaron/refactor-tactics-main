...
| **7.3** | Moduli reazione — **catalogo** (4 dei 7) | `EmergencyDash`, `ReactiveShield` (15), `CounterShot` (14), `AllyIntercept`: i quattro il cui trigger **esiste** (`HitByDirectAttack`, `AllyHitByDirectAttack`); una attivazione per turno, garantita dal percorso E5 e non da un secondo conteggio | `Equipment.ReactionModule.SingleActivation`, `Equipment.CounterShotUsesExistingTrigger` |
| **7.5** | Moduli reazione — **motore** (`#505`) | `HazardEscape`, `Cleanse`, `Anchor` nascono da un **evento** (`Push`/`Pull`/`Status`), non da un colpo: `EvaluateReactionTrigger` riceve `Hits` e `Intents`, quindi non è un enum da allungare ma un punto di valutazione da decidere. Diviso da 7.3 il 2026-08-11 perché il test che il DoD nominava non era scrivibile | `Equipment.Anchor.CancelsPush` (spostato qui da 7.3) |
