...
| **Obiettivi dinamici** | `Map/RTHexCellData.h`, `Turn/RTTurnRules.cpp` | 🟡 la partita **finisce** per obiettivo (`Match.EndsOnObjective`) e la **cella contendibile esiste nella mappa spedita**: `DA_HexMap_Arena` la porta a `(0,-3,L0)`, una sola e calpestabile — asserito da `Objectives.AuthoredArenaDeclaresItsObjective` — e il bot la gioca da CP 26.5 · 🟡 **e dal 2026-09-06 la mappa spedita ha UN oggetto da attivare**: `DA_HexMap_Arena` ha **64 celle e 1 porta** — `Closed`, su `(q=-3,r=2,L=0)` bordo `SE`, posata da `RTSetCellDoor` ([#2330](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2330)) con hash `2839750933 → 2786630622` e revisione `1974 → 1975` — e a dirlo è `HexMap.AuthoredArenaDoorCountMatchesTheRoadmap`. 🔴 *Fino al 2026-09-04 erano **zero**, e questa riga lo diceva: l'anello che mancava non era la capability, era chi SCRIVE.* La *capability* è chiusa — `Action.Interact` apre una porta (CP 10.1, [#74](https://github.com/DegrassiAaron/refactor-tactics-main/issues/74)) — ma i suoi scenari girano su mappe che si costruiscono da sole, e fino a [#2330](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2330) **nessuno dei tre asset mappa versionati** ne portava una. Rimisurati il 2026-09-06: `Arena` 64 celle e **1 porta**, `Sandbox` 0 celle e 0 porte, `_Scratch` **331** celle e 0 porte — e `_Scratch` si rigenera, quindi il suo conteggio non è un invariante. Le altre porte restano nelle fixture generate a runtime (`RelayBasin` una, `GrayKitYard` quattro) · ⚠️ resta 🟡 anche perché con **una** cella la contesa è irraggiungibile per costruzione (CP 31.1, [#1583](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1583)) |
...
| **Facing come stato di gioco** | `Turn/RTFacingLibrary.*`, `Unit/RTUnit.h`, `Combat/RTHexCombatLibrary.*` | ✅ E16 chiusa il 2026-08-09: derivato da Move e Dash, riorientato dal bersaglio, in snapshot/TurnLog e nell'hash della **traccia** (`URTTurnLogLibrary::HashTurnLog`), e l'emisfero posteriore annulla copertura e `Guard`. ⚠️ **Non nel checksum di STATO**: `FRTUnitStateDigest` omette il `Facing` — lo fa entrare [D-261](../decisions/RT_PDR_00_Decision_Log.md), aperta come [#1800](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1800) |
...
🔴 **Questa tabella si dimentica, e il 2026-08-27 le mancavano `E23` ed `E47`** — presenti in §3 e §5 con la
loro sezione `###`, assenti qui: 22 righe contro 24 epic. Chi aggiunge un'epic scrive la sezione e la tabella
dei checkpoint, e salta la vista di stato. Al posto di un avvertimento in prosa c'è un **comando**, nella
stessa convenzione dell'evidenza qui sopra. Output vuoto = completa:

```sh
diff <(awk '/^### 2\.1/,/^## [0-9]/' docs/roadmap/roadmap-v0.1.md | grep -oE '^\| \*\*E[0-9]+\*\*' | grep -oE 'E[0-9]+' | sort -uV) <(grep -oE '^### E[0-9]+' docs/roadmap/roadmap-v0.1.md | grep -oE 'E[0-9]+' | sort -uV)
```

> ⛔ **La prima stesura di questo comando, del 2026-08-27, era VACUA — e la riga che ne dichiarava la verifica
> era FALSA.** Greppava `^\| \*\*E[0-9]+\*\*` sull'**intero file**, ma ogni epic ha **due** righe di quella
> forma: una qui in §2.1 e una in §3. Il difetto descritto qui sopra — la riga che manca **solo** in §2.1 — non
> lo vedeva: usciva `0`, silenzioso. E la verifica di mutazione che lo accompagnava cancellava con `sed` **tutte**
> le occorrenze di `E23`, cioè entrambe le righe: una mutazione che il gate reagiva a vedere, ma che **non era
> il difetto**. Rompere la cosa sbagliata e leggere il rosso non è una verifica, è una coincidenza.
>
> L'`awk` restringe il confronto al **range di §2.1**. Riverificato il 2026-08-27 con tre prove:
> stato attuale → `0`; tolta **solo** la riga `E23` di §2.1 → `1`, e **nomina `E23`**; tolta **solo** quella di
> §3 → `0`, perché quella tabella non è di competenza di questo gate.
...
| **E7** Equipaggiamento e loadout | ✅ **chiusa 2026-09-02** | 29 test `Equipment.*` — le **sei varianti d'arma** esistono e il loro trade-off è un **numero** e non solo prosa: `URTEquipmentData` porta `DamageDelta`/`RangeDeltaCells`/`CooldownDeltaTurns`/`AddedEffects`, `ApplyWeaponVariant` li somma all'attacco base dell'eroe (mai valori assoluti: la variante non sa quale arma modifica) e il validator rifiuta una variante che non paghi in almeno uno dei tre. I **cinque gadget esprimibili** hanno cooldown 3 verificato e i numeri del catalogo; i tre non esprimibili sono **assenti e misurati** da `Equipment.Gadget.ThreeAreNotExpressibleYet`, che pinna anche le loro ragioni — un tripwire, non un rinvio. ✅ **`#2087` ha portato fuori il primo**: `Action.CreateSmoke` è entrata nel catalogo core e `Gadget.SmokeEmitter` la eredita, misurato da `Equipment.Gadget.SmokeEmitterRaisesTheSameSmokeAsMistVeil`, che pinna la catena azione→gadget→superficie→regola di vista. Restano `#2088`–`#2090`. `Equipment.SplitHasNoConsumerYet` pinna l'altro limite dichiarato: il motore v0.1 **non ha cardinalità dei bersagli**, quindi `Weapon.Split` in partita è solo il suo svantaggio · 🔴 *la riga contava **5** test e dichiarava `CP 7.2/7.3` bloccati perché «`HazardEscape`, `Cleanse` e `Anchor` chiedono tre trigger che il motore non ha — e il test `Anchor.CancelsPush` non è scrivibile finché non esistono». **I tre trigger esistono** (`CellBecameHazardous`, `AboutToReceiveControl`, `AboutToBeDisplaced`) e i tre test sono **verdi**; e `CP 7.4` era marcato ⏳ con `#63` chiusa. Rimisurata il 2026-09-02* |
...
| **E10** Obiettivi dinamici e fine partita | ✅ **chiusa 2026-09-02** | 39 test `Match.*` + 6 `Objectives.*` — fine partita a tre vie **con un test per via** (`EndsOnElimination`, `EndsOnObjective`, `EndsOnTurnLimit`), obiettivo contestabile verificato nel **Cleanup** dopo effetti ambientali e KO, progresso **intero** nell'HUD e nel TurnLog · 🔴 *la riga diceva «⏳ nessun oggetto da attivare in mappa» e contava **27** test: il primo è falso dal 2026-08 (`#74` chiuse `Activate`/`Interact`, `#833` ha portato porte e interruttori), il secondo era fermo. Rimisurata il 2026-09-02* · 🔵 **e quella rimisura era imprecisa su metà, corretta il 2026-09-04**: «#833 ha portato porte e interruttori» è vero per il **codice** e falso per la **mappa spedita** — `DA_HexMap_Arena` ha 64 celle e **0 porte**, misurato da `HexMap.AuthoredArenaDoorCountMatchesTheRoadmap`. Capability e contenuto sono due proposizioni diverse, e §2 ora le tiene separate · ⏱️ **e il contenuto è arrivato il 2026-09-06** ([#2330](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2330)): una porta `Closed` posata su `(q=-3,r=2,L=0)` bordo `SE` con `RTSetCellDoor`, quindi ora sono vere entrambe — restano comunque due proposizioni, non una |
...
| **E15** Showcase «Il Relè» e golden replay | 🟡 **CP 15.3 metà A e CP 15.4 chiusi** | 4 test `ShowcaseRelay.*` — fixture stabile, scenario lite deterministico, layout del bacino conforme alla spec · **CP 15.3 metà A (`8ca2cc7`, `#169`)**: gli intenti di una partita sono un **dato** e non un click — movimento e azione principale coesistono nello stesso turno, e con lo script delle decisioni **vuoto** lo scenario resta valido (`ShowcaseRelay.ScriptedInputsDriveMatch`) · ⏳ **metà B** (`DecisionProvider` iniettabile): bloccata non da CP 14.3, che è atterrata, ma dal **produttore** delle finestre — `FRTReactionOpportunity` non ha oggi nessun costruttore fuori dai test, quindi il provider non avrebbe interlocutore. Il produttore nasce con **CP 14.5** (`#165`) · ✅ **CP 15.4 chiuso il 2026-09-03** (`#170`): lo showcase gioca **tutti e otto** i turni con esito `PASS` e 22 assertion, e il suo corpus golden multi-turno — otto `turn-NN.rttl`, il più lungo del repository — pinna la partita intera. Il corpus copre ora **dieci** categorie di TurnLog su undici: `Objective` era dichiarata scoperta *«per assenza di contenuto, non di codice»*, e a coprirla non è stata una feature ma una riga di fixture — **nessun costruttore di arena posava un obiettivo**, quindi il T8 restava impossibile anche dopo la chiusura di `#75`. Il vocabolario di assertion guadagna il filtro per `ActionId`, senza il quale il T3 non era falsificabile ([D-162]: la causa di un danno sta in `ActionId`, non nella categoria) · ⏳ CP 15.5 a valle |
| **E16** Orientamento e direzionalità | ✅ **chiusa** | 13 test `Facing.*` + 3 `Combat.*` direzionali + 5 scenari `Spec.Facing.*` — il facing è stato di gioco (derivato da Move e Dash, riorientato dal bersaglio, in snapshot, TurnLog e hash **della traccia**, filtrato per squadra) e la difesa direzionale annulla copertura bassa e `Guard` fuori dall'arco frontale, con la stessa `HexCone` che userà la vista. Resta fuori `FacingUsedByOverwatch`, che è E14. ⚠️ **2026-08-10**: l'epic **resta chiusa** — i suoi due checkpoint sono stati consegnati e i loro DoD reggono — ma [ADR-0008](../decisions/adr-0008-rotazione-e-policy-di-facing.md) ha **allungato la spec** oltre ciò che E16 aveva costruito (budget di pivot per eroe, facing nei micro-step, policy dichiarative). Per questo `RT-FEAT-MAP-FACING` torna `IMPLEMENTING`: non è E16 a essere stata riaperta, è il metro a essersi spostato. Il lavoro nuovo va assegnato, e non ha ancora un checkpoint |
...
| **E19** Classe di mappa e composizione | ✅ **chiusa 2026-09-02** | 14 test `MatchFormat.*` + 7 `Player.*` — la mappa dichiara la propria **classe**, il formato dichiara **`UnitsPerTeam`** e **`UnitsPerPlayer`**, e il gruppo di controllo partiziona la squadra (`#1124`) · 🔴 *la riga diceva «⏳ la mappa non dichiara ancora la propria classe né il formato le unità per squadra» e contava **5** test: entrambi i buchi erano chiusi dal **2026-08-09** (`#215`/`#216`), cioè la riga ha dichiarato mancante per tre settimane lavoro già mergiato. Rimisurata il 2026-09-02* |
| **E20** HUD Icon Language | 🟡 **CP 20.1 chiuso · il codice di CP 20.3 è su `main`** | **6** test `IconCatalog.*` — ogni chiave risolve, l'ID duplicato e la chiave assente sono **errori di validazione**, la chiave sconosciuta cade sul fallback, le cinque categorie della v0.1 sono popolate · ✅ **i widget consumano il catalogo dal 2026-08-28** ([#1556](https://github.com/DegrassiAaron/refactor-tactics-main/pull/1556)): `DA_IconCatalog` esiste con **61 chiavi** e **62 texture**, e `URTActionSlotWidget::GetResolvedIcon()` passa da `ResolveIcon` · ⏳ [#220](https://github.com/DegrassiAaron/refactor-tactics-main/issues/220) resta aperta per la voce **`PIE-ICON-01`**, che è verifica visiva in editor e non ha sostituto headless · 🔴 il gate `ScreenHud.WidgetApiExposesNoTexture` copre le **sette classi C++** e **non vede i Blueprint**: una `Texture2D` dentro un `WBP_RT_*` lo attraversa — dichiarato nel DoD di #220, non scoperto a schermo <!-- rename-exempt: misura datata: riscriverla la renderebbe falsa --> |
| | | ⚠️ **Riga corretta il 2026-08-30** ([referto](plans/hud-icon-consolidamento-spec-panel-2026-08-30.md)). Diceva «**5** test» e «⏳ i widget **non consumano ancora** il catalogo»: entrambe le metà erano false. I test sono sei (`grep -c IMPLEMENT_SIMPLE_AUTOMATION_TEST Tests/RTIconCatalogTests.cpp`), e il consumo è su `main` da due giorni. È il difetto che questa colonna produce da sola: una riga di stato scritta prima di un merge non si accorge del merge |
...
| **E23** Muri, porte e interaction graph | 🟡 **E23.3 chiuso ([#832](https://github.com/DegrassiAaron/refactor-tactics-main/issues/832)) · E23.4 aperto ([#833](https://github.com/DegrassiAaron/refactor-tactics-main/issues/833)) · E23.6 ed E23.7 aperti il 2026-08-30 ([#1827](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1827), [#1828](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1828)) da `D-289`, con altre cinque sub-issue** | 23 test `InteractionGraph.*` + 27 `Structure.*` + 1 `Door.*`, misurati il 2026-08-27. Anticipata dalla v0.2 da [D-160](../decisions/RT_PDR_00_Decision_Log.md): non è scope nuovo, è una presa d'atto |
| **E46** Frontend shell e ciclo di partita | 🟡 **CP 46.1 chiuso · 46.2 completo, in attesa di aggancio** | **38 test** `Frontend.*` + `Startup.*` — **27** e **11**, rimisurati il 2026-08-18 sull'albero unito con due metodi che concordano: run headless (`Queue Empty 27 tests performed` e `11`) e conteggio statico dei nomi dichiarati in `Tests/`. ⚠️ **La riga precedente diceva «32 test» e «di CP 46.2 manca solo il `.uasset`»**: il primo numero era di due giorni prima, il secondo è stato falsificato dalla seduta **U24**, che ha consegnato **tutti e cinque** i `WBP_RT_*` — i tre con classe base più `FrontendRoot` e `ModalLayer`, che non ne hanno una e per i quali il criterio è `no_widget_creates_widgets`. Le otto `verification` delle due Binary Asset Lease sono **consuntivate**: due headless (matrice incrociata delle classi base, e zero `AddToViewport`/`RemoveFromParent`/`CreateWidget` con controprova 2/1/3 sul navigatore), sei dall'autore in editor. ⏳ Resta l'**aggancio**: nessuno chiama ancora `InitializeFrontend`, che è di CP 46.3 (`#938`). **17 test** `Frontend.*` era la misura di `#936` — back stack, modali, nessun dead-end verificato **per esplorazione con copertura misurata**, e il ciclo di vita dei widget. `FRTScreenStack` è un `USTRUCT` puro e `URTFrontendNavigator` un `UGameInstanceSubsystem`: è l'**unico** punto del codebase con `CreateWidget`/`AddToViewport`/`RemoveFromParent`, e la baseline era zero in tutto `Source/`. ⚠️ **La riga precedente diceva «zero test e zero codice … il repository non ha infrastruttura di test UI»**: la seconda metà era falsa già quando fu scritta — `RTScreenHudWidgetTests` (CP 11.7) prova widget headless — e la navigazione non è UI. ✅ **CP 46.6 chiuso il 2026-08-23** (`#941`): `ESC` apre la pausa — una **schermata**, non un modale, perche' altrimenti il `SETTINGS` della pausa non potrebbe essere quello di CP 46.3 — e `RETURN TO MAIN MENU` **smonta** il mondo invece di disegnarci sopra il menu. 🔴 Ha chiuso un buco che scavalcava all'indietro fino a CP 46.5: nessun `OpenLevel` riportava al frontend, e il consumatore di `PLAY AGAIN` non esisteva dentro una partita. **10 test** `RefactorTactics.Frontend.*` nuovi, fra cui la controprova che l'oracolo del criterio forte non e' vacuo. ⏳ Resta `WBP_RT_PauseMenu`, che e' la seduta `U30`: finche' non c'e', `ESC` copre la partita e non disegna nulla — `RESUME` da tastiera funziona lo stesso, perche' il toggle non passa dal widget. ⏳ CP 46.3–46.5 · ✅ i cinque `WBP_RT_*` sono consegnati (`U24`, 2026-08-18) |
| **E47** Mini v0.1 Autobattle: la partita che si guarda | 🟡 **E47.3 chiuso ([#956](https://github.com/DegrassiAaron/refactor-tactics-main/issues/956))** | 21 test `Match.Autobattle.*`, misurati il 2026-08-27. Non porta meccaniche nuove, porta il **modo di osservarle** ([D-145](../decisions/RT_PDR_00_Decision_Log.md)) |
...
| **E7** | Equipaggiamento e loadout | P2 | 4 | Scelta orizzontale (ogni variante ha uno svantaggio). ✅ **Chiusa il 2026-09-02**: `#60`, `#61`, `#62` e `#63` tutti chiusi, e il gate — *«ogni variante arma ha almeno uno svantaggio verificato da test; nessuna progressione in partita»* — è soddisfatto da `Equipment.WeaponVariantHasTradeoff`, `Catalog.ValidatorRejectsEquipmentWithoutDrawback` e `Equipment.NoInMatchProgression`. ⚠️ La colonna diceva **5** checkpoint: E7 ne ha **quattro** (`7.1`–`7.4`) |
...
| **E10** | Obiettivi dinamici e fine partita | P2 | 3 | Chiude il loop: la partita ha un motivo per muoversi. ✅ **Chiusa il 2026-09-02**: `#74`, `#75` e `#76` tutti chiusi, e il gate — *«un test per ciascuna via»* — è soddisfatto da `Match.EndsOnElimination`, `Match.EndsOnObjective` e `Match.EndsOnTurnLimit`, verificati per nome |
...
| **E19** | Classe di mappa e composizione | P2 | 3 | `URTMatchFormatData` **esiste già**, e i **tre** buchi misurati sono chiusi: la mappa dichiara la propria `MapClass` (`#215`, 2026-08-09), il formato dichiara `UnitsPerTeam` (`#216`, stesso giorno) e `UnitsPerPlayer` con il gruppo di controllo che ne consuma il valore (`#1124`, 2026-09-02) — tutti con consumatori runtime. Dal 2026-08-10 `Format.Skirmish2v2` è **spedito da C++** (`#375`): il formato canonico non dipende da un `.uasset` che il repository non contiene. ✅ **Epic chiusa il 2026-09-02** · 🔴 *la riga dichiarava **2** checkpoint e rimandava lo stato autorevole a `feature-registry.yaml`, **uscito dal repository** con [D-181](../decisions/RT_PDR_00_Decision_Log.md) il 2026-08-21: rimandava a una fonte morta, e il terzo checkpoint esisteva dal 2026-08-17* |
...
> l'istruttoria. ➕ **E sono stati IMPLEMENTATI lo stesso giorno**: il prerequisito
> ([#2258](https://github.com/DegrassiAaron/refactor-tactics-main/pull/2258), `ERTMoveOutcome::Slid`) e la
> Fase 1 ([#2253](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2253), perimetro in
> [D-323](../decisions/RT_PDR_00_Decision_Log.md)). La **Fase 2** — il framework che fa leggere al bot ogni
> status — resta dietro `STA-4`. ⛔ **Il resto dell'elenco non è toccato**, e in particolare lo **slide a catena** resta
...
| **2.4** ✅ | Combat su hex | Attacchi e forme (Single/Area/Line/Cone via `HexLine`/`HexCone`), LOS via `URTHexVisionLibrary`, energia/ultimate, status Root/Slow su `FRTCellId` — ⚠️ **`Reveal` era elencato qui e non e' concedibile**: il tag e' dichiarato e letto, e nessuna azione del catalogo lo applica, quindi il ramo «avversario rivelato» non lo attraversa nessuna partita. In v0.1 resta cosi' per scelta — e' l'unica deroga all'invariante #6 e si progetta, non si aggiunge per far passare una verifica ([#1635](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1635)); resolver «raccogli poi applica» ordine-indipendente | Test per forma + permutazione dell'input; `PIE-HEXPLAY-6` |
...
| **4.2** ✅ | Budget 5 MP e micro-step | Budget 5 MP; costo 1 cella normale, 2 difficile, 2 salita via rampa; Sprint 8 MP + `Status.Exposed`; il percorso **non** viene ricalcolato globalmente durante la resolution | `HexMove.BudgetCostsInMatch`, `HexSim.ReachableRespectsTerrainCost`, `Actions.Sprint.AppliesExposed`, `Actions.Move.NoGlobalRecompute` *(il DoD diceva `Actions.Move.BudgetCosts`, che **non è mai esistito** con quel nome: la proprietà è coperta dai primi due — corretto il 2026-08-27)* |
...
| **4.4** ✅ | Azioni fondamentali | `Wait`, `Move`, `BasicAttack`, `Guard`, `Activate`, `Interact` con fase/priorità del catalogo; `Guard` riduce di 15 **ogni** danno diretto dell'arco frontale e resiste a una spinta di 1, scade nel Cleanup *(era «il primo danno diretto» fino al 2026-09-20: [D-408](../decisions/RT_PDR_00_Decision_Log.md) ritira il pool di D-292 e riporta la Guardia a una riduzione per colpo, col valore dichiarato dal personaggio)* | Un test per azione + `Actions.Guard.ReducesEveryFrontalHit` *(era `Actions.Guard.FirstHitOnly`, rinominato dalla stessa voce)* |
...
✅ **Epic chiusa il 2026-09-02** — `#21`. I quattro checkpoint sono chiusi e il gate — *«ogni variante arma ha almeno uno svantaggio verificato da test; nessuna progressione in partita»* — è soddisfatto per nome: `Equipment.WeaponVariantHasTradeoff`, `Catalog.ValidatorRejectsEquipmentWithoutDrawback`, `Equipment.NoInMatchProgression`.

⚠️ **`CP 7.2` chiude su cinque gadget degli otto, e questa riga lo diceva già** — la issue `#61` no. Il DoD è stato riscritto il 2026-09-02 sullo scope esprimibile: gli assenti non «mancano», sono assenti per ragioni diverse, fissate da `Equipment.Gadget.ThreeAreNotExpressibleYet`, e escono **uno per issue** perché hanno costi asimmetrici. ✅ **`#2087` (`SmokeEmitter`) è uscito il 2026-09-02**: il suo blocco costava un'azione core, e `Action.CreateSmoke` ora esiste. Restano `#2088` `Anchor` (un consumo per turno che non esiste), `#2089` `Insulator` ed `#2090` `Sensor`, che costano **epic non ancora costruite** — E36 ed E13.

🔴 **Qui il documento era avanti e la issue indietro** — l'inverso del difetto che `#14` ha registrato il 2026-08-12. La riga `7.2` portava ✅ e «4 degli 8» da quando il catalogo ha preso quella forma — oggi «5 degli 8», dopo `#2087`; la issue ha continuato a chiederne otto. Nessun gate confronta i due versi.
...
| **7.2** ✅ | Gadget — **5 degli 8** | `Medkit` (cura 18), `BreachCharge` (35 alla struttura e **zero** alle unità), `Sprinkler` (acqua r1, raggio ereditato da `Action.CreateWater`), `SmokeEmitter` (fumo r1, ereditato da `Action.CreateSmoke` — `#2087`), `PortableCover`; cooldown **3** per tutti, che sostituisce quello dell'azione core. ⚠️ I tre assenti mancano per **tre ragioni diverse**, e la differenza porta a lavori diversi: `Insulator` — l'immunità per categoria è E36; `Sensor` — la conoscenza parziale è E13, e i suoi numeri non sono nella fonte; `Anchor` — `PushResistance` è una **soglia** permanente, non un consumo per turno, e alzarla reintrodurrebbe l'immunità che `D-075` ha appena tolto | `Equipment.Gadget.CooldownEnforced`, `Equipment.Gadget.NumbersMatchCatalog`, `Equipment.Gadget.ThreeAreNotExpressibleYet`, `Equipment.Gadget.SmokeEmitterRaisesTheSameSmokeAsMistVeil` |
| **7.3** ✅ | Moduli reazione — **catalogo** (3 dei 7) | `CounterShot` (14), `ReactiveShield` (15), `AllyIntercept`: costruiti su un'azione core che è **già** una reazione, da cui ereditano fase, priorità e `ReactionTrigger`, con numeri propri via `GrantedEffects`. Una attivazione per turno la garantisce il percorso E5, non un cooldown dell'oggetto. ⚠️ `EmergencyDash` è sceso a **7.5 implementando**, per un vincolo su un asse diverso dal trigger: il suo trigger esiste, ma `Reposition 1` sposta *chi reagisce* e nessun `ERTActionEffect` lo esprime — `Push`/`Pull` spostano il bersaglio | `Equipment.ReactionModule.SingleActivation`, `Equipment.CounterShotUsesExistingTrigger`, `Equipment.EmergencyDashMovesTheReactorInPlay`, `Equipment.EmergencyDashRepositionsTheReactor` *(il DoD nominava `Equipment.EmergencyDashIsNotExpressibleYet`, il pin del limite dichiarato: **CP 7.5 ha tolto il limite**, quindi il pin è stato sostituito dai due test del comportamento vero — allineato il 2026-08-27)* |
...
| **8.5** ✅ *(chiuso 2026-08-07)* | Azioni ambientali e di supporto | `Heal` (20, non supera il massimo, non rimuove stati), `CreateWater` (r1, 2 turni), `Ignite` (2 turni), `Electrify`, `CreateCover` (30 integrità, 2 turni, non sovrapponibile), `ModifyArc` | Un test per azione + `Actions.CreateCover.RejectsOutOfRangeAndOccupied` *(il DoD nominava `Actions.CreateCover.NoOverlap`, che non esiste: la non sovrapponibilità la pinna il secondo caso di questo test — «il bordo già riparato non ne accetta un secondo» — corretto il 2026-08-27)* |
...
✅ **Epic chiusa il 2026-09-02** — `#24`. I tre checkpoint sono chiusi (`#74`, `#75`, `#76`) e il gate — *«la partita termina per eliminazione, obiettivo **e** limite di turni, con un test per ciascuna via»* — è soddisfatto per nome: `Match.EndsOnElimination`, `Match.EndsOnObjective`, `Match.EndsOnTurnLimit`.

⚠️ **Le spunte di `10.1` e `10.2` mancavano qui pur essendo chiuse**: è lo stesso difetto di direzione che `#14` ha registrato il 2026-08-12 — chi chiude un checkpoint aggiorna la issue, e la tabella resta indietro **in silenzio**, perché nessun gate la confronta con GitHub.
...
| **10.2** ✅ | Obiettivo contestabile | Un obiettivo può essere contestato (anche con `Wait`); la verifica avviene nel **Cleanup**; contestazione paritaria = nessun progresso; il progresso è un intero e compare **nell'HUD e nel TurnLog** | `Objectives.ContestedNoProgress`, `Objectives.CheckedInCleanup`, `Objectives.ProgressIsInteger`, `Objectives.SilentWithoutObjectiveCell`, `Objectives.AuthoredArenaDeclaresItsObjective`, `Objectives.ObjectiveChangesMapHash`, `HudViewModel.ObjectiveProgressReachesTheHud` (*la riga ne elencava **due** su sette: rimisurata sui test il 2026-09-02, quando `#75` ha portato la metà mancante della DoD — il punteggio esisteva ma nessun widget lo leggeva*) |
...
| **11.8** *(nuovo 2026-08-12)* | **Pointer Interaction Contract**: Hover · LMB · RMB | La matrice `oggetto sotto il puntatore × contesto × input → esito` è dichiarata in [`spec-pointer-interaction.md`](../technical/systems/spec-pointer-interaction.md) e ogni combinazione produce uno degli **otto** esiti di un elenco chiuso (`NoOp`, `Inspect`, `Select`, `Preview`, `Confirm`, `Cancel`, `OpenContext`, `Blocked(reason)`). Il `PlayerController` acquisisce un **contesto esplicito** (`IdleSelection · Planning · Pathing · Targeting · ResolutionPlayback · ReactionWindow · Modal`) al posto della cascata di `if` sul tipo di Actor colpito; il resolver di hit restituisce un **target logico** (`FRTCellId` · `UnitId` · StableObjectId), mai una decisione di gameplay. Precedenza `Modal/Reaction UI > HUD > world tactical hit` esplicita e coperta da test. Ogni rifiuto porta un **reason code**: `Blocked` silenzioso è un difetto. *(Esteso il 2026-08-13)* Esiste uno stato **neutro** — nessuna abilità armata all'ingresso in Planning — e il click su un nemico **ispeziona** invece di pianificare ([D-128](../decisions/RT_PDR_00_Decision_Log.md)); `Targeting` porta un `TargetKind` (`Unit`/`Cell`/`Edge`/`Object`) dichiarato **dall'azione**, non scelto a parte; la precedenza **intra-mondo** è esplicita — in `Pathing` la cella vince sulla mesh di porta/ponte/hazard, in `Targeting`/`Cell` vince sull'unità che la occupa, e un ghost non è mai bersaglio; `RMB` segue un **ordine totale** di Back e non deseleziona mai l'unità; un elemento con più verbi legali produce `OpenContext`, non un `Confirm` implicito | **Esistono** (8): `PlayerInput.NeutralEnemyClickDoesNotPlan`, `PlayerInput.PathingCellWinsOverDoorMesh`, `PlayerInput.TargetCellIgnoresOccupyingUnit`, `PlayerInput.GhostIsNeverAGameplayTarget`, `PlayerInput.RightClickBackFollowsTotalOrder`, `PlayerInput.RightClickNeverDeselects`, ➕ `PlayerInput.HoverNeverCommits` e `PlayerInput.RightClickCancelsPreviewOnly` ([#1766](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1766), 2026-09-01: gli unici due che si posassero su comportamento **gia' esistente**, entrambi passati per la verifica di mutazione). ⛔ **Dichiarati e NON scritti** (8): `HUDConsumesPointerBeforeWorld`, `HiddenEnemyCannotBecomeHoverTarget`, `AllyGhostIsReadOnly`, `PlaybackRejectsPlanningInput`, `ReactionWindowOwnsInputPriority`, `LogicalMapObjectResolvedFromStableId`, `ArmedAbilityThenEnemyClickPlans`, `MultiVerbElementOpensContext` — misurato il 2026-08-29 su `c2f694dc`, e si riverifica con `git grep -l 'RefactorTactics.PlayerInput.<nome>' -- Source/` per ciascuno. ⚠️ `ArmedAbilityThenEnemyClickPlans` non e' un difetto di questa riga ma di questo elenco: [`spec-pointer-interaction.md`](../technical/systems/spec-pointer-interaction.md) dichiara che **non esiste come test a se'** e vive come seconda meta' di `NeutralEnemyClickDoesNotPlan`; `MultiVerbElementOpensContext` la stessa spec lo elenca fra quelli da scrivere. Una colonna «evidenza» che nomina cio' che non esiste non e' un elenco incompleto: e' una prova che non c'e'. `PIE-V01-POINTER` resta registrata e **non ancora eseguita** |
...
implementa il nascondimento. ⚠️ **E [D-227] lo ha completato il 2026-08-28**: gli stati sono **tre**, non due — il terreno già esplorato resta **noto e velato** invece di richiudersi, quindi la mappa statica non è né «nota» come diceva la riga originale né dimenticata a ogni turno.
...
| **13.6** ⏳ | La conoscenza parziale diventa **visibile**: la porta filtrata e i due leak | Il client ha già il dato — `MakeCurrentSnapshot` è pubblica e consegna `TeamKnowledge` — e manca la **porta filtrata**. Due canali stampano ciò che la squadra non sa: `ARTHUD::DrawHUD` itera tutte le unità vive filtrando solo `IsAlive()`, quindi disegna nome, HP e scudo **dei nemici**; il combat log scrive la cella esatta di ogni movimento. ⚠️ **Metà di questo checkpoint è un difetto, non una feature**: i due leak non erano nel DoD di nessuno | [#1466](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1466) |
| **13.7** ✅ | Il velo: tre stati, e il terreno esplorato che resta noto | ✅ **Chiuso il 2026-08-28** con [#1528](https://github.com/DegrassiAaron/refactor-tactics-main/pull/1528). La decisione di scope che il checkpoint apriva è **[D-227]**: il velo ha **tre** stati — osservato ora / esplorato e ricordato / mai visto e **non disegnato** ([D-225]) — perché con due la mappa **si richiuderebbe alle spalle del giocatore**, due volte per turno. `FRTTeamKnowledge` acquista `ExploredCells`, che **non scade**: un contatto invecchia perché l'unità si muove, il terreno no. `CurrentVersion` 1 → 2. ⚠️ **`M_HexCell.uasset` non è stato toccato**: il velo moltiplica l'RGB lineare e `NumCustomDataFloats` resta **3** | `Perception.KnowledgeRemembersExploredCells`, `Veil.CoversExactlyUnobservedCells`, `Veil.CoversEveryInstanceFamily`, `Veil.FollowsRefreshPoints`, `Veil.FullScanCostIsMeasured`, `Hex.VeiledSurfaceColorsAreDistinguishable`; PIE `PIE-HEX-VIZ-VELO` ⏳ |
| **13.8** ⏳ | Il velo si **aggancia** alla presentazione, e con esso si decide di chi è la vista | 🔴 **Il velo esiste e non si vede**: misurato il 2026-08-28 su `9018b5c3`, `ApplyKnowledgeVeil` non ha chiamanti fuori dai test e `OnTeamKnowledgeRefreshed` ha **due** `Broadcast` e **zero** subscriber. ⚠️ **E non è cablaggio**: agganciarlo chiede di decidere **chi** sia il viewer, che [D-227] dichiara esplicitamente non deciso. C'è già un precedente da non ripetere — `ARTHUD` passa `/*PlayerTeamId=*/ 0` **cablato** (`UI/RTHUD.cpp:391`) mentre `ARTPlayerController::PlayerTeamId` esiste: due risposte alla stessa domanda, una non dichiarata. Chiude anche il residuo di 13.7, perché finché non c'è un chiamante **nessuno ha guardato il velo a schermo** | [#1535](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1535); PIE `PIE-HEX-VIZ-VELO` deve passare da ⏳ a **eseguita** |

> ⚠️ **Le righe 13.6 e 13.7 entrano qui il 2026-08-28, e su GitHub esistevano dal 2026-08-27** — [#1466](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1466) e [#1467](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1467), create a **un minuto** di distanza. Questa
> tabella si fermava a **13.5** mentre la prosa dell'obiettivo qui sopra nominava già «il checkpoint E13.7»:
> un documento che cita un checkpoint che la propria tabella non contiene. Non è un'aggiunta di lavoro nuovo
> — è la tabella che si allinea a ciò che era già tracciato. 🔴 **Ed è il motivo per cui 13.8 non entra da
> sola**: infilarla dopo 13.5 avrebbe lasciato il buco al centro e reso la lacuna più difficile da vedere,
> non meno.
...
| **S2-1** | Lo scenario **riferisce** la fixture per nome | S1 | `Scenario.FixtureFactoryResolvesKnownNames` · `.UnknownFixtureIsError` · `.BlockedTurnIsNotAFailure` · `.ShowcaseRelayV01PlaysEveryTurn` | ✅ **fatto 2026-08-08**: `RT_Showcase_Relay_v01` esiste, il turno 1 gira dal resolver normale, `result.json` dice `BLOCKED` col nome della capability. ⏱️ **Il test è stato rinominato il 2026-09-03** (`#170`): si chiamava `ShowcaseRelayV01RunsTurnOne` e ne pinnava sette; ora li gioca tutti e otto e il nome porta la proprietà invece del numero |
...
✅ **Epic chiusa il 2026-09-02** — `#214`. Il criterio è quello che l'epic stessa ha fissato il 2026-08-31: *«E19 si chiude quando si chiude `#1124`, non prima, e il criterio sono le **issue figlie**»* — ciò che resta dopo che [D-181](../decisions/RT_PDR_00_Decision_Log.md) ha ritirato `feature-registry.yaml`, la fonte *«derivata dai gate»* a cui la clausola precedente rimandava. `#215`, `#216` e `#1124` sono tutti chiusi.
...
| **19.3** ✅ *(nuovo 2026-08-17, chiuso il 2026-09-02)* | Il formato dichiara **quanti Hero controlla un Player** | Il terzo buco della stessa forma, un piano più sopra: `UnitsPerTeam` dice quante unità schiera una squadra, **nessun campo** dice quante ne comanda una persona — e nessuno le chiede, perché il `PlayerController` e la Decision Window assumono `1 : 1`. ⚠️ In v0.1 i due numeri valgono **entrambi 2** e leggere quello sbagliato passa ogni test esistente. Il **resolver resta invariante** (id stabili, ordinamento deterministico: non sa quante persone ci sono dietro) e il **Planning resta una finestra per persona**. Nessun `Min…`: l'estremo inferiore è `1` per costruzione. 🔴 **Qui c'era scritto «in v0.1 l'intervallo ha cardinalità uno», ed è la clausola che [D-155](../decisions/RT_PDR_00_Decision_Log.md) registra come **corretta in code review prima del merge**: la v0.1 è già il caso multi-Hero — un umano comanda **due** unità in `Format.Skirmish2v2`. La correzione aveva raggiunto D-155 e l'owner §16.4 e non questa riga, che è rimasta a contraddirli per un mese (rimisurato il 2026-09-23). Owner: [`../gameplay/spec-durata-partita-e-scala-mappe.md`](../gameplay/spec-durata-partita-e-scala-mappe.md) §16.4, decisione [D-155](../decisions/RT_PDR_00_Decision_Log.md) | `MatchFormat.DeclaresUnitsPerPlayer` (*la riga citava `MatchFormat.DeclaresControlledHeroesPerPlayer`, nome mai atterrato: rimisurato sui test il 2026-09-02*) · `MatchFormat.ControlCountIsNotUnitsPerTeam` (i due campi divergono in un formato di prova) · `MatchFormat.ResolverIsInvariantToControlCount` (due partite identiche, conteggio diverso, **stesso `LogHash`** — la forma di `MapClass.NotBranchedInSimulation`) · `Player.SeatCountFollowsUnitsPerPlayer` · `Combat.ControlGroupPartitionsTheTeam` (*aggiunto con `#1124` il 2026-09-02: il campo aveva per lettore il solo validator, e la regola di autorizzazione era stata rimossa in code review perché senza chiamante — rientra ora che `#937` ha rilasciato `ARTGameMode`*) |
...
>
> 🔁 **Le due frasi qui sopra valgono come misura del 2026-08-17 e non sono più vere oggi.** Restano scritte —
> sono la ragione per cui E19 fu riaperta, e cancellarle toglierebbe la motivazione invece dell'errore. Ma il
> `git grep` citato **non dichiarava lo scope**, ed è esattamente l'omissione che lo ha fatto invecchiare in
> silenzio: oggi `ControlGroup` risponde su undici file di produzione
> (`git grep -l ControlGroup -- Source/ ':!Source/RefactorTactics/Tests/'`), mentre `ControlledHero` e
> `ControlledUnits` rispondono ancora **zero**. Il soggetto **lo ha portato CP 19.3 stessa**, chiusa il
> 2026-09-02; a CP 14.8 resta di farlo arrivare dentro `ARTTurnManager::AskReactionDecision`, che è ancora la
> firma che questa nota cita. Rimisurato il 2026-09-23.
>
...
| CP | Obiettivo | DoD misurabile | Test / verifica |
|---|---|---|---|
| **46.1** | Frontend root e **navigation controller** | Un solo owner del flow espone `PushScreen`, `PopScreen`, `ShowModal`, `CloseModal`, `ReturnMain`; nessun widget chiama `CreateWidget`/`AddToViewport`/`RemoveFromParent` di propria iniziativa — verificabile con `grep -rn "AddToViewport\|RemoveFromParent" Source/` che non deve produrre occorrenze fuori dal controller. Back stack esplicito: da ogni schermata `Back` risale a quella che l'ha spinta, e la radice non ha `Back`. ⚠️ **Non possiede il `Modal` del `PlayerController`**: quello è di CP 11.8, che **dichiara** sette contesti e la precedenza `Modal/Reaction UI > HUD > world` — dichiarata e non ancora implementata (delta (c) di quel checkpoint) — sono due strati, e il frontend non ne eredita l'autorità | ⏳ `PIE-V01-FRONTEND-NAV` — scritta il 2026-08-24 ([#1242](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1242)), ⏳ non ancora eseguita |
| **46.2** | **Loading**, **error modal** e **banner di ripiego** | Il loading legge un **evento** `ERTLoadPhase { Map · Scenario · Bots }` emesso da `BeginPlay`, non tre stringhe scelte a mano; **nessuna percentuale**, perché non esiste un progress model. L'error modal porta *causa leggibile* + `Back`; `Details`/`Copy debug info` solo in Development (`#if !UE_BUILD_SHIPPING`). `Back` è `PopScreen` prima dell'avvio e **`ReturnMain` a partita già avviata** — `PopScreen` lascerebbe una partita viva sotto il menu (vietato da CP 46.6). ➕ **Banner di ripiego**: il gioco quasi non fallisce, **ripiega** — 21 warning contro 8 errori in `RTGameMode.cpp`, e uno dei casi principali è **la prima riserva di `G13`** (l'arena di test). Un modale non lo intercetta; il banner riusa la forma di `GetScenarioBannerText`. 🔴 *Questa riga diceva «le due riserve»: falso, corretto dall'implementazione — la seconda è la soglia obiettivo a 0, cioè un valore del formato, non il formato di ripiego.* ⚠️ **Il checkpoint tocca `RTGameMode`**: era nel `writable` di `autobattle`, riallocato a `frontend_shell` il 2026-08-16 dopo la chiusura di `#954` (D-139). Esteso il 2026-08-16 da [`plans/cp462-loading-error-spec-panel-2026-08-16.md`](plans/cp462-loading-error-spec-panel-2026-08-16.md) — **nessun gate della v0.1 lo richiedeva**, è scelta di prodotto come tutta E46 | ⏳ `PIE-V01-FRONTEND-ERROR` — scritta il 2026-08-24 ([#1242](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1242)), ⏳ non ancora eseguita · `Frontend.LoadPhase*` da scrivere |
| **46.3** | **Main Menu** | `PLAY · SETTINGS · QUIT` a schermo, navigabili da mouse **e** tastiera, con focus visibile che non dipende dal solo colore. `SETTINGS` in v0.1 può aprire un pannello vuoto dichiarato *coming soon*: la voce esiste perché il back stack la attraversi, il suo contenuto è v0.2. Version/build label leggibile. Avvio del gioco packaged sul Main Menu, non su una mappa | ⏳ `PIE-V01-FRONTEND-MAIN` — scritta il 2026-08-24 ([#1242](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1242)), ⏳ non ancora eseguita: chiede il **packaged**, non il PIE · ✅ i tre pulsanti sono cablati e il `.uasset` esiste; `PLAY → StartMatch` è del 2026-08-29 ([#938](https://github.com/DegrassiAaron/refactor-tactics-main/issues/938)) ed è difeso da `RefactorTactics.Frontend.MainMenuGraphAsksTheNavigatorToStart`, che legge il grafo dentro il binario invece di fidarsi di una sessione |
| **46.4** | **Play → partita esistente** | `PLAY` avvia il vertical slice 2v2 **senza** aggiungere un secondo percorso di avvio: usa `ARTGameMode` e il formato `Format.Skirmish2v2` **spedito da C++** dal commit `9f44570d` (*«spedito col gioco, non un asset da creare»*), non una configurazione parallela. ⚠️ Una stesura precedente attribuiva quel lavoro a `#375`: è una PR sul **determinismo del checksum**, e il riferimento risolveva perché GitHub redirige `issues/375` → `pull/375` — un link sano con l'etichetta falsa, che `check-docs-links.py` non può vedere. Match Setup v0.1 è **preconfigurato** — non c'è nulla da scegliere finché esiste un solo formato e una sola mappa. La transizione passa da 46.2 | ⏳ `PIE-V01-FRONTEND-PLAY` — scritta il 2026-08-24 ([#1242](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1242)), ⏳ non ancora eseguita |
| **46.5** | **Result** → Main Menu / Play Again | A fine partita compare esito, vincitore e numero di round, **letti dal risultato canonico**: la UI non ricalcola nulla e non decide chi ha vinto — la condizione di fine è di E10 e il TurnLog ne è il registro. `Play Again` ripercorre 46.4; `Main Menu` torna alla radice svuotando il back stack. ⚠️ Il C++ è in `main` — navigazione, lettura del verdetto e apertura a fine partita — e manca `WBP_RT_ResultScreen`, che è la seduta d'editor **U29**: finché non c'è, la schermata si apre e a schermo non compare nulla | ⏳ `PIE-V01-FRONTEND-RESULT` — scritta il 2026-08-24 ([#1242](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1242)), ⏳ non ancora eseguita |
| **46.6** | **Pause** | `ESC` in partita apre `RESUME · SETTINGS · RETURN TO MAIN MENU`. `Return to Main Menu` smonta la partita e torna alla radice senza lasciare stato vivo — verificabile riavviando una partita subito dopo e ottenendo lo stesso esito. ⚠️ **La pausa è offline-only per costruzione**: in multiplayer non esisterà una pausa globale ([v0.5, E40](roadmap-post-v0.1.md)), quindi il pulsante non entra in un contratto condiviso col futuro codice di rete — reso misurabile come *«la pausa non tocca la simulazione»*, e non come una promessa. 🔴 **Il checkpoint ha chiuso un buco che scavalcava all'indietro**: nessun percorso del codebase riapriva il livello del frontend, quindi `ReturnMain()` lasciava il menu **sopra una partita viva** — anche il `MAIN MENU` di CP 46.5, il cui DoD era vero dello stack e falso del mondo — e `PLAY AGAIN` dal Result annunciava a zero ascoltatori, perché `OnMatchRequested` lo raccoglieva solo il GameMode del menu. ⚠️ *Questa riga diceva «a parità di seed»: la clausola è **inerte**, perché il progetto non ha alcun RNG e `RefactorTactics.Simulation.SeedIsDeclaredAndUnconsumed` lo prova. L'oracolo vero è la sequenza degli hash del TurnLog su una partita abbandonata **a metà**, con controprova.* | ⏳ `PIE-V01-FRONTEND-PAUSE` — scritta il 2026-08-24 ([#1242](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1242)), ⏳ non ancora eseguita · ✅ **14 test** in `RTFrontendPauseTests.cpp` (il namespace `RefactorTactics.Frontend.*` ne conta **79** in tutto, misurate il 2026-08-29 con `grep -rhoE '"RefactorTactics\.Frontend\.[A-Za-z0-9_]+"' Source/ \| sort -u \| wc -l` — ⚠️ *questa riga diceva **74**, e prima ancora «14 test `RefactorTactics.Frontend.*`» nominando il namespace col conteggio del file: un numero in prosa invecchia a ogni test aggiunto, e nessun gate lo rilegge*). 🔴 **Quattro sono nati dalla code review**, che ha trovato tre difetti reali nella prima stesura: `RESUME` presentava il **Main Menu sopra la partita viva**, `ESC` su una partita avviata senza passare dal menu inchiodava la pausa in una radice da cui non si esce, e la pausa **non toglieva l'input** — Spazio risolveva il turno dietro la schermata. La correzione e' `RTScreenIds::Match`, cioe' dire al flow che la partita e' uno stato e non l'assenza di uno |
...
>
> ✅ **Le sei voci `PIE-V01-FRONTEND-*` esistono dal 2026-08-24**
> ([#1242](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1242)): una per checkpoint, in
> [`test-manuali-pie.md`](../technical/test-manuali-pie.md), misurate `grep -c "PIE-V01-FRONTEND"` → **6**.
> Fino al 2026-08-20 quel registro era nel `writable` della track `playtest` — *«l'autore davanti a
> Unreal»* — e `D-139` ne faceva uno **stop**;
> [D-178](../decisions/RT_PDR_00_Decision_Log.md) ha rimosso il vincolo, e restava solo il lavoro.
> ⚠️ **Scritte ≠ eseguite, e i DoD qui sopra vanno letti così**: nascono tutte e sei ⏳, nessuna eseguibile
> oggi — quattro aspettano un `.uasset` (U28, U29, U30) e due l'aggancio di `InitializeFrontend`, che è
> CP 46.3. Quello che si chiude è il **debito documentale** dichiarato qui sotto: i DoD di E46 non nominano
> più verifiche che il registro non contiene. La verifica resta tutta da fare.
> ⚠️ **`-ERROR` e `-PLAY` non stanno in nessuna seduta**: diventano eseguibili con l'aggancio ma non sono
> prodotte da un lavoro d'editor, e la regola *«una voce che non sta in una seduta non viene eseguita mai»*
> resta scoperta per quelle due — dichiarato in `U28` e nella spec owner §8.
...
> qui perché non si scopra al momento di chiudere un checkpoint.»* — **chiuso il 2026-08-24**: il registro
> le contiene. Resta il debito **gemello**, che non è lo stesso: le voci esistono e non sono eseguite.
...
> [`D-289`](../decisions/RT_PDR_00_Decision_Log.md). Il testo dei due checkpoint **non è cambiato**: diceva
> già ciò che il Decision Record chiede, e questa è la ragione per cui non è nata un'epic nuova.
>
> | # | Cosa chiude | CP |
> |---|---|---|
> | [#1826](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1826) | 🔴 **Il blocco**: `MSE-4` al centro — un muro passante accende dodici settori su dodici | prerequisito di `E23.6` |
> | [#1827](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1827) | Posa deterministica dal footprint | **`E23.6`** |
> | [#1828](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1828) | Traversata intra-cella `SideA ↔ SideB` | **`E23.7`** |
> | [#1829](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1829) | Intento di movimento con `CoverOption`/`CoverSide` | nuovo, gated su `COV-*` |
> | [#1830](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1830) | LOS e proiettili sulla geometria intra-cella — `D-269`/`D-270`, **accettate e senza owner** | `E23.1` |
> | [#1831](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1831) | L'editor mostra settori, sorgenti, opzioni, lati e la ragione dell'invalido | `E23.5` |
> | [#1832](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1832) | Validator: sei regole di topologia che oggi passano in silenzio | `E23.6`/`E23.7` |
>
