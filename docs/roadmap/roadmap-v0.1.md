...
| **Obiettivi dinamici** | `Map/RTHexCellData.h`, `Turn/RTTurnRules.cpp` | 🟡 la partita **finisce** per obiettivo (`Match.EndsOnObjective`), la cella contendibile **esiste in mappa** — `DA_HexMap_Arena` la porta a `(0,-3,L0)`, e il bot la gioca da CP 26.5 — e l'oggetto da attivare c'è: `Action.Interact` apre una porta, coperto dal kit al TurnLog da `Structures.Door.InteractFromKitOpensDoor` e `Spec/Map/InteractOpensDoor.json` · 🔴 *diceva «**nessun oggetto da attivare** in mappa»; §2.1 lo ha rimisurato il 2026-09-02 dichiarandolo «falso dal 2026-08», e questa copia era rimasta indietro — «attivare» è `Activate`, ritirata da [D-014](../decisions/RT_PDR_00_Decision_Log.md) e assorbita da `Interact` con la chiusura di CP 10.1* · ⚠️ resta 🟡 per due ragioni: con **una** cella la contesa è irraggiungibile per costruzione (CP 31.1, [#1583](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1583)), e che la mappa d'autore contenga una porta è letto dalla **name table** del `.uasset` — `RTHexDoor` c'è, `InteriorWalls` no, quindi l'assenza discrimina — **non da un oracolo**: nessun test lo asserisce |
...
| **E10** Obiettivi dinamici e fine partita | ✅ **chiusa 2026-09-02** | 39 test `Match.*` + 6 `Objectives.*` — fine partita a tre vie **con un test per via** (`EndsOnElimination`, `EndsOnObjective`, `EndsOnTurnLimit`), obiettivo contestabile verificato nel **Cleanup** dopo effetti ambientali e KO, progresso **intero** nell'HUD e nel TurnLog · 🔴 *la riga diceva «⏳ nessun oggetto da attivare in mappa» e contava **27** test: il primo è falso dal 2026-08 (`#74` chiuse `Activate`/`Interact`, `#833` ha portato porte e interruttori), il secondo era fermo. Rimisurata il 2026-09-02* |
