...
| **Obiettivi dinamici** | `Map/RTHexCellData.h`, `Turn/RTTurnRules.cpp` | 🟡 la partita **finisce** per obiettivo (`Match.EndsOnObjective`) e la **cella contendibile esiste nella mappa spedita**: `DA_HexMap_Arena` la porta a `(0,-3,L0)`, una sola e calpestabile — asserito da `Objectives.AuthoredArenaDeclaresItsObjective` — e il bot la gioca da CP 26.5 · 🔴 **ma di oggetti da ATTIVARE la mappa spedita non ne ha nessuno**: misurato il 2026-09-04, `DA_HexMap_Arena` ha **64 celle e 0 porte**, e a dirlo è `HexMap.AuthoredArenaDoorCountMatchesTheRoadmap`. La *capability* è chiusa — `Action.Interact` apre una porta (CP 10.1, [#74](https://github.com/DegrassiAaron/refactor-tactics-main/issues/74)) — ma i suoi scenari girano su mappe che si costruiscono da sole, e **nessuno dei tre asset mappa versionati** ne porta una — `Arena` 64 celle, `Sandbox` 0, `_Scratch` 65, e zero porte in tutti e tre. Le porte esistono solo nelle fixture generate a runtime (`RelayBasin` una, `GrayKitYard` quattro) · ⚠️ resta 🟡 anche perché con **una** cella la contesa è irraggiungibile per costruzione (CP 31.1, [#1583](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1583)) |
...
| **E10** Obiettivi dinamici e fine partita | ✅ **chiusa 2026-09-02** | 39 test `Match.*` + 6 `Objectives.*` — fine partita a tre vie **con un test per via** (`EndsOnElimination`, `EndsOnObjective`, `EndsOnTurnLimit`), obiettivo contestabile verificato nel **Cleanup** dopo effetti ambientali e KO, progresso **intero** nell'HUD e nel TurnLog · 🔴 *la riga diceva «⏳ nessun oggetto da attivare in mappa» e contava **27** test: il primo è falso dal 2026-08 (`#74` chiuse `Activate`/`Interact`, `#833` ha portato porte e interruttori), il secondo era fermo. Rimisurata il 2026-09-02* · 🔵 **e quella rimisura era imprecisa su metà, corretta il 2026-09-04**: «#833 ha portato porte e interruttori» è vero per il **codice** e falso per la **mappa spedita** — `DA_HexMap_Arena` ha 64 celle e **0 porte**, misurato da `HexMap.AuthoredArenaDoorCountMatchesTheRoadmap`. Capability e contenuto sono due proposizioni diverse, e §2 ora le tiene separate |
