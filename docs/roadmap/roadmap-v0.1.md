...
dei checkpoint, e salta la vista di stato. Un avvertimento in prosa non l'ha impedito perché non c'era; al suo
posto c'è un **comando**, nella stessa convenzione dell'evidenza qui sopra. Output vuoto = completa:

```sh
diff <(grep -oE '^\| \*\*E[0-9]+\*\*' docs/roadmap/roadmap-v0.1.md | grep -oE 'E[0-9]+' | sort -uV) <(grep -oE '^### E[0-9]+' docs/roadmap/roadmap-v0.1.md | grep -oE 'E[0-9]+' | sort -uV)
```

Verificato per mutazione il 2026-08-27: cancellando la riga di `E23` il comando esce `1` e **la nomina**.
