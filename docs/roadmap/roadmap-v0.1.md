...
| **E10** Obiettivi dinamici e fine partita | ✅ **chiusa 2026-09-02** | 39 test `Match.*` + 6 `Objectives.*` — fine partita a tre vie **con un test per via** (`EndsOnElimination`, `EndsOnObjective`, `EndsOnTurnLimit`), obiettivo contestabile verificato nel **Cleanup** dopo effetti ambientali e KO, progresso **intero** nell'HUD e nel TurnLog · 🔴 *la riga diceva «⏳ nessun oggetto da attivare in mappa» e contava **27** test: il primo è falso dal 2026-08 (`#74` chiuse `Activate`/`Interact`, `#833` ha portato porte e interruttori), il secondo era fermo. Rimisurata il 2026-09-02* · 🔵 **e quella rimisura era imprecisa su metà, corretta il 2026-09-04**: «#833 ha portato porte e interruttori» è vero per il **codice** e falso per la **mappa spedita** — `DA_HexMap_Arena` ha 64 celle e **0 porte**, misurato da `HexMap.AuthoredArenaDoorCountMatchesTheRoadmap`. Capability e contenuto sono due proposizioni diverse, e §2 ora le tiene separate · ⏱️ **e il contenuto è arrivato il 2026-09-05** ([#2330](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2330)): una porta `Closed` posata su `(q=-3,r=2,L=0)` bordo `SE` con `RTSetCellDoor`, quindi ora sono vere entrambe — restano comunque due proposizioni, non una |
