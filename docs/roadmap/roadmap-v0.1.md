...
⚠️ **`CP 7.2` chiude su quattro gadget degli otto, e questa riga lo diceva già** — la issue `#61` no. Il DoD è stato riscritto il 2026-09-02 sullo scope esprimibile: i quattro assenti non «mancano», sono assenti per **quattro ragioni diverse**, fissate da `Equipment.Gadget.FourAreNotExpressibleYet`, e sono usciti **uno per issue** (`#2087` `SmokeEmitter` · `#2088` `Anchor` · `#2089` `Insulator` · `#2090` `Sensor`) perché hanno costi asimmetrici: uno costa un'azione core, due costano epic che non esistono.

🔴 **Qui il documento era avanti e la issue indietro** — l'inverso del difetto che `#14` ha registrato il 2026-08-12. La riga `7.2` porta ✅ e «4 degli 8» da quando il catalogo ha preso quella forma; la issue ha continuato a chiederne otto. Nessun gate confronta i due versi.
