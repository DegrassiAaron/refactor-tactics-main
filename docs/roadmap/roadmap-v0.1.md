...
| **E23.2** | Porta come oggetto logico unico | Una porta larga più celle resta **un** oggetto con **uno** stato, non N archi indipendenti (gruppo atomico). ⚠️ `D-375`: vale per l'**interazione** — `Open`/`Closed`/`Locked` commutano di gruppo — ma **non** per la distruzione: `Destroyed` è terminale e **per segmento**, e un portone sfondato in un punto è un portone con un buco. 🔁 Non è una novità di `D-375`: [`spec-porte-cp93.md`](../gameplay/spec-porte-cp93.md) lo scriveva già — *«se un bordo del gruppo viene distrutto, gli altri restano»* — ed è **questa riga** che era rimasta indietro. | |
