...
| Allestimento partita (mappa hex, roster da catalogo) | `RTGameMode.{h,cpp}`, `Turn/RTMatchSetupLibrary.*` | ✅ CP 2.1/6.1 · formazioni `Flux+Riva` vs `Bastion+Vektor` |
...
> ⏱️ **Numeri misurati su `80d967ee`, con uno scarto dichiarato prima di prodursi.** Questa riga diceva
> «21 epic, 100 checkpoint» e vale `22/106` con la sola **E47**. Ma
> [`docs/menu-frontend-consolidamento`](https://github.com/DegrassiAaron/refactor-tactics-main/issues/934)
> aggiunge **E46** (6 CP) alla stessa tabella e nessuno dei due rami vede l'altro: quando atterrano
> entrambi il valore sarà **23 epic, 112 checkpoint**, e va **rimisurato sull'albero unito** con lo script
> qui sotto, mai sommato. È la lezione che questo file ha già pagato quattro volte — vedi il blocco
> seguente, che l'aveva imparata in un solo verso.
...
> manca alle reazioni d'eroe; **CP 6.7** le cabla. `Vektor.InterceptShot` **non** rientra qui: è un trigger
...
> effetti, `ReactiveCapacitor` finirebbe come `if (Flux…)` nel resolver.
...
| **14.8** *(nuovo 2026-08-09)* | **Decision Time Bank** — budget di decisione per giocatore | Una risorsa temporale **per giocatore**, condivisa da tutte le Decision Window live: dentro la `Grace` non consuma, oltre consuma il tempo effettivo, e allo scadere costa `MaxWindow − Grace` per intero. `InitialBank` è **derivato** — `RoundLimit × (MaxWindow − Grace)`, 24 s in 2v2 — non un numero fisso. La singola finestra **resta 3,0 s**: il bank non la allunga mai e non tocca mai le `AllowedResponses`. È **wall-clock, non regola**: sta accanto a `PlanningSeconds` e non entra in `URTMatchFormatData` né in alcun hash (`RTMatchFormatData.h`). Il residuo è un **input canonico registrato**: il replay lo legge dal TurnLog, non lo ricalcola. Visibilità **owner-only** — un bank pubblico riaprirebbe il canale che [D-021](../decisions/RT_PDR_00_Decision_Log.md) e [D-048](../decisions/RT_PDR_00_Decision_Log.md) chiudono. Il **bot ha un bank** e lo consuma per policy: nessun ramo `IsBot` nella Decision Window. Vincolo che rende equo il costo pieno del timeout: il fallback è **preselezionato e raggiungibile entro la grace**, apparizione del prompt inclusa — **da misurare**, non da assumere. **Sostituisce D20** («nessun cap aggregato») prima della misura che l'avrebbe informata: rischio dichiarato, e i due rientri di ADR-0004 §Revisione restano validi. **Non precede 14.5/14.6**: la prima misura arriva prima della taratura. Owner: [`spec-decision-time-bank.md`](../gameplay/spec-decision-time-bank.md) | I **dieci** del Feature Registry — `TimeBank.GraceDoesNotDrain`, `TimeBank.DrainsAfterGrace`, `TimeBank.NeverBelowZero`, `TimeBank.TimeoutCostsFullWindow`, `TimeBank.TimeoutSpendsNoCharge`, `TimeBank.ClashCostsFullWindow`, `TimeBank.BotDrainsLikePlayer`, `TimeBank.ExhaustionKeepsResponsesLegal`, `TimeBank.ReplayReadsRecordedBank`, `TimeBank.PacketOrderInvariant` (*`ExhaustionKeepsResponsesLegal` mancava da questa riga e dal corpo di `#319`: allineate entrambe il 2026-08-13*); UI `TimeBank.FallbackReachableWithinGrace`; scenari `Spec.TimeBank.*`, **scrivibili dal 2026-08-10** — `LogEventAmount` (`a7e4677b`) ha chiuso l'ultimo blocco dell'harness |
...
`Overwatch.CancelledByStun`/`…ByForcedMovement` → **CP 14.6**;
