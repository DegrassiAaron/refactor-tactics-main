...
| **E47.3** ⏳ | Grammatica visiva della board | Ogni categoria leggibile da **due** canali, `colore + forma` ([D-146](../decisions/RT_PDR_00_Decision_Log.md)). La legenda è **derivata** da superficie, coperture di bordo ed entità obiettivo: zero `enum` nuovi, nessuna migrazione di formato. **Baseline misurata**: sulla tavolozza di `URTHexLibrary::SurfaceColor`, in scala di grigi **7 coppie di superfici su 36 collassano** sotto la soglia del gate che già esiste — il criterio è portarle a **0**, o dichiarare quali non hanno bisogno del secondo canale | ⏳ `PIE-V01-BOARD` — voce da creare, [#1262](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1262) |
...
> 🔴 **E l'atto che U23 nomina — [#1242](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1242)
> — non le copre**: il suo criterio conta le sole `PIE-V01-FRONTEND-*` e dichiara fuori scope «le altre voci
> PIE mancanti». `PIE-V01-BOARD` e `PIE-V01-PACKAGED` sono quindi **orfane**, non rinviate — l'atto è
> [#1262](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1262). Le sei `PIE-V01-FRONTEND-*` di
> **E46** erano nella stessa condizione e hanno la propria issue: è il motivo per cui l'attribuzione a #1242
> sembrava sufficiente e non lo era.
>
