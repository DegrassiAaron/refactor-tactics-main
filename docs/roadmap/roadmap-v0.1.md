...
| **E11** HUD, log e debug | 🟡 **CP 11.3 in gran parte chiuso** | 4 `Preview.*`, 4 `PlayerInput.*`, 8 `Playback.*`; console `rt.Debug.DrawCells` e `rt.Debug.Pacing` esistono · **CP 11.3 (2026-08-10, `#79`)**: le voci di combattimento non sono più **anonime** — portano `ActionId` e `BaseActionId`, quindi con due attaccanti nello stesso Blast il replay sa attribuire il colpo (`TurnLog.CombatEntryNamesItsAction`, `TurnLog.BasicAttackLogsBaseAndProfile`) · ⏳ `Priority` nella voce (serve un incremento di formato, vedi CP 11.3) · ⏳ Ghost Timeline (CP 11.5/11.6) · ⏳ comandi `rt.Debug.*` (CP 11.4) |
...
| **11.3** 🟡 | Combat log con reason code | Ogni esito del TurnLog compare nel log con coordinate assiali `(q,r,L)`, `ActionId`, `Priority` e `ValidationResult`; i fallback applicati sono espliciti. **2026-08-10 (`#79`)**: coordinate ✅ · `ActionId` ✅ — le voci `Combat` erano **anonime**, ora nominano l'azione e la generica di cui è profilo · fallback e `ValidationResult` ✅ (`ERTActionInvalidReason` in `Amount`) · ⏳ **`Priority`**: è un campo nuovo, cioè una versione nuova del formato, e la `v6` è già presa dalla PR `#396` — rimandato al prossimo incremento (`spec-turnlog.md` §4.3) | `TurnLog.CombatEntryNamesItsAction`, `TurnLog.BasicAttackLogsBaseAndProfile`; ⏳ `PIE-V01-LOG` |
