...
| Deflect: pool commutativa, con anti-mutazione | **D-309** e **D-312** nel [Decision Log](../decisions/RT_PDR_00_Decision_Log.md) · issue `#1918` · PR `#2032` · `1ac6e734`. Test: `Combat.DeflectPoolIsPermutationInvariant`, `Combat.DeflectAbsorbsBeforeGuardReduces`, `Combat.GuardAndDeflectAbsorbInDeclaredOrder`. ⏱️ *I primi due si chiamavano `Combat.GuardPoolIsPermutationInvariant` e `Combat.DeflectPoolAbsorbsBeforeGuardPool` fino a [D-408] (2026-09-20), che ritira il pool della sola `Guard`: il `Deflect` resta un pool ([D-309]) e questa riga non cambia oggetto, cambiano i nomi. ⚠️ Il primo era la **seconda metà** di un test condiviso con la Guardia, ed è uscito insieme al nome quando la prima metà è stata riscritta: ripristinato come test proprio nella stessa PR.* |
| Determinismo, 100 ripetizioni | **`G4` verde dal 2026-08-24** — `Replay.Verifier.ResimulationIsDeterministic`. |
| **`PIA-1.1`** golden showcase — misurato il 2026-09-09 su `682cc574` | ✅ `ShowcaseRelay.EightTurnsGoldenMatches` · `Scenario.ShowcaseRelayV01PlaysEveryTurn` · `ShowcaseRelay.EveryKeyEventAppearsInTheLog`, verdi in una passata di **23**. **Oracolo anti-shortcut**: `git grep -nE 'ShowcaseTurn *== *[0-9]' Source/` → **nessun match**. Un golden verde non basta, e questo è ciò che lo rende non-vacuo. |
| **`PIA-1.3`** rivalidazione della copertura sull'effective target — 2026-09-09 | ✅ `Heroes.BranthInterpositionRevalidatesCoverOnEffectiveTarget` ([#2616](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2616)). Pinna [D-017] con l'oracolo del **danno**: `11` contro `21`. ⚠️ **I tre test preesistenti erano ciechi a questa proprietà** — restano verdi con la rivalidazione mutata via. |
