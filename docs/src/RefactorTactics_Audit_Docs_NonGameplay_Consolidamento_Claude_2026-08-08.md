...
> Le decisioni marcate **BLOCKED / HUMAN DECISION** devono essere discusse con l'autore prima di essere applicate.
...
7. registrare come `OPEN_DECISION` i dubbi emersi nelle discussioni più recenti;
8. non modificare `docs/gameplay/` durante questa passata.
...
La grammatica definitiva degli slot/action economy va chiusa con l'autore.

---

## `decisions/adr-0004-finestre-di-reazione.md`

**Azione: UPDATE MINORE + BLOCKED**
...
2. privacy temporale:
   - se il mondo si ferma globalmente mentre un player risponde, l'avversario può dedurre che è apparso un Decision Boundary;
   - decidere se questo timing leak è accettabile o va mascherato.

---

## `decisions/adr-0005-orientamento.md`

**Azione: UPDATE + BLOCKED**

Il facing come stato gameplay è corretto.

Chiarire in modo inequivocabile:
...
L'attuale concetto "si sceglie alla fine del Move e vale nel turno successivo" può entrare in tensione con reaction/attacchi che devono dipendere dal facing dopo un Dash nello stesso turno.

Non scegliere per deduzione.
...
**Azione: UPDATE + BLOCKED P0**

Problema principale:

il catalogo non ha ancora assorbito completamente D-012.
...
Manca una voce canonica chiara per:
...
Prepared reaction slot:
hero/module reaction
```

Overwatch consuma/opportunizza il main action ma poi crea uno stato/reaction armata.
...
Vedi Human Decision #1.

### Altro punto

`SuppressiveLine` e Overwatch sembrano occupare una nicchia simile.

Stabilire se:
- SuppressiveLine è una action distinta;
- oppure un profilo/variant di Overwatch.

Non fondere senza decisione.
...
Aggiungere inoltre un campo/profilo Overwatch per ogni eroe **solo quando viene definita la parametrizzazione**.

Non inventare i quattro profili.
...
Oggi esistono due concetti da non confondere:

1. vantaggio geometrico/LOS;
2. `OccupantDamageBonus` / `EffectiveAttackPower`.

La discussione recente ha suggerito di **non aggiungere un bonus numerico alla vision range** senza playtest.

Questo NON implica automaticamente rimuovere un eventuale bonus danno già esistente.

Serve decisione separata:
- quota = solo geometria?
- quota = geometria + bonus danno esistente?
- nessun `+VisionRange` finché non misurato?
...
**Azione: BLOCKED — RECLASSIFY OR REBUILD**

Il workbook è molto più vecchio del catalogo Markdown.

Divergenze individuate:
...
Non correggere singole celle a mano finché non decidiamo il ruolo del workbook.

Opzioni:

### A — raccomandata nel breve

```text
Workbook = RESEARCH / BALANCE EXPLORATION
Markdown catalogs = canonical
```

### B

Generare `RefactorTactics_Balance_Matrices_v0.2.xlsx` dai dati canonici correnti.
...
# 6. Human Decisions — NON applicare automaticamente

Queste sono le decisioni che l'audit NON deve chiudere per plausibilità.

## HD-1 — Grammatica delle azioni generiche

Oggi il catalogo corrente contiene:
...
La discussione recente aggiunge:

```text
Overwatch
```

e alcuni brief sembrano suggerire di sostituire parte delle azioni precedenti.

### Raccomandazione dell'audit
...
almeno finché non esiste una ragione di gameplay per fondere.

In particolare:
- `Guard` = protezione generale;
- `Brace` = anti-displacement / stance più specifica;
- `Activate` = attivazione di dispositivo/mappa;
- `Interact` = interazione generica/obiettivo.

**Decisione richiesta:** confermare o correggere.

---

## HD-2 — Sneak / Move / Sprint

Due modelli incompatibili sono emersi.

### Modello corrente

`Sprint` è una vera azione della fase **Dash**:
- usa Movimento + Principale;
- 8 MP;
- Exposed;
- niente prepared reaction.

### Modello alternativo

Sneak / Move / Sprint sono profili della stessa famiglia di movimento.

### Raccomandazione

Per v0.1:

- mantenere Sprint come action Dash corrente;
- se serve, aggiungere in futuro `Sneak` come profilo/azione separata;
- non rifare ora action economy.

**Decisione richiesta.**

---

## HD-3 — Predictive Action nella v0.1

Le Tactical Bets sono un pilastro interessante, ma il framework completo di trap/prediction può esplodere lo scope.

### Raccomandazione

Portare nella showcase **una sola predictive action concreta**, preferibilmente Vektor:

```text
scegli cella / linea / boundary in Planning
→ se un avversario attraversa, payoff
→ se nessuno attraversa, azione sprecata
```

Questo dimostra il concetto senza costruire tutte le trap.

**Decisione richiesta:** dentro o fuori v0.1?

---

## HD-4 — Intercept + cover

Quando un attacco destinato ad A viene intercettato da B:

### Opzione A

mantiene geometria/modificatori calcolati su A.
...
contro **B**, che è il bersaglio reale.

Vincolo:
- la rivalidazione non apre una nuova reaction annidata.
...
- LOS geometrica/elevazione;
- vision range;
- danno.

### Raccomandazione

- **NO `+VisionRange` automatico** finché il playtest non lo giustifica;
- quota deve già modificare geometria, LOS, cover, percorso;
- decidere separatamente se mantenere il bonus danno già presente nel modello/codice.

**Decisione richiesta:** il bonus danno da quota resta?
...
Tenere il termine, ma solo per:

> decisione live limitata, durante una Decision Window, come continuazione della propria azione.

NON usarlo per:
- Delayed Action;
- Predictive Action;
- azione completamente scelta durante Planning.

Non implementarlo finché non abbiamo una ability concreta.

**Decisione richiesta:** confermare.

---

## HD-7 — Facing dopo Dash

Serve una timeline formale.

Esempio:

```text
Facing start turn = East

Prep
Dash → unità va NE
Blast
Move → termina verso West
```

Domande:

- il Blast usa East o NE?
- una Overwatch armata in Prep usa il facing iniziale?
- una reaction dopo Dash usa il facing post-Dash?
- il facing finale West vale solo dal turno dopo?

**Non dedurre la risposta.**

---

## HD-8 — Timing privacy delle Decision Window

Il payload può essere perfettamente privato, ma se tutta la Resolution si ferma per 3 secondi, anche l'avversario può osservare:

> "è comparsa una decisione a qualcuno".

Possibili policy:

### A — leak di timing accettato

L'avversario sa solo che c'è una pausa, non:
- chi;
- perché;
- quali opzioni;
- quale trigger.
...
presentazione/pacing maschera la finestra agli altri client.

### C — server continua presentation bufferizzata ma non logical resolution

più complesso.

Per MVP raccomandazione: **A**, ma deve essere una decisione esplicita perché il progetto ha privacy molto rigorosa.
...
Il repo è di fatto bloccato su UE 5.8.1.

`OPEN_DECISIONS` conserva ancora D-007 come assunzione.

### Raccomandazione

Consolidare formalmente:

```text
UE baseline = 5.8.1
upgrade only between milestones / explicit migration
```
...
## HD-10 — Ruolo del workbook

### A

`Balance_Matrices_v0.1.xlsx` = **RESEARCH**, Markdown = canone.

### B

Creare v0.2 e renderlo una rappresentazione sincronizzata dei cataloghi.

### Raccomandazione

A nel breve, B solo quando esiste un processo di generazione/validazione che impedisce una nuova deriva.
...
## Fase 3 — Non modificare i contenuti BLOCKED

Per i temi HD-1…HD-10:

```text
DO NOT GUESS
DO NOT CANONIZE
DO NOT UPDATE BALANCE VALUES
```

Inserire marker/document links dove necessario e fermarsi.
...
Claude deve fermarsi e chiedere decisione umana se una modifica:

- cambia action economy;
- elimina un'azione;
- cambia il costo di Overwatch;
- cambia Sprint;
- cambia High Ground damage;
- cambia Intercept semantics;
- introduce Fast Action;
- decide facing timing;
- decide privacy timing policy;
- rende il workbook canonical;
- decide il formato 3v3/4v4.

Questi sono **design decisions**, non "doc cleanup".
