...
Oggi la cartella contiene una cosa sola: [`icons/`](icons/), **755** file rigenerati il 2026-09-06 da
...
| `docs/generated/icons/` | `tools/hud-assets/generate_hud_assets.py` | `python tools/hud-assets/generate_hud_assets.py --out <dir>` | ⛔ **nessuno** | la geometria vive nel generatore. ⚠️ La struttura prodotta (`Icons/` piatto) **non è** quella di questa cartella (`svg/` + `png/&lt;size&gt;/`): serve una rimappatura, e la dimensione **96** non è più producibile |
...
un oracolo, e la frase che diceva *«finché è aperta»* è rimasta a indicare una condizione che non esiste più. ⚠️ **Il fatto sopravvive al suo riferimento**: dopo la rigenerazione del 2026-09-06 i master sono **755** e continuano a non avere un `--check`. Averlo scritto in tabella non basta: il gate verifica che il generatore
