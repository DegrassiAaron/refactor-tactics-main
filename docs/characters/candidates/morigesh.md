...
![Morigesh — Paragon asset base](../images/paragon/morigesh.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Morigesh  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Morigesh** è uno slot asset Paragon candidato con macro ruolo **Hexer / Hunter**. La Signature definita è **Hex**, affiancata da **Remote payoff**. Il design è legato ai framework **Mark, Information** e richiede soprattutto **Target marks, visibility rules, delayed effects**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_MORIGESH` |
| Asset base | Morigesh |
| Macro ruolo RT | Hexer / Hunter |
| Signature primaria | **Hex** |
| Signature secondaria | Remote payoff |
| Framework principali | Mark, Information |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Target marks, visibility rules, delayed effects |
...
**Hex** è la Signature assegnata a Morigesh nel Character Master Matrix. La meccanica secondaria associata è **Remote payoff**.

Il documento collega questa identità ai framework **Mark, Information** e alle dipendenze **Target marks, visibility rules, delayed effects**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Hexer / Hunter**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Morigesh non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Target marks, visibility rules, delayed effects.
...
```text
docs/characters/images/paragon/morigesh.png
```
