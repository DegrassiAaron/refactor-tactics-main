...
![Dekker — Paragon asset base](../images/paragon/dekker.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Dekker  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Dekker** è uno slot asset Paragon candidato con macro ruolo **Controller**. La Signature definita è **Control Geometry**, affiancata da **Confinement**. Il design è legato ai framework **Territory, PersistentEntity** e richiede soprattutto **Walls/edges, cover, path revision**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_DEKKER` |
| Asset base | Dekker |
| Macro ruolo RT | Controller |
| Signature primaria | **Control Geometry** |
| Signature secondaria | Confinement |
| Framework principali | Territory, PersistentEntity |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Walls/edges, cover, path revision |
...
**Control Geometry** è la Signature assegnata a Dekker nel Character Master Matrix. La meccanica secondaria associata è **Confinement**.

Il documento collega questa identità ai framework **Territory, PersistentEntity** e alle dipendenze **Walls/edges, cover, path revision**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.
...
Il kit completo di Dekker non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Walls/edges, cover, path revision.
...
```text
docs/characters/images/paragon/dekker.png
```
