...
![Kallari — Paragon asset base](../images/paragon/kallari.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Kallari  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Kallari** è uno slot asset Paragon candidato con macro ruolo **Infiltrator**. La Signature definita è **Hunt / Trace**, affiancata da **Silent movement**. Il design è legato ai framework **Information, Mark, Noise** e richiede soprattutto **Fog of War, noise, stealth, team knowledge**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_KALLARI` |
| Asset base | Kallari |
| Macro ruolo RT | Infiltrator |
| Signature primaria | **Hunt / Trace** |
| Signature secondaria | Silent movement |
| Framework principali | Information, Mark, Noise |
| Complessità tecnica | 5/5 |
| Dipendenze tecniche | Fog of War, noise, stealth, team knowledge |
| Design status | `SIGNATURE_DEFINED` |
| ReleaseVersion | `UNASSIGNED` |
| RT Character ID | `TBD` |
| Player question | Quanto so davvero della posizione della preda? |
| Provenienza asset | Final Paragon asset release (2018) |

## Descrizione della Signature

**Hunt / Trace** è la Signature assegnata a Kallari nel Character Master Matrix. La meccanica secondaria associata è **Silent movement**.

Il documento collega questa identità ai framework **Information, Mark, Noise** e alle dipendenze **Fog of War, noise, stealth, team knowledge**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Infiltrator**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.

### Player question

> Quanto so davvero della posizione della preda?
...
Il kit completo di Kallari non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Fog of War, noise, stealth, team knowledge.
...
```text
docs/characters/images/paragon/kallari.png
```
