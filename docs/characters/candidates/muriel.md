...
![Muriel — Paragon asset base](../images/paragon/muriel.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Muriel  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Muriel** è uno slot asset Paragon candidato con macro ruolo **Support**. La Signature definita è **Guardian Link**, affiancata da **Remote protection**. Il design è legato ai framework **Link, Reaction** e richiede soprattutto **Team links, shields, ally events**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_MURIEL` |
| Asset base | Muriel |
| Macro ruolo RT | Support |
| Signature primaria | **Guardian Link** |
| Signature secondaria | Remote protection |
| Framework principali | Link, Reaction |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Team links, shields, ally events |
...
**Guardian Link** è la Signature assegnata a Muriel nel Character Master Matrix. La meccanica secondaria associata è **Remote protection**.

Il documento collega questa identità ai framework **Link, Reaction** e alle dipendenze **Team links, shields, ally events**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Support**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Muriel non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Team links, shields, ally events.
...
```text
docs/characters/images/paragon/muriel.png
```
