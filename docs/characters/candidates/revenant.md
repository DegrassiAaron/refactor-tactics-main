...
![Revenant — Paragon asset base](../images/paragon/revenant.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Revenant  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Revenant** è uno slot asset Paragon candidato con macro ruolo **Execution Marksman**. La Signature definita è **Chamber**, affiancata da **Execution mark**. Il design è legato ai framework **StateMachine, Resource, Mark** e richiede soprattutto **Ammo/chamber state, target mark, payoff rules**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_REVENANT` |
| Asset base | Revenant |
| Macro ruolo RT | Execution Marksman |
| Signature primaria | **Chamber** |
| Signature secondaria | Execution mark |
| Framework principali | StateMachine, Resource, Mark |
| Complessità tecnica | 5/5 |
| Dipendenze tecniche | Ammo/chamber state, target mark, payoff rules |
...
**Chamber** è la Signature assegnata a Revenant nel Character Master Matrix. La meccanica secondaria associata è **Execution mark**.

Il documento collega questa identità ai framework **StateMachine, Resource, Mark** e alle dipendenze **Ammo/chamber state, target mark, payoff rules**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Execution Marksman**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Revenant non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Ammo/chamber state, target mark, payoff rules.
...
```text
docs/characters/images/paragon/revenant.png
```
