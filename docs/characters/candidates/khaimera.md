...
![Khaimera — Paragon asset base](../images/paragon/khaimera.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Khaimera  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Khaimera** è uno slot asset Paragon candidato con macro ruolo **Pursuer**. La Signature definita è **Predator / Pursuit**, affiancata da **Prey lock**. Il design è legato ai framework **Mark, MovementHistory** e richiede soprattutto **Target relation, pursuit, reaction hooks**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_KHAIMERA` |
| Asset base | Khaimera |
| Macro ruolo RT | Pursuer |
| Signature primaria | **Predator / Pursuit** |
| Signature secondaria | Prey lock |
| Framework principali | Mark, MovementHistory |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Target relation, pursuit, reaction hooks |
...
**Predator / Pursuit** è la Signature assegnata a Khaimera nel Character Master Matrix. La meccanica secondaria associata è **Prey lock**.

Il documento collega questa identità ai framework **Mark, MovementHistory** e alle dipendenze **Target relation, pursuit, reaction hooks**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Pursuer**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Khaimera non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Target relation, pursuit, reaction hooks.
...
```text
docs/characters/images/paragon/khaimera.png
```
