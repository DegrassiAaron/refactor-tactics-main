...
![Shinbi — Paragon asset base](../images/paragon/shinbi.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Shinbi  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Shinbi** è uno slot asset Paragon candidato con macro ruolo **Duelist / Setup**. La Signature definita è **Resonance**, affiancata da **Sequence payoff**. Il design è legato ai framework **Rhythm, Mark** e richiede soprattutto **Hit/event history, ordered stacks**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_SHINBI` |
| Asset base | Shinbi |
| Macro ruolo RT | Duelist / Setup |
| Signature primaria | **Resonance** |
| Signature secondaria | Sequence payoff |
| Framework principali | Rhythm, Mark |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Hit/event history, ordered stacks |
...
**Resonance** è la Signature assegnata a Shinbi nel Character Master Matrix. La meccanica secondaria associata è **Sequence payoff**.

Il documento collega questa identità ai framework **Rhythm, Mark** e alle dipendenze **Hit/event history, ordered stacks**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Duelist / Setup**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Shinbi non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Hit/event history, ordered stacks.
...
```text
docs/characters/images/paragon/shinbi.png
```
