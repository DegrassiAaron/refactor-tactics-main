...
![Iggy & Scorch — Paragon asset base](../images/paragon/iggy-and-scorch.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Iggy & Scorch  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Iggy & Scorch** è uno slot asset Paragon candidato con macro ruolo **Terrain Controller / Duo**. La Signature definita è **Combustion Field**, affiancata da **Persistent deployables**. Il design è legato ai framework **Territory, PersistentEntity, Environment** e richiede soprattutto **Fire propagation, devices, hazard state**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_IGGY_SCORCH` |
| Asset base | Iggy & Scorch |
| Macro ruolo RT | Terrain Controller / Duo |
| Signature primaria | **Combustion Field** |
| Signature secondaria | Persistent deployables |
| Framework principali | Territory, PersistentEntity, Environment |
| Complessità tecnica | 5/5 |
| Dipendenze tecniche | Fire propagation, devices, hazard state |
...
**Combustion Field** è la Signature assegnata a Iggy & Scorch nel Character Master Matrix. La meccanica secondaria associata è **Persistent deployables**.

Il documento collega questa identità ai framework **Territory, PersistentEntity, Environment** e alle dipendenze **Fire propagation, devices, hazard state**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Terrain Controller / Duo**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Iggy & Scorch non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Fire propagation, devices, hazard state.
...
```text
docs/characters/images/paragon/iggy-and-scorch.png
```
