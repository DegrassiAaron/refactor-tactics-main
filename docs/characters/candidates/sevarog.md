...
![Sevarog — Paragon asset base](../images/paragon/sevarog.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Sevarog  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Sevarog** è uno slot asset Paragon candidato con macro ruolo **Tank / Controller**. La Signature definita è **Essence Harvest**, affiancata da **Area dominance**. Il design è legato ai framework **Resource, Mark** e richiede soprattutto **Event-driven gain rules, KO/objective events**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_SEVAROG` |
| Asset base | Sevarog |
| Macro ruolo RT | Tank / Controller |
| Signature primaria | **Essence Harvest** |
| Signature secondaria | Area dominance |
| Framework principali | Resource, Mark |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Event-driven gain rules, KO/objective events |
...
**Essence Harvest** è la Signature assegnata a Sevarog nel Character Master Matrix. La meccanica secondaria associata è **Area dominance**.

Il documento collega questa identità ai framework **Resource, Mark** e alle dipendenze **Event-driven gain rules, KO/objective events**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Tank / Controller**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Sevarog non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Event-driven gain rules, KO/objective events.
...
```text
docs/characters/images/paragon/sevarog.png
```
