...
![Serath — Paragon asset base](../images/paragon/serath.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Serath  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Serath** è uno slot asset Paragon candidato con macro ruolo **Duelist**. La Signature definita è **Dual State**, affiancata da **Risk/reward stance**. Il design è legato ai framework **StateMachine, Resource** e richiede soprattutto **Stances, ability variants, status rules**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_SERATH` |
| Asset base | Serath |
| Macro ruolo RT | Duelist |
| Signature primaria | **Dual State** |
| Signature secondaria | Risk/reward stance |
| Framework principali | StateMachine, Resource |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Stances, ability variants, status rules |
...
**Dual State** è la Signature assegnata a Serath nel Character Master Matrix. La meccanica secondaria associata è **Risk/reward stance**.

Il documento collega questa identità ai framework **StateMachine, Resource** e alle dipendenze **Stances, ability variants, status rules**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Duelist**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Serath non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Stances, ability variants, status rules.
...
```text
docs/characters/images/paragon/serath.png
```
