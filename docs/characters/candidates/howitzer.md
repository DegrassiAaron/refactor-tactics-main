...
![Howitzer — Paragon asset base](../images/paragon/howitzer.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Howitzer  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Howitzer** è uno slot asset Paragon candidato con macro ruolo **Artillery**. La Signature definita è **Artillery Heat**, affiancata da **Trajectory control**. Il design è legato ai framework **Resource, Geometry** e richiede soprattutto **Arcing trajectory, AoE, heat management**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_HOWITZER` |
| Asset base | Howitzer |
| Macro ruolo RT | Artillery |
| Signature primaria | **Artillery Heat** |
| Signature secondaria | Trajectory control |
| Framework principali | Resource, Geometry |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Arcing trajectory, AoE, heat management |
...
**Artillery Heat** è la Signature assegnata a Howitzer nel Character Master Matrix. La meccanica secondaria associata è **Trajectory control**.

Il documento collega questa identità ai framework **Resource, Geometry** e alle dipendenze **Arcing trajectory, AoE, heat management**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Artillery**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Howitzer non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Arcing trajectory, AoE, heat management.
...
```text
docs/characters/images/paragon/howitzer.png
```
