...
![Phase — Paragon asset base](../images/paragon/phase.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Uso corrente:** base visuale del **roster v0.1** (`Hero.Muiren`) — [D-037](../../decisions/RT_PDR_00_Decision_Log.md). L'asset è impegnato; il concept di Phase resta un candidato senza release.  
> **Asset base:** Paragon — Phase  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Phase** è uno slot asset Paragon candidato con macro ruolo **Support / Mobility**. La Signature definita è **Phase Link**, affiancata da **Ally relocation**. Il design è legato ai framework **Link, Movement** e richiede soprattutto **Team link, forced movement, validation**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_PHASE` |
| Asset base | Phase |
| Macro ruolo RT | Support / Mobility |
| Signature primaria | **Phase Link** |
| Signature secondaria | Ally relocation |
| Framework principali | Link, Movement |
| Complessità tecnica | 5/5 |
| Dipendenze tecniche | Team link, forced movement, validation |
...
**Phase Link** è la Signature assegnata a Phase nel Character Master Matrix. La meccanica secondaria associata è **Ally relocation**.

Il documento collega questa identità ai framework **Link, Movement** e alle dipendenze **Team link, forced movement, validation**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Support / Mobility**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Phase non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Team link, forced movement, validation.
...
```text
docs/characters/images/paragon/phase.png
```
