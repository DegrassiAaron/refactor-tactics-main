...
![Lt. Belica — Paragon asset base](../images/paragon/lt-belica.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Lt. Belica  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Lt. Belica** è uno slot asset Paragon candidato con macro ruolo **Counter / Control**. La Signature definita è **Suppression Charge**, affiancata da **Resource denial**. Il design è legato ai framework **Reaction, Resource** e richiede soprattutto **Ability-use triggers, resources, counter windows**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_LT_BELICA` |
| Asset base | Lt. Belica |
| Macro ruolo RT | Counter / Control |
| Signature primaria | **Suppression Charge** |
| Signature secondaria | Resource denial |
| Framework principali | Reaction, Resource |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Ability-use triggers, resources, counter windows |
...
**Suppression Charge** è la Signature assegnata a Lt. Belica nel Character Master Matrix. La meccanica secondaria associata è **Resource denial**.

Il documento collega questa identità ai framework **Reaction, Resource** e alle dipendenze **Ability-use triggers, resources, counter windows**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Counter / Control**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Lt. Belica non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Ability-use triggers, resources, counter windows.
...
```text
docs/characters/images/paragon/lt-belica.png
```
