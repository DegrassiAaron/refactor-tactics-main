...
![Countess — Paragon asset base](../images/paragon/countess.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Countess  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Countess** è uno slot asset Paragon candidato con macro ruolo **Assassin**. La Signature definita è **Blood Contract**, affiancata da **Execution debt**. Il design è legato ai framework **Mark, Resource** e richiede soprattutto **Target marks, HP/resource trade-offs**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_COUNTESS` |
| Asset base | Countess |
| Macro ruolo RT | Assassin |
| Signature primaria | **Blood Contract** |
| Signature secondaria | Execution debt |
| Framework principali | Mark, Resource |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Target marks, HP/resource trade-offs |
...
**Blood Contract** è la Signature assegnata a Countess nel Character Master Matrix. La meccanica secondaria associata è **Execution debt**.

Il documento collega questa identità ai framework **Mark, Resource** e alle dipendenze **Target marks, HP/resource trade-offs**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Assassin**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Countess non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Target marks, HP/resource trade-offs.
...
```text
docs/characters/images/paragon/countess.png
```
