...
![Drongo — Paragon asset base](../images/paragon/drongo.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Drongo  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Drongo** è uno slot asset Paragon candidato con macro ruolo **Striker / Controller**. La Signature definita è **Payload Mix**, affiancata da **Ammo preparation**. Il design è legato ai framework **StateMachine, Resource** e richiede soprattutto **Ability variants, status payloads, AoE**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_DRONGO` |
| Asset base | Drongo |
| Macro ruolo RT | Striker / Controller |
| Signature primaria | **Payload Mix** |
| Signature secondaria | Ammo preparation |
| Framework principali | StateMachine, Resource |
| Complessità tecnica | 4/5 |
| Dipendenze tecniche | Ability variants, status payloads, AoE |
...
**Payload Mix** è la Signature assegnata a Drongo nel Character Master Matrix. La meccanica secondaria associata è **Ammo preparation**.

Il documento collega questa identità ai framework **StateMachine, Resource** e alle dipendenze **Ability variants, status payloads, AoE**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Striker / Controller**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Drongo non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Ability variants, status payloads, AoE.
...
```text
docs/characters/images/paragon/drongo.png
```
