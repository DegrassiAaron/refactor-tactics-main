...
![Zinx — Paragon asset base](../images/paragon/zinx.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Zinx  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Zinx** è uno slot asset Paragon candidato con macro ruolo **Sustain / Controller**. La Signature definita è **Energy Debt**, affiancata da **Resource conversion**. Il design è legato ai framework **Resource, StateMachine** e richiede soprattutto **Negative resources, delayed costs, validation**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_ZINX` |
| Asset base | Zinx |
| Macro ruolo RT | Sustain / Controller |
| Signature primaria | **Energy Debt** |
| Signature secondaria | Resource conversion |
| Framework principali | Resource, StateMachine |
| Complessità tecnica | 5/5 |
| Dipendenze tecniche | Negative resources, delayed costs, validation |
...
**Energy Debt** è la Signature assegnata a Zinx nel Character Master Matrix. La meccanica secondaria associata è **Resource conversion**.

Il documento collega questa identità ai framework **Resource, StateMachine** e alle dipendenze **Negative resources, delayed costs, validation**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Sustain / Controller**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Zinx non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Negative resources, delayed costs, validation.
...
```text
docs/characters/images/paragon/zinx.png
```
