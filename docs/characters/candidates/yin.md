...
![Yin — Paragon asset base](../images/paragon/yin.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Yin  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Yin** è uno slot asset Paragon candidato con macro ruolo **Reaction Duelist**. La Signature definita è **Deflection**, affiancata da **Opportunity conversion**. Il design è legato ai framework **Reaction, Geometry** e richiede soprattutto **Projectile events, reaction opportunities**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Asset key | `ASSET_YIN` |
| Asset base | Yin |
| Macro ruolo RT | Reaction Duelist |
| Signature primaria | **Deflection** |
| Signature secondaria | Opportunity conversion |
| Framework principali | Reaction, Geometry |
| Complessità tecnica | 5/5 |
| Dipendenze tecniche | Projectile events, reaction opportunities |
...
**Deflection** è la Signature assegnata a Yin nel Character Master Matrix. La meccanica secondaria associata è **Opportunity conversion**.

Il documento collega questa identità ai framework **Reaction, Geometry** e alle dipendenze **Projectile events, reaction opportunities**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Reaction Duelist**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Yin non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Projectile events, reaction opportunities.
...
```text
docs/characters/images/paragon/yin.png
```
