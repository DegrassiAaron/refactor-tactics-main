...
![Greystone — Paragon asset base](../images/paragon/greystone.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Greystone  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Greystone** è uno slot asset Paragon candidato con macro ruolo **Vanguard**. La Signature definita è **Resolve**, affiancata da **Last-stand conversion**. Il design è legato ai framework **Resource, StateMachine** e richiede soprattutto **HP/state transitions, deterministic thresholds**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_GREYSTONE` |
| Asset base | Greystone |
| Macro ruolo RT | Vanguard |
| Signature primaria | **Resolve** |
| Signature secondaria | Last-stand conversion |
| Framework principali | Resource, StateMachine |
| Complessità tecnica | 3/5 |
| Dipendenze tecniche | HP/state transitions, deterministic thresholds |
...
**Resolve** è la Signature assegnata a Greystone nel Character Master Matrix. La meccanica secondaria associata è **Last-stand conversion**.

Il documento collega questa identità ai framework **Resource, StateMachine** e alle dipendenze **HP/state transitions, deterministic thresholds**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Vanguard**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.
...
Il kit completo di Greystone non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
HP/state transitions, deterministic thresholds.
...
```text
docs/characters/images/paragon/greystone.png
```
