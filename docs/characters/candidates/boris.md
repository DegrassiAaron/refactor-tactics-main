...
![Boris — Paragon asset base](../images/paragon/boris.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Boris  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Boris** è uno slot asset Paragon candidato con macro ruolo **Bruiser**. La Signature definita è **Overdrive / Heat**, affiancata da **Cooling windows**. Il design è legato ai framework **Resource, StateMachine** e richiede soprattutto **Heat thresholds, ability gating**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_BORIS` |
| Asset base | Boris |
| Macro ruolo RT | Bruiser |
| Signature primaria | **Overdrive / Heat** |
| Signature secondaria | Cooling windows |
| Framework principali | Resource, StateMachine |
| Complessità tecnica | 3/5 |
| Dipendenze tecniche | Heat thresholds, ability gating |
...
**Overdrive / Heat** è la Signature assegnata a Boris nel Character Master Matrix. La meccanica secondaria associata è **Cooling windows**.

Il documento collega questa identità ai framework **Resource, StateMachine** e alle dipendenze **Heat thresholds, ability gating**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.
...
Il kit completo di Boris non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Heat thresholds, ability gating.
...
```text
docs/characters/images/paragon/boris.png
```
