...
![Feng Mao — Paragon asset base](../images/paragon/feng-mao.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Feng Mao  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Feng Mao** è uno slot asset Paragon candidato con macro ruolo **Skirmisher**. La Signature definita è **Flow**, affiancata da **Directional momentum**. Il design è legato ai framework **MovementHistory, Facing** e richiede soprattutto **Hex movement history, dash, facing**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_FENG_MAO` |
| Asset base | Feng Mao |
| Macro ruolo RT | Skirmisher |
| Signature primaria | **Flow** |
| Signature secondaria | Directional momentum |
| Framework principali | MovementHistory, Facing |
| Complessità tecnica | 3/5 |
| Dipendenze tecniche | Hex movement history, dash, facing |
...
**Flow** è la Signature assegnata a Feng Mao nel Character Master Matrix. La meccanica secondaria associata è **Directional momentum**.

Il documento collega questa identità ai framework **MovementHistory, Facing** e alle dipendenze **Hex movement history, dash, facing**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.
...
Il kit completo di Feng Mao non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Hex movement history, dash, facing.
...
```text
docs/characters/images/paragon/feng-mao.png
```
