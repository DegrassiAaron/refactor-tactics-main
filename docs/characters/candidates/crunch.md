...
![Crunch — Paragon asset base](../images/paragon/crunch.png)

> 🧩 **Stato RefactorTactics:** `CANDIDATE / SIGNATURE_DEFINED`  
> **Release:** `UNASSIGNED`  
> **Roster ufficiale:** `NO`  
> **Asset base:** Paragon — Crunch  
> **RT Character ID:** `TBD`

## Panoramica

Nel Character Master Matrix, **Crunch** è uno slot asset Paragon candidato con macro ruolo **Combo Fighter**. La Signature definita è **Combo State Machine**, affiancata da **Finisher branches**. Il design è legato ai framework **StateMachine, Rhythm** e richiede soprattutto **Ordered action history, combo transitions**. Il kit completo, le statistiche competitive, il CharacterId e la release non sono ancora definiti.
...
| Campo | Valore |
| --- | --- |
| Asset key | `ASSET_CRUNCH` |
| Asset base | Crunch |
| Macro ruolo RT | Combo Fighter |
| Signature primaria | **Combo State Machine** |
| Signature secondaria | Finisher branches |
| Framework principali | StateMachine, Rhythm |
| Complessità tecnica | 5/5 |
| Dipendenze tecniche | Ordered action history, combo transitions |
| Design status | `SIGNATURE_DEFINED` |
| ReleaseVersion | `UNASSIGNED` |
| RT Character ID | `TBD` |
| Player question | Quale sequenza preparo? |
| Provenienza asset | Initial Paragon asset release (2018) |

## Descrizione della Signature

**Combo State Machine** è la Signature assegnata a Crunch nel Character Master Matrix. La meccanica secondaria associata è **Finisher branches**.

Il documento collega questa identità ai framework **StateMachine, Rhythm** e alle dipendenze **Ordered action history, combo transitions**. Trigger, stato preciso, payoff, counterplay numerico e telegraphing non sono ancora definiti: non vengono completati qui per evitare di trasformare una pagina Wiki in una nuova fonte di design non approvata.

## Identità tattica attuale

Il ruolo di lavoro è **Combo Fighter**. La Signature indica quale parte del sistema tattico questo slot dovrebbe mettere in primo piano, ma il comportamento completo verrà definito solo quando il personaggio passa da `SIGNATURE_DEFINED` a `KIT_DRAFT` / `DATA_SPEC`.

### Player question

> Quale sequenza preparo?
...
Il kit completo di Crunch non è ancora stato progettato per RefactorTactics. Non vengono importate automaticamente le abilità originali di Paragon e non vengono inventati nomi, danni, cooldown, range o effetti.
...
Ordered action history, combo transitions.
...
```text
docs/characters/images/paragon/crunch.png
```
