...
| 2. Nessuna consapevolezza di fase nell'input | ✅ **chiuso il 2026-09-06** ([#2518](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2518) + [#2555](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2555)) — il **produttore** esce da `ARTTurnManager::IsResolving()`; i tre ingressi che mutano il piano lo consultano via `IsWorldReadOnly()`; e `Inspect` **e' consentito**, perche' `OnResolvePlaybackFinished` ricalcola l'anteprima a risoluzione finita. La coppia `OnLockInCommitted` / `OnResolvePlaybackFinished` e' cio' che rende lo stato sempre intero: uno spegne al commit, l'altro riapre alla fine. 🔴 **Questa riga ha dichiarato «chiuso» per tre settimane una cosa irraggiungibile** — il predicato era `Phase != Planning && Phase != MatchEnded`, e quel membro nasce `Planning` e viene assegnato **una volta sola**, a `MatchEnded`. ⚠️ **Il produttore esisteva, ed e' questo che ha ingannato.** ⛔ **E il primo tentativo di chiuderla ci e' ricascato**, dichiarando `✅` con due dei tre ingressi ancora ciechi: trovato in code review. Questo `✅` e' il terzo, ed e' il primo con tutti e tre gli ingressi coperti E `Inspect` restituito |
