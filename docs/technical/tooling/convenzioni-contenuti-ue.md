...
| **Strumenti di authoring solo-Editor** | `/Game/RT/Editor/<Strumento>/` | **D-280**. Widget e asset che servono a *costruire* il gioco e non ne fanno parte: `Editor/Scenario/` per il Composer (#1804), `Editor/GrayKit/UI/` per il Playground Panel (**D-304**). ⚠️ **La cartella è organizzazione: `/Editor/` non garantirebbe da sé l'esclusione dal cook** — `Config/DefaultGame.ini` dichiara `+DirectoriesToAlwaysCook=(Path="/Game/RT")`, che coprirebbe anche questa. ✅ **Dal 2026-09-02 l'esclusione è configurata** ([#1804](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1804)): `+DirectoriesToNeverCook=(Path="/Game/RT/Editor")`, letta dal cooker in `CookOnTheFlyServer.cpp:12463` e **solo in CookByTheBook**, cioè la via di `BuildCookRun`. 🔴 **È una famiglia aperta, e il rischio è il tuo**: tutto ciò che metti qui sotto sparirà dal pacchetto **per sempre e in silenzio** — nessun avviso, nessun errore di cook. È lo scopo del namespace, ma **se un asset serve a runtime non va qui.** Presidiata da **tre** oracoli, e servono tutti e tre perche' guardano cose diverse: `RefactorTactics.Packaging.EditorNamespaceIsNeverCooked` ⛔ verifica la *configurazione*; `PIE-PKG-EDITOR-NAMESPACE` verifica il *contenuto del pacchetto*, **eseguito il 2026-09-03**; e da [#2150](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2150) `RefactorTactics.Packaging.NoReferencesIntoEditorNamespace` verifica i **riferimenti**, cioe' l'altra via d'ingresso: un asset escluso rientra lo stesso se qualcosa di cotto lo referenzia. ⚠️ Quest'ultimo nasce su un invariante **gia' vero** (0 riferimenti entranti su 115 package, misurato) ed e' validato per mutazione: il suo valore e' tutto nel futuro, e chi lo trovasse verde credendolo inutile starebbe leggendo la sua riuscita come inutilita'. 🔴 **E ha scoperto che oggi non è questa riga a escludere i due strumenti.** Cotti due pacchetti veri, uno con il never-cook e uno senza, i container sono **identici**: il Composer e il Playground Panel sono `EditorUtilityWidget`, quindi editor-only, e il cook li scarta **per classe** (`LogCook: SkipOnlyEditorOnly is enabled`) prima che la directory conti. ✅ Che il never-cook funzioni è stato provato a parte, con un controllo positivo su `/Game/RT/UI/Icons`: 62 package spariti. ∴ **la riga vale per la famiglia, non per questi due** — il primo asset NON editor-only messo qui sotto sarà escluso solo da lei, e chi la togliesse vedendo che «non cambia niente» starebbe misurando il caso in cui è ridondante. ⚠️ La riga d'allowlist esiste da 2026-08-31: `!Content/RT/Editor/**/*.uasset` |
