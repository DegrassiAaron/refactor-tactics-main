...
| `Spec.Reaction.DeflectionPoolSpansMultipleHits` | r4 | 5 | il gemello di `Deflect` della riga qui sopra, e la stessa lacuna trovata due volte: [D-309] ha reso `Deflect` un **pool** commutativo, e il corpus non lo misurava perché ogni abilità principale tranne `ImpactShot` vale 16 o più — sopra i 20 del pool, dove pool e delta-sul-primo-colpo coincidono. Quattro colpi da 8 contro un budget da 20 lasciano `83`, dove `ApplyFirstHitDelta` lascerebbe `71`. ⚠️ **Quattro colpi e non tre, ed è aritmetica**: con tre ne passano 4, sotto i 5 dello scudo base ([D-224]), e l'assertion non distinguerebbe nulla — nemmeno una reazione che non fosse partita. 🔑 **Non sostituisce `Spec.Reaction.DeflectionReducesByTwenty`**, che misura il *numero* su una partita vera e dichiara di non misurare la *forma*: questo misura la forma. Aperto da `#2190` |
