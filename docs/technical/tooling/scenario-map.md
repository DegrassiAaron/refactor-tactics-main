...
> il corpus passava a **82** scenari — `A 49 + B 21 + D-bloccati 12`. I quattro nuovi sono gli `AutoBattle.*`:
> tre in **A** (`OpenField`, `Obstacles`, `Hazard` — girano nel corpus e si decidono in 10, 14 e 12 turni) e
> `Objective` in **D**, `BLOCKED` sulla capability `Objective`.
>
> ⚠️ **Il metodo che questa riga prescrive — «si rilegge dal generato dopo il merge» — non è più
> eseguibile**: `scenariomap.shortlist.md` e il registry che lo produceva sono usciti con
> [D-181](../../decisions/RT_PDR_00_Decision_Log.md) il 2026-08-21. La misura qui sopra viene da due comandi,
> non da un generato, e la colonna «`planned` nel registry» **non ha più una fonte**:
>
> ```bash
> git ls-files Scenarios | grep '\.json$' | grep -vc '/_[^/]*$'   # 82 versionati
> # i BLOCKED si contano dal log della suite: RefactorTactics.Scenario.EveryShippedScenarioRuns li elenca
> ```

Totale corpus versionato: **117** scenari (`A 78 + B 31 + D-bloccati 8`). Totale registro PIE: **203** voci
(`B 22 + C 179 + 2 fuori classe`).
...
> `C` è `199 − 22 − 2`, con lo stesso metodo del 2026-08-13. ⚠️ *Questa riga diceva `197 − 22 − 2` — cioè `173` contro il `175` dichiarato due volte sopra — perché il minuendo era rimasto al totale **pre-merge**: la formula era giusta e il numero da cui parte no. Trovato da una code review, ed è la firma esatta del difetto che questo blocco esiste per prevenire, reintrodotto dalla nota che lo diagnostica* **Due metodi indipendenti che concordano**
> sulla riga `D`, come questo documento chiede da sé.
>
> ```bash
> find Scenarios -name '*.json' ! -name '_*' | wc -l      # 117  corpus
> find Scenarios/Visual -name '*.json' | wc -l            #  32  classe B
> grep -c '^| \*\*PIE-'     docs/technical/test-manuali-pie.md   # 203  totale registro
> grep -c '^| \*\*PIE-VIS-' docs/technical/test-manuali-pie.md   #  24  classe B
...
| `Movement.CostlyCorridor` | r4 | 4 | **il banco a corridoio di [#2914](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2914)**: l'unico scenario golden-adiacente che esercita **terreno costoso**. Prima di lui un solo file in tutto `Scenarios/` dichiarava `moveCost` (`AutoBattle/Obstacles.json`), e non era fra i golden: ogni traccia di riferimento girava su pavimento a costo `1`, dove ogni arco dura un micro-step e il resolver si comporta come prima di [`D-381`](../../decisions/RT_PDR_00_Decision_Log.md). 🔑 **Discrimina attraverso l'ESITO, non il tempo**: un `expect` non puo' asserire un micro-step, quindi `X` e `Y` convergono sulla stessa cella con costi d'ingresso diversi — con le durate `X` arriva prima e la prende, senza le durate la contesa e' simultanea e si fermano entrambe. ⛔ **Non ha un inseguitore, e non e' una dimenticanza**: `FindPathAvoiding` esclude le celle occupate, quindi un seguito non e' autorabile in uno scenario — vive nei test C++ che bypassano la pianificazione |
...
| `Spec.Movement.BlastOriginIsPostDashCell` | r4 | 1 | **diagnostico di `CM-04`** ([#2277](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2277)): l'attacco parte dalla cella **post-Dash**, provato in partita e non dedotto. Una barriera di **sola vista** rende il colpo possibile solo dopo lo scatto — se il Blast leggesse la posizione pre-Dash il bersaglio resterebbe a 90 HP, che è ciò che `Combat.BlockedByWall` misura sulla stessa arena. ⛔ La terza posizione della review non esiste: `D-028` dà un solo movimento per turno |
| `Spec.Facing.CoverChoiceCostsTenPoints` | r5 | 2 | **diagnostico di `CM-03`** ([#2277](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2277)): quanto **vale** la postura, in punti. Due scene identiche nello stesso file, stesso pannello e stesso colpo: chi guarda il tiratore chiude a **114**, chi gli dà le spalle a **104**. La differenza — **10** — è l'intera copertura che si accende o si spegne col facing. ⚠️ `FrontHitOnCoverIsSilent`/`RearHitOnCoverIsTraced` asseriscono il **log**; questo asserisce gli **HP** |
| `Spec.Overwatch.ThreeArmedWatchersOnOneEntry` | r4 | 1 | **diagnostico di `CM-05`** ([#2277](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2277)): il **moltiplicatore** del pacing che la DoD di [#166](https://github.com/DegrassiAaron/refactor-tactics-main/issues/166) chiede. Tre linee di Overwatch convergono su una cella, un solo ingresso: si aprono **3** boundary, uno per watcher. Il costo **scala** con le unità armate — a `3,0 s` ciascuno, un passo vale **9 s** dei 20 di `ADR-0004`. ⛔ Non misura secondi: quelli restano `CP 14.6` con decisori veri |
| `Spec.Overwatch.PushSilencesTheWatcher` | r5 | 4 | **registro eseguibile di un comportamento indeciso** ([#2367](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2367)): un guardiano armato e poi **spinto** non fa scattare la reazione su chi attraversa la sua linea — e la cella e' **dentro** la zona, non fuori portata. ⚠️ Vale solo se **nessun compagno** copre il corridoio: `Observers` sono tutta la squadra, e un solo compagno che vede riapre la finestra. 🔴 `UnitHpEquals M1 90` **non asserisce che sia giusto**: asserisce che non cambi per sbaglio — tre delle quattro uscite di #2367 lo rovesciano, e chi ne sceglie una aggiorna la riga nello stesso commit. ➕ Seconda sentinella: **nessuna `decision` dichiarata**, quindi una finestra che tornasse ad aprirsi uscirebbe `ERROR` |
| `Spec.Objective.HeldAgainstAnAdjacentEnemy` | RelayBasin | 3 | **diagnostico di `CM-09`** ([#2277](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2277)): il deadlock temuto dalla review **non è raggiungibile** in v0.1 — con un obiettivo su una cella `ResolveObjectiveControl` non può dare `Contested`. Misura il rischio opposto: chi occupa il Relay segna a ogni Cleanup **senza fare niente**, e chi gli sta a un passo non toglie nulla. Il counterplay posizionale non esiste. ⏳ La contesa arriva con `CP 31.1` ([#1583](https://github.com/DegrassiAaron/refactor-tactics-main/issues/1583)) |
...
| `Spec.Facing.OverwatchHitCameFromSide` | r5 | 1 | la stessa relazione a sei lati per un colpo che **non passa da `Plan.Hits`**: il fuoco di Overwatch (`#2128`). Il lato è `Rear`, e il facing di fine turno è `E` — **diversi apposta**: il colpo misura l'orientamento d'**ingresso** nella fase Move, perché `DerivedFromMove` si scrive dopo il ciclo dei micro-step. ⚠️ **Un solo colpo per costruzione**, stessa cautela di `SixRelativeSides`; e il discriminante del fuoco è la **troncatura**, non `ReactionDecision/Chosen`, che non distingue FIRE da HOLD (`#1118`) |
...
| `Spec.Combat.GuardPoolSpansMultipleHits` | r5 | 7 | ⏱️ **Da [D-408] (2026-09-20) la Guardia non è più un pool ma una riduzione di 15 su OGNI colpo frontale, e questo file asserisce `120`**: tre colpi da 8 si azzerano uno per uno. *Fino ad allora misurava [D-292] — un **pool** da 15, non uno sconto sul primo colpo — e lasciava `116`, dove la regola precedente ancora lasciava `109`. Il file regge tutti e tre i modelli perché misura il caso a più colpi, che è l'unico dove la forma si vede; il suo NOME però nomina il pool, e il rename ha una issue propria.* ⚠️ **Scritto perché nessuno esercitava il caso che le distingue** (`#1919`): sopra i 15 del pool le due regole coincidono per costruzione, e il corpus era fatto di colpi da 16 in su. 🔑 **La distinzione è misurata, non dedotta** — mutando `ApplyAbsorptionPool` perché perda l'avanzo, questo scenario cade con `atteso 116, ottenuto 109`. ⚠️ *Al momento della misura era l'**unico** del corpus a cadere, e la riga lo diceva; oggi non è più vero e la correzione è dovuta a una code review: `Visual.Combat.GuardVsBraceUnderSmallHits` — aggiunto dalla stessa PR, poche ore dopo — mette lo stesso caso e cade identico. Il gemello visivo non toglie valore a questa riga: quello si guarda, questo giudica* |
| `Spec.Reaction.DeflectionPoolSpansMultipleHits` | r4 | 5 | il gemello di `Deflect` della riga qui sopra, e la stessa lacuna trovata due volte: [D-309](../../decisions/RT_PDR_00_Decision_Log.md) ha reso `Deflect` un **pool**, e il corpus non lo misurava perché era fatto di colpi grossi — sopra i 20 del budget pool e delta-sul-primo-colpo coincidono per costruzione. Quattro colpi da 8 contro un budget da 20 lasciano `83`, dove `ApplyFirstHitDelta` lascerebbe `71`. 🔑 **La distinzione è misurata, non dedotta**: mutando `Deflect` su `ApplyFirstHitDelta` questo scenario cade con `atteso 83, ottenuto 71`, ed è l'**unico** dei 117 a cadere — il `comm` fra i due bracci ha una sola riga. ⚠️ **Quattro colpi e non tre**, ma non perché tre non distinguerebbero (darebbero `79`, e senza reazione `71`): con tre l'attesa sarebbe `90`, cioè la vita piena, indistinguibile da un allestimento che non ha prodotto nessun colpo. 🔑 **Non sostituisce `Spec.Reaction.DeflectionReducesByTwenty`**, che misura il *numero* su una partita vera e dichiara di non misurare la *forma*: questo misura la forma. Aperto da `#2190` |
| `Spec.Harness.RepeatedRunsAgree` | r4 | 28 | il **determinismo del runner**: `repeatCount: 5` esegue lo stesso allestimento cinque volte e confronta le tracce byte per byte. Le assertion sono **28** e non 20: `RunRepeated` accumula le 4 di ogni giro e vi aggiunge, per ciascuno dei quattro confronti, un `SameTurnLogAcrossRuns` e un `SameStateHashAcrossRuns`. ⚠️ *La colonna diceva `4 ×5`, cioè il conto senza i confronti — lo stesso tipo di errore corretto nella riga qui sopra, commesso una riga più sotto e trovato da una code review*. ⚠️ **È il primo consumatore della chiave**, che fino al 2026-09-03 era implementata in tutto il percorso — loader, runner, writer, quattro test — e usata da **zero** dei 112 scenari versionati. Non è il corpus di E47.5 e non prova a esserlo: quel checkpoint ha sei casi con i loro nomi |
...
| `Visual.Combat.GuardReducesFirstHit` | r4 | `PIE-VIS-GUARD` | `Guard` toglie 15 a **ogni** colpo dell'arco frontale, e qui i colpi sono due: 120 → **112** ⏱️ *era «toglie 15 al primo colpo e finisce lì» — la regola pre-[D-292], rimasta qui anche dopo che quella l'aveva superata; riscritta da [D-408] il 2026-09-20* |
...
| `Visual.Map.BlockVolumes` | **BlockYard** | `PIE-HEX-VIZ-BLOCCHI` ⚠️ *senza prefisso `PIE-VIS-`, come `PIE-PC-GYM`* | i **tre** casi in fila — solo-movimento, entrambi, solo-vista — con `entrambi` in mezzo perché il confronto si fa col vicino |
| `Visual.Map.SightWallIsWalkable` | mapRadius | `PIE-VIS-SIGHTWALL` | la cella che nega il tiro **e si attraversa**: porta la lastra **senza** la colonna, ed è l'unico banco in cui la lastra si giudica da sola |
| `Visual.Map.TwoLayersSameColumn` | TestArena | `PIE-VIS-TWOLAYERS` | due unità sulla **stessa colonna**, piani diversi: si vedono **entrambe**, e a quote diverse |
...
| Mini v0.1 Autobattle (E47) | 5 | **Ciò che si guarda mentre gira da solo**: la board che si legge da due canali, e la partita non presidiata *registrata* invece che dichiarata. ➕ **La quarta è `PIE-V01-PREPWINDOW` (2026-09-06, [#2386](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2386)), e nessuno scenario può sostituirla — non per una capability mancante, ma **per costruzione**: la finestra di preparazione si arma in `OnPlanningTimeout`, e `RTScenarioSession` chiama `LockInAndResolve()` **diretto** (`:1477`) dopo `SetPlanningSeconds(0.f)` (`:946`). L'harness non attraversa quel ramo, ed è la proprietà per cui la finestra non mette tre secondi d'attesa in ogni run headless. ⛔ Uno scenario scritto per questa voce sarebbe **verde senza aver provato niente** ➕ **La quinta è `PIE-V01-SPECTATOR-ROSTER`** (2026-09-09, [#2744](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2744)), e la ragione è diversa dalla quarta: non è un ramo che l'harness non attraversa, è un **widget che l'harness non monta**. `RTScenarioSession` non costruisce lo Screen HUD, quindi «due liste a schermo» non ha nemmeno un soggetto in headless — e i test che esistono provano il **dato** che le due query restituiscono, non che una persona le distingua guardando |
...
| Scenari compositi di acceptance | 5 | **Che la scena sia allestita e leggibile** — le voci ombrello di `Visual.Perception.Acceptance` e `Visual.Hud.FirstPlayable`. I criteri restano dove sono |
...
**Somma: 171** — e la somma è il punto.
...
> **venti** sezioni di classe C, e la somma si è mossa di **34** — da `141` a `175`. La scomposizione, che
> deve chiudere per intero o non è un audit:
>
> · **12** dalle sei sezioni che mancavano del tutto — *La sonda di movimento* `3`, *Le coordinate sul
>   pavimento* `2`, *Velo e obiettivo* `2`, *Scenari compositi* `3`, *PC Gym* `1`, *Gate visivo end-to-end* `1`;
> · **22** da **otto** righe rimaste indietro — *Checklist* `48→56` (+8), *Il gesto dell'autore* `4→9` (+5),
>   *Contenuto della v0.1* `22→26` (+4), e a +1 ciascuna *M6*, *Strumenti di leggibilità*, *Mini Autobattle*,
>   *Scenario Test Harness* e *Durata, ritmo e scala*.
>
> `12 + 22 = 34`, e il totale torna. ⚠️ **La prima stesura di questa nota non chiudeva**: diceva `11 + 21 = 32`
> contro un salto di 34, perché contava *Scenari compositi* a `2` — mentre la sua stessa tabella dice `3`, da
> quando questa PR ha aggiunto `PIE-ACC-GUARDBRACE` — e dichiarava **sette** righe indietro invece di otto.
> Trovato da una code review. Una somma che non chiude in un audit dei conteggi è il difetto che l'audit
> cerca, commesso dall'audit: *«una somma giusta fa credere corretti gli addendi»*, e una sbagliata li fa
> credere tutti da rifare.
>
