...
> | `WithUnitId = 6` | `UnitId`, `TurnNumber` ([D-063](../decisions/RT_PDR_00_Decision_Log.md)) e `GraphRevision` ([D-065](../decisions/RT_PDR_00_Decision_Log.md)): tre int32 in coda alla voce, dopo `BaseActionId` | i primi due **no** — rendono la traccia spiegabile, non la discriminano. `GraphRevision` **sì**: due tracce possono differire solo per lei, ed è un'altra partita | leggibili, campi a `0` (`UnitId = 0` = nessuna unità) |
>
...
> [D-065](../decisions/RT_PDR_00_Decision_Log.md), pinnato da `TurnLog.CanonicalOrderCoversSerializedFields`.
