...
| **PIE-SCEN-FILTER** | I filtri restringono la tendina, **e la tendina si aggiorna** | `BP_GameMode` → Class Defaults → *RefactorTactics\|Test* | Con i filtri vuoti, `Scenario To Run` elenca tutti e 8 gli scenari. Impostando `Scenario Filter A = movement` la tendina scende a **6**; aggiungendo `Scenario Filter B = core` scende a **3** (`Movement.Basic`, `Movement.Collision`, `Movement.SwapRejectedByPlanning`). Il vocabolario dei due filtri contiene solo tag esistenti, con la voce vuota in testa | ⏳ **porta l'unico rischio tecnico della feature**: la logica è coperta headless (`ScenarioIndex.TwoFiltersIntersect`, `Scenario.AutoRunOptionsAreFilteredByTags`), ma nessun test può vedere se il Details Panel **ridisegna** la tendina al cambio di filtro. Se restasse ferma finché non si deseleziona e riseleziona l'actor, serve una `PostEditChangeProperty` che forzi il refresh (dipendenza `PropertyEditor`, oggi non nel modulo) |
| **PIE-SCEN-KEEP** | Filtrare **non perde** lo scenario selezionato | come sopra, con `Scenario To Run = Combat.BasicAttack` già salvato | Impostando `Scenario Filter A = movement` — che esclude `Combat.BasicAttack` dalla vista — la property **resta** su `Combat.BasicAttack`, e premendo Play parte quello. Il filtro è una vista, non un vincolo | ⏳ coperto headless da `Scenario.AutoRunOptionsAreFilteredByTags`; al PIE resta la conferma che il combo box **mostri** il valore corrente anche quando non è fra le opzioni, invece di apparire vuoto e far credere che la selezione sia andata persa |
