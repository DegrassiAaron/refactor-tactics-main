...
| **PIE-AS2** | Personaggio skeletal appoggiato a terra | un `BP_Unit_<Pack>` (`VisualZOffset=0`) registrato in **`HeroUnitClasses`** con la chiave del suo `HeroId` | Al posto del cilindro compare il personaggio, a terra (nessun «fluttuamento») | ✅ 2026-08-05 **sull'archetipo Guardian**, che non esiste più. ⏳ **da riverificare sui quattro BP di U7**: la precondizione citava `BP_Unit_Guardian` (Gideon) → `GuardianUnitClass`, e quel campo **è stato rimosso** — oggi la registrazione passa da `HeroUnitClasses`, una `TMap` per `HeroId` (riallineata 2026-08-11) |
