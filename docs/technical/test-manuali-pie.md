...
| **PIE-NAME** | I nomi sopra le unità sono quelli **canonici** | Partita 2v2 avviata; nessuna: il nome arriva dal catalogo via `ConfigureFromHeroData` | Sopra le quattro unità si legge **Gadget**, **Phase**, **Riktor**, **Wraith** — *non* `Flux`/`Riva`/`Bastion`/`Vektor`, che restano gli Stable ID e non si vedono a schermo ([D-120](../decisions/RT_PDR_00_Decision_Log.md)) | ⏳ — automazione già verde su `RefactorTactics.Heroes.CanonicalNamesReachTheLabel`, che pinna il percorso catalogo → unità → etichetta. Qui resta il solo giudizio **a schermo**: che l'etichetta si legga davvero, sopra la mesh giusta. Issue [#715](https://github.com/DegrassiAaron/refactor-tactics-main/issues/715) |
