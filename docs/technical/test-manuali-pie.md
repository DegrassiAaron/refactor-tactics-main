...
**114 voci**: ✅ **25 verdi** · 🟡 **21 parziali** (regola coperta da test, resta il visivo) · ⏳ **68 aperte**.

*(Rimisurate col comando qui sotto il **2026-08-09**, dopo le **3 voci** `PIE-VIS-GUARD`, `PIE-VIS-BRACE` e
`PIE-VIS-COORD`. Verdi e parziali non cambiano — le nuove nascono ⏳ — e `senza-marcatore` resta **0**.)*
...
Delle 68 aperte, **51 stanno negli otto gruppi qui sotto** (`2+9+9+4+3+1+2+21 = 51`). Le **17 mancanti** sono
...
| **PIE-PREVIEW-AREA** `RELEASE-V01` | Anteprima dell'area d'attacco e del fuoco amico | unità selezionata, un'abilità ad **area** o **cono** pianificata su un bersaglio | Le celle colpite si evidenziano in **rosso**; se un **alleato** è dentro l'area la sua cella è **arancione** — e si vede **prima** del lock-in. Le celle raggiungibili col budget corrente sono in **verde tenue** (il fango accorcia il raggio a vista d'occhio). Premendo Spazio l'anteprima **sparisce** | ⏳ **due difetti trovati e corretti il 2026-08-08, da riverificare — e ora si riproduce in un clic.** Ricetta: `BP_GameMode` → `Scenario To Run = Combat.FriendlyFire`, `Scenario Turn Pause Seconds = 30` (il tempo per guardare), poi Play. Lo scenario schiera Flux, Riva adiacente al bersaglio e Bastion, **seleziona Flux da solo** e punta `Overload` su Bastion: l'anteprima compare senza toccare mouse o tastiera, e resta ferma per tutta la pausa prima che il turno risolva. Lo fa il campo `previewUnit` dello scenario, che seleziona passando da `HandleClickOnUnit` — la stessa porta di un clic vero — ed è **presentazione**: headless non esiste un controller, il ramo non gira, e `Scenario.PreviewUnitDoesNotChangeTheOutcome` verifica che con e senza il campo l'esito e lo `StateHash` siano identici. **(a) L'arancione non si vedeva.** «Non vedo arancioni, forse il cilindro lo copre» — ed era esattamente così. L'arancione veniva disegnato (`FColor(255,150,30)`, contorno più largo del rosso) ma come `DrawDebugLine` con `DepthPriority=SDPG_World`, cioè **soggetto al test di profondità**, a 2,5 unità sopra il piano della cella: sopra a quel piano c'è il cilindro dell'unità, che da camera dall'alto lo copriva per intero. Il dettaglio che chiude la diagnosi: l'arancione **esiste solo su celle con un alleato sopra**, quindi era l'unico contorno sistematicamente nascosto — e con lui il rosso sul bersaglio. Corretto passando a `SDPG_Foreground` le sole celle che possono essere occupate (zona colpita e cella sotto il cursore); raggiungibili e traccia restano depth-tested, stanno per definizione su celle vuote. **(b) Si vedeva ma non si capiva.** Riprovata dopo la correzione: «si vede», ma subito dopo «non capisco se sto facendo un tiro e se nel tiro si interseca con un cilindro». Il difetto sotto è di **modello, non di colore**: l'anteprima parla di **celle**, la domanda di chi guarda è sulle **unità** — «questo cilindro lo prendo o no?». Due informazioni diverse, e c'era solo la prima. Aggiunti due segni, entrambi su elementi che esistevano già: il **nome sopra la testa** diventa arancione con `!` per un alleato nella zona, rosso con `*` per chiunque altro; e sopra la barra abilità compare `TIRO: N celle  -  M ALLEATO NELLA ZONA`, che distingue «sto scegliendo» da «sto per tirare». **Nessun test automatico può vedere nessuno dei due difetti**: il wiring era ed è verde (`Preview.HitCellsMatchCombatShape`, `Preview.AllyInAreaIsFlagged`, `Preview.ClearedWhenPlanIsCancelled`, `Preview.ReachableCellsArePassedThrough`) — l'anteprima riceveva le celle giuste e le disegnava prima dove non si vedevano, poi in un linguaggio che non rispondeva alla domanda |
