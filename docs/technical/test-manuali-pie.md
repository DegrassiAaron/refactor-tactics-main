...
| **PIE-FMT1** | La versione di formato finisce nei byte di un package **risalvato dall'editor** | Aprire `DA_HexMap_Arena` nell'editor, modificarlo in modo banale (una cella) e **salvare** | Ricaricandolo, il log di `RefactorTactics.HexMap.SerializedAssetMigratesWithoutGainingData` deve dire «asset risalvato dopo #687» invece di «pre-meccanismo», cioè `LoadedFormatVersion` **8** e non 0: è la prova che `FCustomVersionRegistry` scrive davvero nel summary del package su disco. ⚠️ Finché questa voce è ⏳, il lato-scrittura è verificato **su archivio in memoria** (`FormatVersionTravelsInSerializedBytes`) e sul caricamento dell'asset committato, non sui byte di un `.uasset` riscritto | ⏳ — aperta il **2026-08-14** con [#687](https://github.com/DegrassiAaron/refactor-tactics-main/issues/687). ⚠️ **Da fare su un branch o con l'asset ripristinato dopo**: risalvando l'arena si perde il solo binario pre-meccanismo del repository, che `FMT-2` ha scelto come rivelatore |
