...
| **PIE-TEST-VISUAL** | Il movimento si **vede**, non si teletrasporta | come sopra, dopo l'AUTO-RUN | L'unità `A1` si sposta da `(-2,0,0)` a `(-1,0,0)` **scorrendo** lungo il percorso, non saltando. A fine turno è esattamente sul centro della cella. ⚠️ Se invece appare già arrivata, il runner sta risolvendo più rapidamente di quanto il playback riesca a mostrare: **non è un difetto di simulazione** (i test headless verificano già l'esito) ma di presentazione, e va annotato qui — sarà la modalità *Visual* del harness a doverlo gestire | ❌ **RISPOSTA: non si vede** (misurato il 2026-08-08). Il runner risolve tutti i turni dentro `BeginPlay`: quando compare la schermata lo scenario è **già finito**. Il primo tentativo sembrò riuscito — «si vede» — ma il movimento osservato erano **turni fantasma** prodotti da un difetto (piani rimasti appesi + timer da 3 s che li ririsolveva), corretto in `4e6c2e0`. I timestamp lo dicono senza margine: `AUTO-RUN … 07:24:01.824`, poi turni 4, 5, 6… a 07:24:04, :07, :10. **Per vedere lo scenario giocare serve un runner latente** (uno step per frame invece che tutto in `BeginPlay`): non è più una previsione, è una misura |
