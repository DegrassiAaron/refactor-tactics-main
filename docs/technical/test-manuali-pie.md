...
|----|-----------------|---------------|--------------|-------|
| **PIE-BAL1** | `Guard` e `Brace` sono **leggibili come scelte diverse**? | Tre unità con `PushResistance = 0` (Gadget/Phase/Wraith): una in `Action.Guard`, una in `Action.Brace`, una senza difesa; una Phase avversaria usa `Riva.PressureJet` (16 danni **e** spinta 1 nello stesso colpo) su ciascuna, stesso turno | Guardando il turno **senza leggere i numeri**, «questo si è piantato» dev'essere distinguibile da «questo ha parato». Oggi le due difese producono lo **stesso** esito sulla spinta — nessuno dei due arretra — e differiscono solo di 5 danni: se non si distinguono, è un dato per `BAL-1`, non un difetto di presentazione | ⏳ — **non** in `RELEASE-V01`: `BAL-1` non blocca la consegna. Owner: [`../roadmap/plans/bal-1-guard-brace-roadmap-2026-08-10.md`](../roadmap/plans/bal-1-guard-brace-roadmap-2026-08-10.md) §6 |
...
| **PIE-HEXPLAY-2** `RELEASE-V01` | Selezione e cella sotto il cursore | partita hex avviata | Click su un'unità la seleziona; l'evidenziazione (esagono **giallo**) segue la cella **esagonale** sotto il mouse; su mappa multilivello si seleziona la cella del **layer giusto** — il layer viene dalla **quota** del punto colpito, quindi cliccando il ponte si evidenzia la cella del ponte | 🟡 **2026-08-06** — selezione ✅ (`Selezionata: RTUnit_1`, `RTUnit_0`), guardia sulle avversarie ✅ (`e' avversaria: seleziona prima una tua unita'`), evidenziazione gialla sotto il cursore ✅ confermata a schermo dall'utente. ⏳ resta il **layer su mappa multilivello**: l'arena di *ripiego* ha un solo layer, serve la mappa di prova — cioe' `MapSource=GeneratedTestArena`, che genera la piattaforma sul layer 1 sopra celle del layer 0 con gli stessi `q,r`. **Non serve l'arena di U1**: il nome esplicito e' qui perche' «mappa di prova» e' stato letto come l'asset da costruire (2026-08-10) |
...
| **PIE-HEXPLAY-10** `RELEASE-V01` | Partita completa fino alla vittoria | partita hex avviata, almeno un'unità per squadra col bot | La partita arriva a una **conclusione**: una squadra viene eliminata, compare «PARTITA FINITA» e `R` riavvia. Durante i turni: nessuna unità sovrapposta o fuori mappa, nessun blocco della pianificazione. **Dato di riferimento** (misurato headless il 2026-08-06): bot contro bot la partita si decide al **turno 10**, dentro il limite di 12 del catalogo. Era **25** finché lo scudo di supporto non scadeva e si accumulava (issue `#96`, risolta) | 🟡 **2026-08-10** — prima partita completa al PIE su `GeneratedTestArena`: e' arrivata a conclusione, ma **per scadenza dei round e non per eliminazione** — `Partita finita: Pareggio - allo scadere dei round (round 5/5, obiettivo 0-0, formato Format.Skirmish2v2)`, con il team 1 in vantaggio **2 contro 1** (`Eliminata: RTUnit_1 (team 0)`) e il bot che gia' puntava l'ultimo superstite (`attacca RTUnit_0 score=190`). Giudizio sul ritmo dall'utente: «un lampo». ⚠️ Quella run girava con `RoundLimit 5`, che rendeva il **dato di riferimento** qui accanto (turno 10) irraggiungibile per costruzione. **Corretto nello stesso giorno**: il formato spedito e' passato a `RoundLimit 12` / `ExpectedRounds 10`, allineato a D-010 (10-14 in 2v2), quindi il dato di riferimento torna confrontabile e la conclusione **per eliminazione** ridiventa possibile. ⏳ restano da osservare «PARTITA FINITA» a schermo, il riavvio con `R` e una partita che chiuda per eliminazione — **da rifare con il limite nuovo**: la run del 2026-08-10 non vale piu' come misura del ritmo. Coperta headless da `RefactorTactics.HexMatch.PlaysToCompletion` (tenuta e invarianti) |
| **PIE-HEXPLAY-11** | I colpi della fase Blast si rivelano **uno alla volta** ([#911](https://github.com/DegrassiAaron/refactor-tactics-main/issues/911)) | partita hex, almeno **due** attacchi che risolvono nello stesso Blast | Durante il playback della fase Blast i colpi non compaiono tutti insieme: si rivelano scaglionati, uno ogni `AttackShowSeconds`. ⚠️ **Oggi non può succedere e la voce nasce `⏳` per questo**: `ARTTurnManager::TickPlayback` ha due rami per `Blast` e il secondo — quello che scagliona — è **irraggiungibile**, perché `Blast` compare già nella condizione del primo (`RTTurnManager.cpp:5533` e `:5545`). Serve **più di un colpo nello stesso Blast** per falsificarla: con un colpo solo la voce passa anche col difetto presente. ⚠️ E il ritmo non decide nulla del risultato — se l'esito logico cambiasse col tempo di presentazione sarebbe l'invariante rotta, non una voce di registro | ⏳ |
...
| ID | Cosa verificare | Precondizione | Esito atteso | Stato |
|----|-----------------|---------------|--------------|-------|
| **PIE-GEO-GHOST** | Il ghost dice **prima** del rilascio se il segmento sarà accettato | tool Geometry attivo | Tenendo premuto e trascinando dentro una cella, una linea **verde** segue il gesto quando lo snap trova un segmento legale, **rossa** quando non lo trova (gesto troppo corto, o troppo lontano dalla cella). La differenza si vede **mentre** si trascina, non dopo. Domanda di giudizio: *capisci che quel gesto verrà rifiutato senza doverlo rilasciare?* | ⏳ |
| **PIE-GEO-SNAP** | Lo snap si sente, e il pannello dice cosa ha capito | tool Geometry attivo | Il ghost verde **non segue il mouse esattamente**: scatta sulle direttrici della grammatica. Il pannello mostra `SnappedAxis`, `SnappedOffset`, `SnappedFrom`, `SnappedTo` e si aggiorna durante il gesto. Tracciando lungo un lato dell'esagono, il ghost ci si appoggia **esattamente**. Domanda di giudizio: *lo scatto aiuta o combatte la mano?* | ⏳ |
| **PIE-GEO-UNDO** | **Una gesture = un `Ctrl+Z`** | almeno due segmenti disegnati | Un `Ctrl+Z` annulla **l'ultimo segmento intero** — non metà, e non due insieme. Un secondo `Ctrl+Z` annulla quello prima. `Ctrl+Y` li rifà nello stesso ordine. La copertura appare e sparisce dalla vista a ogni passo | ⏳ |
| **PIE-GEO-RESIDUI** | Uscire e rientrare non lascia niente dietro | almeno un segmento disegnato | Cambiando tool e tornando su Geometry, il ghost del gesto precedente **non c'è più**. Uscendo dall'Hex Map mode e rientrando, le coperture cotte **restano** (sono dato dell'asset) ma nessuna linea di preview sopravvive. `git status` sul `.umap` deve restare **pulito**: la geometria non si salva nel livello | ⏳ |
...
| **PIE-VIS-KO** | Eliminazione di un'unità | `Visual.Combat.Defeat` | Due colpi per turno, barra che scende due volte, e al **quarto** turno la rimozione. L'unità non deve sparire prima che il colpo sia arrivato. ⚠️ Erano **due** turni fino ad [ADR-0007](../decisions/adr-0007-attacco-base-per-eroe.md) (2026-08-09): `Bastion.ImpactShot` è passato da 24 a 8, la somma per turno da 45 a 29, e i 90 HP di Gadget non finiscono più in due. La seduta dura quindi il doppio, ed è la stessa cosa da guardare | ⏳ |
