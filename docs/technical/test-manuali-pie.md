...
| **PIE-NAME** | I nomi sopra le unità sono quelli **canonici** | — (il nome arriva dal catalogo via `ConfigureFromHeroData`, nessun asset da preparare) | Sopra le quattro unità si legge **Gadget**, **Phase**, **Riktor**, **Wraith** — *non* `Flux`/`Riva`/`Bastion`/`Vektor`, che restano gli Stable ID e non si vedono a schermo ([D-120](../decisions/RT_PDR_00_Decision_Log.md)) | 🟡 **2026-08-13** — **metà verificata a schermo**: **`Gadget`** e **`Phase`** si leggono sopra le unità, nitidi, in **due** frame indipendenti (turno 4 e turno 12, `-game` 1600×900), e **nessun nome legacy compare come etichetta** in nessun frame. ➕ Conferma non cercata: il combat log riporta `Vektor.Deflection` e `Bastion.Interposition` — sono **token di abilità**, cioè Stable ID, e devono restare così: i due piani di D-120 si vedono separati a schermo. ⏳ **`Riktor` e `Wraith` non sono mai comparsi in un frame**: non c'è prova che si vedano. 🔴 **E il motivo non è il naming** — inseguendo l'inquadratura è emerso che la sovrapposizione (nome **e** barre insieme) esce dal viewport quando l'unità è vicina alla camera, perché l'ancora è un offset **world space** proiettato senza clamp: [#729](https://github.com/DegrassiAaron/refactor-tactics-main/issues/729). Questa voce si rilegge **dopo** quella correzione, che è anche ciò che ha trovato. Issue [#715](https://github.com/DegrassiAaron/refactor-tactics-main/issues/715) |
