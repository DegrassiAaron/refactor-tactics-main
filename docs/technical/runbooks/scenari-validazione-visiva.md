...
| `Visual.Combat.UnbalancedAmplifiesPush` | r5 | due Wraith identici, la stessa `PressureJet`, una sola variabile: uno e' `Status.Unbalanced`. Chi e' in piedi arretra di **una** cella, chi e' sbilanciato di **due** e resta a terra. ⚠️ La domanda non e' la distanza — quella si conta — ma il **contraccolpo**: se si ripete due volte, una spinta amplificata si legge come due spinte. ➕ E nello stesso istante `Prone` nasce e `Unbalanced` muore: due fatti che il fotogramma puo' fondere in uno · **assertion**: `UnitAtCell B1 (-1,0,0)` · `UnitAtCell V1 (-2,-2,0)` · entrambi a **79** HP (lo stato non tocca il danno) | scritto |
| `Visual.Movement.ProneStandUpCosts` | r6 | due Wraith identici, la stessa rotta di sei celle, lo stesso budget: uno arriva in fondo, l'altro si ferma **una cella prima**. Quella cella e' il punto movimento dello StandUp. ⚠️ Il rischio non e' il conto ma la **lettura**: senza un momento proprio per il rialzarsi, resta un movimento troncato senza motivo · **assertion**: `UnitAtCell C1 (3,0,0)` · `UnitAtCell P1 (2,2,0)` | scritto |
...
| `Visual.Combat.GuardReducesFirstHit` | r4 | `Guard` toglie **15 a ogni colpo frontale**, e qui i colpi sono due (22 e 21): 120 → **112** (il `BaseShield` di D-224 ne assorbe altri 5). ⏱️ 🔴 *Era «un **pool** di 15 assorbibili, e qui il primo colpo da 22 lo esaurisce da solo: 120 → 97» fino a [D-408], il 2026-09-20. È la SECONDA riscrittura di questa cella, e le tre regole si succedute danno lo stesso numero su un colpo solo — che è la ragione per cui la cella ha potuto restare sbagliata due volte.* 🔴 *La cella diceva «toglie 15 al primo colpo e finisce lì», la regola che [D-292] ha superato il 2026-08-31: il numero non cambia — con un colpo sopra la soglia le due regole coincidono — ma la descrizione sì* | scritto |
...
| `Visual.Combat.GuardVsBraceUnderSmallHits` | r5 | **il confronto affiancato, sotto colpi PICCOLI**: tre difensori identici, tre `ImpactShot` da 8 a testa, tre risposte. 🔴 **Da [D-408] non c'è più un capovolgimento da guardare**: il `Brace` resta illeso (120) e **anche la Guardia** (120), perché una riduzione da 15 azzera un colpo da 8 come fa una da 10; solo chi non si difende perde 19 (101). ⏱️ *Era «la Guardia perde 4 (116) — il capovolgimento rispetto alle due righe qui sopra, dove con colpi grandi è la Guardia a dominare».* La scena va ri-puntata sul caso che oggi le distingue, un colpo **alle spalle**: [#3223](https://github.com/DegrassiAaron/refactor-tactics-main/issues/3223). Voce ombrello `PIE-ACC-GUARDBRACE`, seduta **U42** | scritto, `PASS 7/7` il 2026-09-03 |
...
| `Visual.Map.Acceptance` | RelayBasin | **composito 1↔N**: le tre cose che la fixture sa dire sul terreno costruito — la porta e la copertura appartengono a un **bordo**, l'altura non dà bonus — in tre turni, un tema per turno perché la linea del colpo attraversa la cella di partenza del Phase. Voce ombrello `PIE-ACC-MAP`; i criteri restano in `PIE-VIS-COVER`, `-DOOR`, `-HIGH` | scritto, `PASS 7/7` il 2026-09-03 |
| `Visual.Environment.Acceptance` | RelayLite | **composito 1↔N**: i cinque fenomeni di superficie della fixture in cinque turni — acqua+elettricità, ghiaccio, rough, fumo, fuoco e lo spegnimento — **un fenomeno per turno**, perché l'acqua conduce e due esperimenti nello stesso Blast potrebbero interferire. Voce ombrello `PIE-ACC-ENVIRONMENT`; i criteri restano nelle sei `PIE-VIS-*` che allestisce | scritto, `PASS 12/12` il 2026-09-03 |
...
| `Spec.Map.BridgeBreaksThePath` | `ArcModification` | un arco è **additivo**: romperlo non allunga il percorso, lo **annulla**. ⌫ Chiedeva `EnvironmentalActionOwner` fino al 2026-09-08, e quella è **disponibile**: il turno risultava giocabile, non trovava intents e lo scenario usciva `PASS 2/2` a vuoto ([#2549](https://github.com/DegrassiAaron/refactor-tactics-main/issues/2549)). Chi cabla le abilità ambientali agli eroi **non sblocca questo file**: aspetta `Action.ModifyArc`, che `D-046` lascia senza owner nel roster per decisione |
