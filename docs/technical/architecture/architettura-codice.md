...
| `Unit/RTUnit` | `AActor` + `IRTSelectable` | Team, cella (`FRTCellId`), HP/scudo, energia/ultimate; lista abilità `TArray<URTActionData*>`; piani (`PlannedCell`/`PlannedPath`/`PlannedWaypoints`/`PlannedAttackTarget`/`PlannedAbilityIndex`/`PlannedDashAbility`/`PlannedReactionAbility`); status (tag→turni); kiting; colore team (`M_Unit`); eliminazione. Il roster viene **interamente** dal catalogo eroi (`ConfigureFromHeroData`). ⚠️ Questa cella dichiarava che `ERTArchetype{Ranger,Guardian}` *«sopravvive come configurazione di test (`ConfigureAsArchetype`)»*: enum e funzione sono stati **rimossi** — `git grep ERTArchetype -- Source/` non risponde nulla |
| `Unit/RTAnimCatalogTypes.h`, `RTAnimCatalogLibrary` | `USTRUCT` + Function Library (pure) | **Owner del formato del catalogo animazioni** (#2445). `FRTAnimCatalog` (`formatVersion`, `nextId`, voci) letto da `Data/Anim/AnimCatalog.json` — **authoring source text-based versionato, fuori da ogni `.uasset`**. Ogni voce separa `Derived` (rigenerabile dall'asset) da `Authored` (giudizio umano: `Status`, `Label`, note), e la separazione e' **strutturale**, non una convenzione sui nomi. `AV_ID` (`AV_0001`) e' coniato **solo** da `AllocateIds` a partire dall'high-water mark persistito `nextId`, **mai** da `max(esistenti)+1`: e' cio' che impedisce a un ID rimosso di tornare su un'altra clip. Validator in due meta': `ValidateCatalog` **strutturale** (gira ovunque, vuoto = valido, **catalogo vuoto = NON valido**) e `ValidateReferents` **condizionale** — richiede i pack Paragon gitignorati (`.gitignore:105`) e con `bOutRan = false` obbliga chi chiama a scrivere `NOT RUN`, mai `PASS`. ⛔ Non entra in nessun percorso di decisione: TurnLog, ordinamento e `StateHash` non lo vedono. `Data/` **non** e' in `DirectoriesToAlwaysStageAsUFS`: non entra nel pacchetto |
| `RTAnimBrowserModel`, `SRTAnimBrowserPanel`, `SRTAnimPreviewViewport` (Editor) | modello puro + Slate | **Il browser delle animazioni** (#2443). Il MODELLO vive separato dal widget perche' resti qualcosa da misurare: Slate su un editor vivo non lo vede nessun automation test. ⛔ `ApplyUserStatus` e' l'**unico punto del progetto che scrive `Status`** — non e' una promessa, e' la struttura: nessun percorso automatico (scan, bind, filtro, caricamento) la attraversa. `BindToRole` rifiuta cio' che non e' `Promoted` e la variante entra **sempre inattiva**; `MakeActive` e' atomico; `Unbind` dell'attiva lascia il ruolo **senza attiva** e non elegge una sostituta. ⚠️ L'anteprima ha una `FPreviewScene` propria: `RTScenarioPreviewActor` mostra scenari esagonali e **zero** riferimenti a skeletal — misurato, non dedotto. La mesh viene dallo **Skeleton della clip**, e se manca la scena resta vuota **dichiarando perche'** |
| `Content/RTBuildAnimBindingsCommandlet` (Editor) | Commandlet | **Genera `/Game/RT/Anim/ABP_RTUnitAuthored`** dai binding del catalogo (#2443). E' il ponte fra il testo e il runtime: il pannello scrive un file diffabile, il gioco legge il C++. `D-229` — la sorgente e' il codice, l'asset e' il suo output, come `RTBuildGrayboxMeshes` e `RTBuildIconCatalog`. ⚠️ Cio' che lo rende utile e non cerimoniale: `ClipsPerHero` e' `EditDefaultsOnly` e `ApplyUnitAnimClass` dichiara che *«una Anim Class gia' scelta in Blueprint VINCE»*, quindi l'asset scavalca il default **senza ricompilare**. ⛔ Si rifiuta di generare da un catalogo non valido: due attive sullo stesso `(eroe, ruolo)` sono rappresentabili nel testo e non a runtime, e generare comunque farebbe dipendere la clip che suona dall'**ordine delle righe di un file**. Uso: `UnrealEditor-Cmd <progetto> -run=RTBuildAnimBindings [-DryRun]` — si avvia ed esce da solo |
