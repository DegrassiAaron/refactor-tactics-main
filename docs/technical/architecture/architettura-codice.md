...
| `RTGameMode` | `AGameModeBase` | **Composition root**, non allestitore: imposta pawn/controller/HUD, decide *scenario vs partita*, risolve le tre precedenze (`ResolveMapSource`/`ResolveAutobattle`/`ResolveMatchPlanningSeconds`), chiama il bootstrapper, collega frontend e presentation, avvia il replay. Il *come* nasce una partita sta in `Match/` |
