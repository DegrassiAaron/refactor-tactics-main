...
| `Turn/RTMatchFormatData` | `UPrimaryDataAsset` + `USTRUCT` | `URTMatchFormatData` (formato di partita: `FormatId`, `FormatVersion`, `RoundLimit`, `ExpectedRounds`, `ScoreToWin`) e `FRTMatchRules`, le regole **risolte** che il `TurnManager` legge — una sola verità per valore ([issue #185](spec-durata-partita-e-scala-mappe.md)) |
