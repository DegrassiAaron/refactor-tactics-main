...
| ID | Cosa verificare | Precondizione | Esito atteso | Stato |
|----|-----------------|---------------|--------------|-------|
| **PIE-HEXPLAY-1** | Allestimento della partita su mappa hex | livello di prova + GameMode hex | All'avvio del PIE si vede la griglia **esagonale** con 4 unità (2v2) **centrate sui centri-cella** (nessun offset né compenetrazione); la camera inquadra l'arena; nessun residuo della griglia quadrata | ⏳ **eseguibile da CP 6.1**. Tre cose da sapere: (1) le unità sono **cilindri** (i `BP_Unit_*` non esistono più, fallback previsto); (2) se il livello contiene un `ARTGridActor` **posato a mano** la griglia quadrata si vede ancora — il GameMode non ne crea più, ma non rimuove quelli già nel livello; (3) `L_Prototype` **non contiene una mappa esagonale**: dal 2026-08-06 il GameMode genera un'**arena di ripiego** (esagono r=4) e lo dice con un warning nel log. Verificato headless: la partita parte e i turni si susseguono. Posando un `ARTHexMapActor` col proprio `MapAsset` si gioca sulla mappa d'autore e il warning sparisce |
...
| **PIE-V01-DASHCOVER** | Dash contro copertura alta | copertura alta sulla traiettoria del Dash | Il Dash si ferma prima della copertura oppure è invalidato in pianificazione (nessun attraversamento, nessun crash) | 🟡 **coperto in parte headless 2026-08-06** da `RefactorTactics.HexMove.DashRefusesBlockedDestination`: lo scatto non entra in una cella che blocca il movimento e l'unità resta dov'era. ⏳ resta la regola del catalogo «scatto **lineare** che non attraversa muri»: oggi lo scatto usa l'A\* sul grafo e aggira gli ostacoli — è la migrazione di **CP 4.5** (issue #46), va verificata quando c'è |
