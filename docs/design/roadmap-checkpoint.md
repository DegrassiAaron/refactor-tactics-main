...
**Sviluppo consolidato su `main`**: l'MVP (M0–M5) è stato sviluppato in un unico branch di fase e poi mergiato; gli incrementi post-MVP usano feature branch dedicati. **166 test automatici verdi** (`Source/RefactorTactics/Tests/`; conteggio autorevole del repo, misurato 2026-08-05; include TurnLog P3 + hash + serializzazione versionata + checksum + I/O su file + **simulazione esagonale H6.1–H6.5**: snapshot/budget/collisioni, TurnLog e replay su hex, LOS e forme di targeting, bot con utility scoring esagonale).
