...
| **PF.4** · grafo multilivello | 🟡 | **motore ✅**: `FRTGridCoord`+`Layer`, `FRTTraversalEdge` (scale/portali/cross-layer), `ReachableCellsByGraph`/`FindPathByGraph` (Dijkstra edge-aware, deterministico) — TDD. **Mappa visiva/gameplay ⏳** (gated da un design di mappa multilivello) | idem §3 |
