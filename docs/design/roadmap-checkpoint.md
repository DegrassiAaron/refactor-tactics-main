...
|---|---|---|---|
| **6.0** | Riordino dei contenuti | Asset proprietari sotto `/Game/RT` con struttura **feature-first**, percorsi hard-coded (`DefaultEngine.ini`, default `TSoftObjectPtr` in C++) e `.gitignore` aggiornati, redirector risolti. **Prima** di creare gli asset di M6, così nascono già al posto giusto | Piano e mapping in [`convenzioni-contenuti-ue.md`](convenzioni-contenuti-ue.md) §A; PIE su `L_Prototype` + apertura di `L_DevSandbox` |
