...
struct FRTShowcaseSpawn
{
	...
	/** `HeroId` del catalogo eroi (`Hero.Gadget`, `Hero.Phase`, `Hero.Branth`, `Hero.Ivrin`), non un archetipo legacy. */
...
public:
	...
	/** Posizioni di partenza canoniche della showcase: Gadget + Phase (team 0) contro Branth + Ivrin (team 1). */
	...
	/** Spawn canonici del Relay Basin: Gadget `(-4,0)` + Phase `(-4,1)` contro Branth `(4,0)` + Ivrin `(4,1)`. */
	 ...
	 * (**5**, Phase e Branth; Ivrin 6, Gadget 7): una squadra vede quasi tutto al primo turno. E poiche'
