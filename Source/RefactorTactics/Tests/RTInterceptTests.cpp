...
namespace
{
	...
	ARTUnit* SpawnItcUnit(UWorld* World, int32 TeamId, const FRTCellId& Cell)
	{
		...
		U->ConfigureFromHeroData(URTHeroCatalogLibrary::MakeIvrin()); // attacco base a colpo singolo
...
bool FRTInterceptNoSecondOpportunityTest::RunTest(const FString&)
{
	...
	const int32 ShotRange = URTHeroCatalogLibrary::MakeIvrin()->Actions[0]->RangeCells;
