 ...
 */
/**
 * Nessuno scenario del corpus PASSA senza esercitare cio' che descrive (`#2549`).
 *
 * ## Il buco che questo test chiude
 *
 * `Spec.Map.BridgeBreaksThePath` passava `2/2` con `intents: []` in entrambi i turni: nessun movimento,
 * nessun arco che cade, nessun percorso che fallisce — e un verde nel conteggio. Il suo stesso `_nota` si
 * dichiara *«BLOCCATO PER DECISIONE … CP 9.4 resta verificato da questo scenario come SPECIFICA»*, ma il
 * runner non aveva modo di saperlo: chiedeva `EnvironmentalActionOwner`, che **e' disponibile** perche'
 * copre `Electrify` e `CreateWater`; cio' che gli serviva era `Action.ModifyArc`, che D-046 lascia senza
 * owner in v0.1.
 *
 * ⚠️ **Non era un caso isolato, ed e' la ragione per cui il rimedio non e' su quello scenario.** Misurati
 * il 2026-09-08: **cinque** file dichiarano turni e non hanno un solo intent — oltre al ponte, i tre
 * `Spec.Bot.*` e `Spec.Objective.HeldAgainstAnAdjacentEnemy`. Altri tre erano gia' `BLOCKED` per una
 * capability indisponibile, e li' il meccanismo funzionava.
 *
 * ## Perche' il criterio sono i turni DICHIARATI
 *
 * Undici scenari hanno `turns: []`, e sono legittimi: sono CONFIGURAZIONI, non regole.
 * `Visual.Map.TwoLayersSameColumn` mette due unita' sulla stessa colonna a quote diverse perche' una
 * persona guardi se il viewport perde il layer alto — non ha nulla da far accadere, e bocciarlo sarebbe
 * un falso positivo. Chi dichiara N turni promette invece che qualcosa avvenga in quegli N turni.
 *
 * 🔑 **Il test guarda l'ESITO del runner, non il file.** Rileggere il JSON qui dentro sarebbe una seconda
 * implementazione della regola, libera di divergere da quella che il runner applica davvero: e' lo stesso
 * difetto che `#140` ha chiuso fra `IsLinearReachable` e `ResolveLinearMove`. Qui si chiede al runner cosa
 * ha deciso, e si pretende che un vacuo non sia mai `Pass`.
 */
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FRTScenarioCorpusNoVacuousPassTest,
	"RefactorTactics.Scenario.NoShippedScenarioPassesVacuously",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)
bool FRTScenarioCorpusNoVacuousPassTest::RunTest(const FString&)
{
	...
	// ⚠️ **Controllo positivo, e senza di esso il test sarebbe vacuo quanto cio' che misura.** Se nessuno
	// scenario del corpus fosse mai vacuo, questo giro non guarderebbe niente e resterebbe verde per
	// assenza — la stessa forma di verde che esiste per vietare. Si conta quanti ne trova, e si pretende
	// che il meccanismo abbia avuto almeno un soggetto su cui pronunciarsi.
	int32 Vacui = 0;
	TArray<FString> PassatiAVuoto;

	for (const FString& Id : AllIds)
	{
		if (ExpectedToFail.Contains(Id)) { continue; }

		FString ResolveError;
		const FString Path = URTScenarioIndex::ResolvePath(Id, ResolveError);
		if (Path.IsEmpty()) { continue; } // l'indice lo riporta gia' in `EveryShippedScenarioRuns`

		FRTTestScenario Scenario;
		FString LoadError;
		if (!URTScenarioLoader::LoadFromFile(Path, Scenario, LoadError)) { continue; }

		if (Scenario.Turns.Num() == 0) { continue; } // configurazione: nulla da esercitare, ed e' dichiarato
		...
		if (bQualcunoAgisce) { continue; }

		++Vacui;

		FRTTestResult Result;
		if (!RunCorpusScenario(*this, Id, Result)) { continue; }

		if (Result.Outcome == ERTTestOutcome::Pass)
		{
			PassatiAVuoto.Add(Id);
		}
		else
		{
			AddInfo(FString::Printf(TEXT("%s: %d turni senza intent -> %s (corretto)"),
				*Id, Scenario.Turns.Num(), *Result.OutcomeString()));
		}
	}

	TestTrue(TEXT("il corpus contiene almeno uno scenario senza intent, altrimenti questo test non misura nulla"),
		Vacui > 0);

	if (PassatiAVuoto.Num() > 0)
	{
		AddError(FString::Printf(
			TEXT("%d scenari dichiarano turni, non esercitano nulla e il runner li dichiara PASS: %s"),
			PassatiAVuoto.Num(), *FString::Join(PassatiAVuoto, TEXT(", "))));
	}
	TestEqual(TEXT("nessuno scenario passa senza esercitare cio' che descrive"), PassatiAVuoto.Num(), 0);
	return true;
}
