...
namespace
{
	...
	{
		return FRTMoveRequest(FRTCellId(FromX, FromY), FRTCellId(ToX, ToY));
	}
}
...
bool FRTMoveSingleTest::RunTest(const FString&)
{
	...
	const TArray<FRTCellId> Out = URTMovementResolver::ResolveMoves(R);
	TestTrue(TEXT("si muove a (0,1)"), Out.Num() == 1 && Out[0] == FRTCellId(0, 1));
	return true;
}
...
bool FRTMoveContestedTest::RunTest(const FString&)
{
	...
	const TArray<FRTCellId> Out = URTMovementResolver::ResolveMoves(R);
	TestTrue(TEXT("A resta a (0,0)"), Out[0] == FRTCellId(0, 0));
	TestTrue(TEXT("B resta a (2,2)"), Out[1] == FRTCellId(2, 2));
	return true;
}
...
bool FRTMoveSwapTest::RunTest(const FString&)
{
	...
	const TArray<FRTCellId> Out = URTMovementResolver::ResolveMoves(R);
	TestTrue(TEXT("A -> (0,1)"), Out[0] == FRTCellId(0, 1));
	TestTrue(TEXT("B -> (0,0)"), Out[1] == FRTCellId(0, 0));
	return true;
}
...
bool FRTMoveBlockedTest::RunTest(const FString&)
{
	...
	const TArray<FRTCellId> Out = URTMovementResolver::ResolveMoves(R);
	TestTrue(TEXT("A bloccata a (0,0)"), Out[0] == FRTCellId(0, 0));
	TestTrue(TEXT("B resta a (0,1)"), Out[1] == FRTCellId(0, 1));
	return true;
}
...
bool FRTMoveVacatedTest::RunTest(const FString&)
{
	...
	const TArray<FRTCellId> Out = URTMovementResolver::ResolveMoves(R);
	TestTrue(TEXT("A -> (0,1)"), Out[0] == FRTCellId(0, 1));
	TestTrue(TEXT("B -> (0,2)"), Out[1] == FRTCellId(0, 2));
	return true;
}
...
bool FRTMoveOrderIndependentTest::RunTest(const FString&)
{
	...
	const TArray<FRTCellId> OutForward = URTMovementResolver::ResolveMoves(Forward);
	TArray<FRTCellId> OutBackward = URTMovementResolver::ResolveMoves(Backward);
	...
	TestTrue(TEXT("U0 resta a (0,0)"), OutForward[0] == FRTCellId(0, 0));
	TestTrue(TEXT("U1 resta a (2,2)"), OutForward[1] == FRTCellId(2, 2));
	TestTrue(TEXT("U2 -> (5,6)"), OutForward[2] == FRTCellId(5, 6));
	TestTrue(TEXT("U3 -> (5,5)"), OutForward[3] == FRTCellId(5, 5));
	return true;
}
...
namespace
{
	...
	bool PathResultEquals(const FRTPathResult& R, const FRTCellId& Final, const TArray<FRTCellId>& Entered)
	{
...
bool FRTResolvePathsBehaviorsTest::RunTest(const FString&)
{
	...
	{
		const TArray<TArray<FRTCellId>> P = { { FRTCellId(0,0), FRTCellId(1,0), FRTCellId(2,0) } };
		const TArray<FRTPathResult> R = URTMovementResolver::ResolvePaths(P);
		TestTrue(TEXT("libero: (2,0) attraversando (1,0)(2,0)"),
			PathResultEquals(R[0], FRTCellId(2,0), { FRTCellId(1,0), FRTCellId(2,0) }));
	}
	// (2) Destinazione contesa: entrambi fermi.
	{
		const TArray<TArray<FRTCellId>> P = {
			{ FRTCellId(0,0), FRTCellId(1,0) },
			{ FRTCellId(2,0), FRTCellId(1,0) } };
		const TArray<FRTPathResult> R = URTMovementResolver::ResolvePaths(P);
		TestTrue(TEXT("contesa: u0 fermo"), PathResultEquals(R[0], FRTCellId(0,0), {}));
		TestTrue(TEXT("contesa: u1 fermo"), PathResultEquals(R[1], FRTCellId(2,0), {}));
	}
	// (3) Scambio diretto: consentito.
	{
		...
		const TArray<FRTPathResult> R = URTMovementResolver::ResolvePaths(P);
		TestTrue(TEXT("scambio: u0 -> (1,0)"), PathResultEquals(R[0], FRTCellId(1,0), { FRTCellId(1,0) }));
		TestTrue(TEXT("scambio: u1 -> (0,0)"), PathResultEquals(R[1], FRTCellId(0,0), { FRTCellId(0,0) }));
	}
	// (4) Bloccata da unita' ferma.
	{
		const TArray<TArray<FRTCellId>> P = {
			{ FRTCellId(0,0), FRTCellId(1,0) },
			{ FRTCellId(1,0) } }; // u1 non si muove
		const TArray<FRTPathResult> R = URTMovementResolver::ResolvePaths(P);
		TestTrue(TEXT("bloccata: u0 fermo"), PathResultEquals(R[0], FRTCellId(0,0), {}));
		TestTrue(TEXT("ferma: u1 resta"), PathResultEquals(R[1], FRTCellId(1,0), {}));
	}
	// (5) Treno: u1 libera (1,0) mentre u0 vi entra -> entrambi avanzano (correttezza sincrona).
	{
		...
		const TArray<FRTPathResult> R = URTMovementResolver::ResolvePaths(P);
		TestTrue(TEXT("treno: u0 -> (1,0)"), PathResultEquals(R[0], FRTCellId(1,0), { FRTCellId(1,0) }));
		TestTrue(TEXT("treno: u1 -> (2,0)"), PathResultEquals(R[1], FRTCellId(2,0), { FRTCellId(2,0) }));
	}
	// (6) Troncamento: u0 avanza poi si blocca quando u1 si ferma davanti.
	{
		...
		const TArray<FRTPathResult> R = URTMovementResolver::ResolvePaths(P);
		TestTrue(TEXT("troncamento: u0 si ferma a (1,0)"), PathResultEquals(R[0], FRTCellId(1,0), { FRTCellId(1,0) }));
		TestTrue(TEXT("troncamento: u1 a (2,0)"), PathResultEquals(R[1], FRTCellId(2,0), { FRTCellId(2,0) }));
	}
	return true;
}
...
bool FRTResolvePathsOrderIndependenceTest::RunTest(const FString&)
{
	...
	const TArray<TArray<FRTCellId>> P = {
		{ FRTCellId(0,0), FRTCellId(1,0) },   // A: treno con B
		{ FRTCellId(1,0), FRTCellId(2,0) },   // B
		{ FRTCellId(4,0), FRTCellId(3,0) },   // C: contesa con D su (3,0)
		{ FRTCellId(2,0), FRTCellId(3,0) } }; // D
	TArray<TArray<FRTCellId>> Rev = P; Algo::Reverse(Rev);
