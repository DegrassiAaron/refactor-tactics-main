...
FName URTIconLibrary::MakeActionIconId(const FName& ActionId)
{
	...
	//
	// ⚠️ **Il percorso sopravvive INTERO, e l'eroe e' la parte che non si puo' buttare via.** Il dock
	// risponde a «quale abilita' e' questa», quindi due voci dello stesso kit devono avere chiavi diverse
	// — tradurre verso la core le farebbe collassare, e `TideGuard` e un secondo scudo mostrerebbero lo
	// stesso disegno.
	//
	// 🔴 **Ma tenere il solo NOME non bastava, e il difetto era invisibile da qui** (`#2963`). Il glifo
	// di un'abilita' d'eroe **porta il sigillo del suo eroe**: `generate_hud_assets.py` compone
	// `hero_sigil(hero_of(semantic))` dentro il disegno, e `hero_of` legge l'eroe dal segmento centrale.
	// ∴ l'asset E' specifico d'eroe, e una chiave che l'eroe lo scarta non puo' indirizzarlo. Il
	// generatore lo sapeva e scriveva `Action.Hero.Aevik.Overload`; qui si chiedeva `Action.Overload`,
	// e i venti glifi disegnati restavano irraggiungibili con il loro nome giusto in mano.
	//
	// ⛔ **Non e' una preferenza di stile fra due nomi**: uno dei due indirizza un asset che esiste e
	// l'altro no. La regola vive in due linguaggi — qui e nel generatore — ed e' cio' che le ha fatte
	// divergere; `IconIdMatchesTheGeneratorNaming` le pinna l'una contro l'altra.
	FString Path = ActionId.ToString();
	if (Path.Contains(TEXT(".")))
	{
		return MakeIconId(FName(*FString::Printf(TEXT("%s.%s"),
			*CategoryName(ERTIconCategory::Action), *Path)));
	}

	// ⛔ Un id senza punto non ha un percorso da qualificare. Non si compone `UI.Icon.Action.` a vuoto: la
...
TArray<FName> URTIconLibrary::RequiredIconIds()
{
	...
	// ── ABILITA' D'EROE SENZA RIPIEGO — e la condizione e' la regola, non un elenco.
	//
	// 🔑 **Il criterio e' «non ha dove degradare», non «e' d'eroe».** Un'abilita' DERIVATA mostra l'icona
	// della core da cui viene — `TideGuard` quella di `Action.Shield` — ed e' il ripiego che fa il suo
	// mestiere: pretenderne una propria chiederebbe un disegno per qualcosa che gia' si vede. Un'abilita'
	// PROPRIA non ha quella via: `MakeActionIconFallbackId` torna `NAME_None` per progetto, e senza icona
	// il dock mostra `MissingIcon`. E' il difetto misurato in PIE che ha aperto `#2963`.
	//
	// ⚠️ **Si autoalimenta, ed e' il punto**: un'abilita' propria nuova entra qui da sola, il commandlet la
	// pretende fail-closed, e chi la aggiunge deve disegnarne il glifo. Un elenco scritto a mano sarebbe
	// invece scaduto il giorno dopo — lo stesso difetto che `RequiredIconIds` evita gia' per le azioni core.
	//
	// ⛔ **La categoria resta `Action`**: `Action.Hero.Aevik.Overload` ha `Action` come segmento di
	// categoria, quindi il validator lo accetta e `V01CategoriesPopulated` non vede una tredicesima
	// categoria comparire. `Hero` non e' fra le dodici di `D-031`, e non lo diventa qui.
	for (const URTHeroData* Hero : URTHeroCatalogLibrary::GetHeroRoster())
	{
		...
		for (const URTActionData* Action : Hero->Actions)
		{
			...
			if (MakeActionIconFallbackId(Action->Def).IsNone())
			{
				Ids.AddUnique(MakeActionIconId(Action->Def.ActionId));
			}
		}
	}
