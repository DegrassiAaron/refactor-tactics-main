	...
	const FRTAbilityLabFixtureSpec& Spec, FRTTestScenario& OutScenario, FString& OutError)
{
	...
	if (!FindHero(HeroId, Hero))
	{
		OutError = FString::Printf(
			TEXT("HeroId non canonico: '%s'. Il roster della v0.1 e' Hero.Gadget, Hero.Phase, Hero.Branth, Hero.Ivrin."),
			*HeroId.ToString());
		return false;
	}

	// L'unica domanda che l'Ability Lab non puo' porre. Per #2599 ogni ability canonica e' eseguibile; qui
	// conta anche DI CHI e'. Senza questo controllo Hero Lab farebbe lanciare a Gadget un'abilita' di Ivrin
