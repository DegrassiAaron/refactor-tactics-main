...
namespace
{
	// 🔴 **Nomi PREFISSATI, e non e' pedanteria: la unity build condivide la translation unit.**
	// `RTRemoveEnergyBarCommandlet.cpp` dichiara nel PROPRIO namespace anonimo un `OverlayPath` e un
	// `SaveWidgetPackage` — gli stessi nomi che questo file aveva alla prima stesura, per la buona ragione
	// che fanno la stessa cosa sullo stesso asset. Finche' i due file finivano in blob diverse compilava;
	// il giorno in cui un merge altrove ha cambiato le dimensioni e li ha messi nella stessa, e' uscito
	// `C2374: ridefinizione` su un file che il diff non toccava. ⚠️ **«Il mio diff non tocca quel file» non
	// assolve**: il raggruppamento e' per dimensione, quindi la collisione la decide un cambiamento di
	// qualcun altro. Un prefisso distintivo la rende impossibile invece che improbabile.
	const TCHAR* DamageTokenOverlayPath = TEXT("/Game/RT/UI/Match/WBP_RT_UnitOverlay.WBP_RT_UnitOverlay");

	/** Il nome e' la MANIGLIA del `BindWidgetOptional`: cambiarlo qui scollega il C++ in silenzio. */
	const TCHAR* DamageTokenWidgetName = TEXT("DamageTokenText");

	/** Il contenitore verticale che l'asset gia' porta, con dentro nome, vita, scudo e stati. */
	const TCHAR* DamageTokenRootBoxName = TEXT("RootBox");
	 ...
	 */
	constexpr int32 DamageTokenFontSize = 26;
	 ...
	 */
	constexpr int32 DamageTokenOutlineSize = 2;

	/** Stile del token: applicato sia quando nasce sia quando esiste gia', cosi' il comando e' RIPETIBILE. */
	void ApplyDamageTokenStyle(UTextBlock* Token)
	{
		FSlateFontInfo Font = Token->GetFont();
		Font.Size = DamageTokenFontSize;
		Font.OutlineSettings.OutlineSize = DamageTokenOutlineSize;
	...
	bool SaveDamageTokenOverlayPackage(UPackage* Package, UBlueprint* Blueprint)
	{
...
int32 URTAddDamageTokenCommandlet::Main(const FString& Params)
{
	...
	UWidgetBlueprint* Overlay = LoadObject<UWidgetBlueprint>(nullptr, DamageTokenOverlayPath);
	if (!Overlay || !Overlay->WidgetTree)
	{
		UE_LOG(LogRTDamageToken, Error, TEXT("overlay non caricato: %s"), DamageTokenOverlayPath);
		return 1;
	}

	// ------------------------------------------------------------------ 1. c'e' gia'?
	if (UWidget* Existing = Overlay->WidgetTree->FindWidget(FName(DamageTokenWidgetName)))
	{
		if (!Cast<UTextBlock>(Existing))
		{
			...
			UE_LOG(LogRTDamageToken, Error,
				...
				DamageTokenWidgetName, *Existing->GetClass()->GetName());
			return 1;
		}
		...
		ApplyDamageTokenStyle(Presente);
		...
		UE_LOG(LogRTDamageToken, Display,
			...
			DamageTokenWidgetName, DamageTokenFontSize, DamageTokenOutlineSize);
		...
		if (!SaveDamageTokenOverlayPackage(Overlay->GetOutermost(), Overlay))
		{
	...
	// ------------------------------------------------------------------ 2. dove va
	UVerticalBox* RootBox = Cast<UVerticalBox>(Overlay->WidgetTree->FindWidget(FName(DamageTokenRootBoxName)));
	if (!RootBox)
	{
	...
	if (bDryRun)
	{
		UE_LOG(LogRTDamageToken, Display,
			...
			DamageTokenWidgetName, DamageTokenFontSize, DamageTokenRootBoxName, RootBox->GetChildrenCount());
	...
	UTextBlock* Token = Overlay->WidgetTree->ConstructWidget<UTextBlock>(
		UTextBlock::StaticClass(), FName(DamageTokenWidgetName));
	if (!Token)
	{
		UE_LOG(LogRTDamageToken, Error, TEXT("costruzione di '%s' fallita"), DamageTokenWidgetName);
		return 1;
	}

	ApplyDamageTokenStyle(Token);
	...
	UE_LOG(LogRTDamageToken, Display,
		...
		DamageTokenWidgetName, DamageTokenRootBoxName, RootBox->GetChildrenCount());
